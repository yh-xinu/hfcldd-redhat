/*
 * hfcl_detect.c
 * Copyright (C) 2007, 2020, Hitachi, Ltd.
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
#include <linux/blkdev.h>	/* FCLNX-GPL-FX-683 */
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
uint	hfc_rhel75 = 0;		/* FCLNX-GPL-FX-683 */
#endif
								...
								if ( !( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.lg_target_info ) )  {
									if(ap->ld_err_limit_s) {
										if ( test_bit(HFC_WAITING_DEV_LOSS_TMO, (ulong *)&target->flags) ) {
											hfc_watched_errcount_i(ap, target, HFC_TGT_LDS_ERR);
										}
									}
								}
#endif							/* FCLNX-GPL-FX-733 End */
								set_bit(HFC_NEED_LOGIN, (ulong *)&target->status);
								clear_bit(HFC_NEED_CANCEL, (ulong *)&target->status);					/* FCLNX-GPL-038 */
...
{
	/* Set Default Parameter *//* FCLNX-GPL-FX-684 */
	if (hfc_manage_info.hfcldd_mp_mod) {
		ap->max_lun = HFC_DEFAULT_LUN;
	} else {
...
void hfc_fx_set_vport_count(struct port_info *pp)
{
	...
	pp->max_vport_num   = HFC_FX_DF_VPORT_COUNT; /* FCLNX-GPL-FX-558 */
	if(pp->defparam) return;
	...
	if(hfc_chk_conf_val(HFC_FX_MIN_VPORT_COUNT,HFC_FX_MAX_VPORT_COUNT,hfc_vport_count)){ /* in case global parameter is set*/
		...
		pp->max_vport_num = hfc_vport_count; /* FCLNX-GPL-FX-558 */
	}
	...
	if( hfc_chk_conf_val(HFC_FX_MIN_VPORT_COUNT,HFC_FX_MAX_VPORT_COUNT,hfcmp_vport_count[pp->instance]) ) {
		...
		pp->max_vport_num = hfcmp_vport_count[pp->instance]; /* FCLNX-GPL-FX-558 */
	}
}
...
{
//	pp->mq_num = HFC_FX_DF_MQ_NUM; /* FCLNX-GPL-FX-547 */
//	pp->mq_num = HFC_FX_MAX_MQ_NUM; /* FCLNX-GPL-FX-547 */
	/* FCLNX-GPL-FX-602 start ------ */
	hfc_fx_calc_cpu_num(pp);
	if( HFC_FX_MAX_MQ_NUM >= pp->manage_info->online_cpu_num){
		pp->mq_num = pp->manage_info->online_cpu_num;
	}else if(HFC_FX_MAX_MQ_NUM >= pp->manage_info->cpu_core_num){
		pp->mq_num = pp->manage_info->cpu_core_num;
	}else{
		pp->mq_num = HFC_FX_MAX_MQ_NUM;
	}
	/* ------ FCLNX-GPL-FX-602 end */
...
void hfc_fx_set_mq_enable(struct port_info *pp)
{
	...
	if(HFC_FX_MMODE_CHECK_MLPF(pp)) return; /* FCLNX-GPL-FX-547 */
...
/* Set xlog mode */ /* FCLNX-GPL-FX-582,595 >>>*/
void hfc_fx_set_xlog_func(struct port_info *pp)
{
	/* Set Default Parameter */
	pp->hvm_extended_log = HFC_HVM_EXTENDED_LOG_DISABLE;
	...
	/* in case global parameter is set */
	if(hfc_chk_conf_val(0, 1, hfc_hvm_extended_log)){
		pp->hvm_extended_log = (uchar)hfc_hvm_extended_log;
	}	
	return;
}
/* >>>FCLNX-GPL-FX-582 */
...
void hfc_conf_setup(struct adap_info *ap)
{
	...
	/* FCLNX-GPL-FX-611 >>> */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 8, 0) /* FCLNX-GPL-FX-597,611 */
#if !defined(BLK_MQ_ONLY)
	if (shost_use_blk_mq(ap->hosts))
#endif
	{
		ap->hosts->nr_hw_queues = 1;
	}
#endif
	/* FCLNX-GPL-FX-611 <<< */
...
void hfc_param_setup(void)
{
	...
	/* "hfc_hvm_extended_log" */ /* FCLNX-GPL-FX-592,595 */
	if (hfc_param_search("hfc_hvm_extended_log", &value)) 
		hfc_hvm_extended_log = value;
...
#if defined(HFC_RHEL81) /* FCLNX-GPL-FX-713 start */
static int hfc_map_queues(struct Scsi_Host *shost) {
	int rc = -EINVAL; /* Invalid argument */
	if (shost) {
		struct blk_mq_queue_map *queue_map = &shost->tag_set.map[0];
		rc = blk_mq_map_queues(queue_map);
	}
	return rc;
}
#endif /* defined(HFC_RHEL81) */ /* FCLNX-GPL-FX-713 end */
...
void hfc_set_pub_symbol_list(void)
{
	...
	pubp->hfc_fx_choose_core				=	hfc_fx_choose_core;			/* FCLNX-GPL-FX-645 */
...
#if defined(HFC_RHEL81) /* FCLNX-GPL-FX-713 start */
	pubp->hfc_map_queues					=	hfc_map_queues;
#endif /* defined(HFC_RHEL81) */ /* FCLNX-GPL-FX-713 end */
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
	if (hfc_manage_info.npubp->hfc_mp_scsi_host_alloc_new == NULL)			rtn++;	/* FCLNX-GPL-FX-683 */
#endif
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
	if (hfc_manage_info.npubp->hfc_fx_mp_scsi_host_alloc_new == NULL)		rtn++;	/* FCLNX-GPL-FX-683 */
#endif
...
hfc_slave_destroy(struct scsi_device *sdev)
{
	...
	struct dev_info		*temp_dev2 = NULL;	/* FCLNX-GPL-FX-614 */
	struct target_info	*target = NULL;
	ulong				flags = 0;
		...
		if( dev != NULL ){
			if( !test_bit(HFC_DEVINF_VALID, (ulong *)&dev->flags) ){
				...
				if( target != NULL){
					...
					while( temp_dev != NULL ){
						if( temp_dev == dev ){
							HFC_DBGPRT("hfc_slave_destroy  - 1\n");
							break;
						}
					...
					if( temp_dev != NULL){
						if( temp_dev == target->dev ){
							...
							if( temp_dev2 != NULL){
								HFC_DBGPRT("hfc_slave_destroy - 7\n");
								temp_dev2->prev = NULL;
							}
							/* FCLNX-GPL-FX-614 <<< */
							
							HFC_DBGPRT("hfc_slave_destroy - 2\n");
						}
						else{
							temp_dev = temp_dev->prev;
							HFC_DBGPRT("hfc_slave_destroy - 3\n");
							if( temp_dev != NULL){
								/* FCLNX-GPL-FX-614 >>> */
								temp_dev->next = dev->next;
								HFC_DBGPRT("hfc_slave_destroy - 4\n");
								temp_dev2 = dev->next;
								if( temp_dev2 != NULL){
									temp_dev2->prev = temp_dev;
									HFC_DBGPRT("hfc_slave_destroy - 5\n");
								}
								/* FCLNX-GPL-FX-614 <<< */
							}
						}
				...
				HFC_DBGPRT("hfc_slave_destroy - 6\n");
				memset( dev, 0, sizeof(struct dev_info) );
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
static int hfc_change_queue_type_new(struct scsi_device *sdev, int tag_type)			/* FCLNX-GPL-FX-683 */
{
...
void hfc_scsi_activate_tcq(struct scsi_device *sdev, int depth, int alloc_policy)	/* FCLNX-GPL-FX-683 */
{
	if (!sdev->tagged_supported)
		return;
#if defined(BLK_MQ_ONLY)
	...
	if (shost_use_blk_mq(sdev->host))
		...
		if( !hfc_rhel75 ){
			hfc_blk_queue_init_tags(sdev->request_queue, depth,
				    sdev->host->bqt);
		}
		else{
			hfc_blk_queue_init_tags_new(sdev->request_queue, depth,
				    (struct blk_queue_tag_new *)sdev->host->bqt, alloc_policy);
		}
	}
#endif
...
/* FCLNX-GPL-FX-683 */
static struct blk_queue_tag *__hfc_blk_queue_init_tags(struct request_queue *q,
						   int depth)
{
	...
	if (hfc_init_tag_map(q, tags, depth))
		goto fail;
...
/* FCLNX-GPL-FX-683 */
static struct blk_queue_tag_new *__hfc_blk_queue_init_tags_new(struct request_queue *q,
						   int depth, int alloc_policy)
{
	struct blk_queue_tag_new *tags_tmp;

	tags_tmp = kmalloc(sizeof(struct blk_queue_tag_new), GFP_ATOMIC);
	if (!tags_tmp)
		goto fail;

	if (hfc_init_tag_map_new(q, tags_tmp, depth))
		goto fail;

	atomic_set(&tags_tmp->refcnt, 1);
	
	tags_tmp->alloc_policy = alloc_policy;
	tags_tmp->next_tag = 0; 
	return tags_tmp;
fail:
	kfree(tags_tmp);
...
/* FCLNX-GPL-FX-683 */
int hfc_blk_queue_init_tags(struct request_queue *q, int depth,
			struct blk_queue_tag *tags)
{
	int rc;
	...
	if (!tags && !q->queue_tags) {
		...
		tags = __hfc_blk_queue_init_tags(q, depth);
...
/* FCLNX-GPL-FX-683 */
int hfc_blk_queue_init_tags_new(struct request_queue *q, int depth,
			struct blk_queue_tag_new *tags_new, int alloc_policy)
{
	int rc;

	if (!tags_new && !q->queue_tags) {
		
		tags_new = __hfc_blk_queue_init_tags_new(q, depth, alloc_policy);
		...
		atomic_inc(&tags_new->refcnt);

	/*
	 * assign it, all done
	 */
	q->queue_tags = (struct blk_queue_tag *)tags_new;
	queue_flag_set_unlocked(QUEUE_FLAG_QUEUED, q);
	INIT_LIST_HEAD(&q->tag_busy_list);
	return 0;
...
/* FCLNX-GPL-FX-683 */
int hfc_init_tag_map(struct request_queue *q, struct blk_queue_tag *tags, int depth)
{
...
/* FCLNX-GPL-FX-683 */
int hfc_init_tag_map_new(struct request_queue *q, struct blk_queue_tag_new *tags_new, int depth)
{
	struct request **tag_index;
	unsigned long *tag_map;
	int nr_ulongs;
...
//		       __func__, depth);
	...
	tags_new->real_max_depth = depth;
	tags_new->max_depth = depth;
	tags_new->tag_index = tag_index;
	tags_new->tag_map = tag_map;
...
void hfc_kernel_check(void)
{
	...
	memset(buf,0,sizeof(buf));
	strncpy(buf, init_utsname()->release, sizeof(init_utsname()->release));
	j = sizeof(init_utsname()->release);
	
	for(i=0; i<j; i++){
		buf_a[i] = (int)(buf[i]-'0');
//		HFC_ERRPRT("hfcldd buf[%d] = %d   %c   buf_a[%d] = %d  %c\n", i, buf[i], buf[i], i, buf_a[i], buf_a[i]);
	}
	
	ker_ver = buf_a[7]*100 + buf_a[8]*10 + buf_a[9];
	
	if( ( ker_ver == 229 ) || ( ker_ver == 327 ) || ( ker_ver == 514 ) || ( ker_ver == 693 )) {
		/* Check 7.1, 7.2, 7.3 or 7.4 */
		hfc_rhel75 = 0;
	}
	else{
		hfc_rhel75 = 1;
...
{
	off_t    offset        = 0;
	int      max_length    = PAGE_SIZE-1; /* kernel's PAGE_SIZE */ /* FCLNX-GPL-FX-537 */
	struct adap_info	*ap;
...
{
	off_t    offset        = 0;
	int      max_length    = PAGE_SIZE-1; /* kernel's PAGE_SIZE *//* FCLNX-GPL-FX-701 */
	struct adap_info	*ap;
...
struct scsi_host_template hfcldd_driver_template = {
	...
	.proc_dir=					NULL,					
	.proc_name=					"hfcldd",
#if defined(HFC_RHEL7) || defined(HFC_X8664_SLES12)|| defined(HFC_X8664_OEL7)|| defined(HFC_RHEL8)	/* FCLNX-GPL-FX-699 */
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
	.change_queue_type=			hfc_change_queue_type_new,	/* FCLNX-GPL-450 *//* FCLNX-GPL-FX-683 */
#else
	.change_queue_type=			hfc_change_queue_type,
#endif
#endif /* FCLNX-GPL-FX-496 end */
...
#endif /* KERNEL_VERSION(2,6,16) */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) && LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, 0) /* FCLNX-GPL-FX-534 */
	.use_blk_tags =				1,
#endif
	/* FCLNX-GPL-FX-527-3 start */ 
#if !defined(HFC_X8664_SLES12SP3) && !defined(HFC_RHEL8) && ((LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(4, 8, 0))) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-597,611 */
	.disable_blk_mq =			1,
#endif
/* FCLNX-GPL-FX-527-3 end */
};

/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
/* FCLNX-GPL-FX-683 */
struct scsi_host_template_new hfcldd_driver_template_rhel75 = {
...
//	.proc_info=					hfc_proc_info_k26,			
	...
	.change_queue_depth=		hfc_change_queue_depth,						/* FCLNX-GPL-450 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 19, 0) /* FCLNX-GPL-FX-496 start */
	.change_queue_type=			hfc_change_queue_type_new,						/* FCLNX-GPL-450 */
#endif /* FCLNX-GPL-FX-496 end */
	.queuecommand=				hfc_strategy,
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,17) && !defined(HFC_X8664_SLES10) /* FCLNX-0309 */
	.eh_strategy_handler=		NULL,
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
static struct scsi_host_template_new hfcldd_driver_template_mp_rhel75 = {
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
static struct scsi_host_template_new hfcldd_driver_template_platform_bus_rhel75 = {		/* FCLNX-GPL-204 */
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
		hfc_kernel_check();		/* FCLNX-GPL-FX-683 */
#endif
		instance++;
...
#ifdef HFC_RHEL7
		/* FCLNX-GPL-FX-683 */
		hfc_manage_info.hfcldd_driver_template_rhel75 = &hfcldd_driver_template_rhel75;
		hfc_manage_info.hfcldd_driver_template_mp_rhel75 = &hfcldd_driver_template_mp_rhel75;
		hfc_manage_info.hfcldd_driver_template_platform_bus_rhel75 = &hfcldd_driver_template_platform_bus_rhel75;
#endif
...
#if !defined(HFC_X8664_SLES12SP3) && !defined(HFC_RHEL8) && ((LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(4, 8, 0))) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-597,611 */
                {       /* Set driver_template */
                        bool disable_blk_mq = TRUE; /* Default */
                        if (pdev->device == HFC_PCI_DEVICE_ID_3070) {
                                        /* FIVE-FX multi queue Only */
                                        disable_blk_mq = FALSE; /* enable scsi-mq feature for hfcldd driver */
                        }
						/* FCLNX-GPL-FX-601start ------ */
						/* second kernel default disable_blk_mq */
						if( (is_kdump_kernel()) || (hfc_manage_info.hfcldd_mp_mod) ){	/* FCLNX-GPL-FX-624 */
							disable_blk_mq = TRUE;
						}
			/* ------ FCLNX-GPL-FX-601 end */
                        hfc_manage_info.hfcldd_driver_template->disable_blk_mq = disable_blk_mq;
                        hfc_manage_info.hfcldd_driver_template_mp->disable_blk_mq = disable_blk_mq;
                        hfc_manage_info.hfcldd_driver_template_platform_bus->disable_blk_mq = disable_blk_mq;
                       
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
                        /* FCLNX-GPL-FX-683 */
                        hfc_manage_info.hfcldd_driver_template_rhel75->disable_blk_mq = disable_blk_mq;
                        hfc_manage_info.hfcldd_driver_template_mp_rhel75->disable_blk_mq = disable_blk_mq;
                        hfc_manage_info.hfcldd_driver_template_platform_bus_rhel75->disable_blk_mq = disable_blk_mq;
#endif
                }
#endif
...
/* FCLNX-GPL-FX-689 */
	...
	if( !hfc_rhel75 ){
		...
		if ( hfc_manage_info.hfcldd_mp_mod ) {
			host = hfc_scsi_host_alloc_new(&hfcldd_driver_template_mp_rhel75, sizeof(struct adap_info));
		} 
		else {
			if( hfc_rport_lu_scan != 1){
				host = hfc_scsi_host_alloc_new(&hfcldd_driver_template_mp_rhel75, sizeof(struct adap_info));
				...
				host = hfc_scsi_host_alloc_new(&hfcldd_driver_template_rhel75, sizeof(struct adap_info));
			}
		}
	}
	/* FCLNX-GPL-565 end *//* FCLNX-GPL-575 end */
	...
	/* FCLNX-0429 */	/* FCLNX-0651 *//* FCLNX-GPL-FX-683 */
	if (hfc_manage_info.hfcldd_mp_mod) {			/* FCLNX-GPL-204 */
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
		if( !hfc_rhel75){
			...
			vrt_host_alloc = hfc_manage_info.npubp->hfc_mp_scsi_host_alloc_new(&hfcldd_driver_template_platform_bus_rhel75, 
															host, ap, &hsd_host, &pfb_host);
		}
#else
		vrt_host_alloc = hfc_manage_info.npubp->hfc_mp_scsi_host_alloc(&hfcldd_driver_template_platform_bus, 
															host, ap, &hsd_host, &pfb_host);	/* FCLNX-GPL-473 */
...
#ifdef HFC_RHEL7
	/* FCLNX-GPL-FX-683 */
	hfcldd_driver_template_rhel75.can_queue = ap->can_queue;
	hfcldd_driver_template_rhel75.sg_tablesize = ap->sg_tblsize;
#endif
...
//		}
	...
	if(hfc_manage_info.hfcldd_mp_mod) {											/* FCLNX-GPL-204 */
		...
		if (hfc_rport_lu_scan == 1) {
			...
			hfc_scan_start(host);
			while (!hfc_scan_finished(host, jiffies - start))
				msleep(10);
		} else {
			scsi_scan_host(host);
		}
...
void hfc_dev_loss_tmo_callbk(struct fc_rport *rport)
{
	...
	if (target) {
		clear_bit(HFC_WAITING_DEV_LOSS_TMO, (ulong *)&target->flags);
		clear_bit(HFC_DEVLOSS_PENDING, (ulong *)&target->flags); /* FCLNX-GPL-FX-523 */
...
void hfc_rport_add(struct adap_info *ap, struct target_info *target)
{
	...
	/* FCLNX-GPL-FX-523 start */
	if (target->rport) {
		if (test_bit(HFC_DEVLOSS_PENDING, (ulong *)&target->flags)) {
			...
			/* This rport is ready. */
			HFC_DBGPRT("hfc_rport_add() - fc_remote_port is ready()\n");
			return;
		}
	}
	/* FCLNX-GPL-FX-523 end */
...
void hfc_rport_delete(struct target_info *target)
{
	...
	if ((target->rport != NULL) && 	!test_bit(HFC_WAIT_CLOSE, (ulong *)&target->ap->status)) {
		...
		set_bit(HFC_DEVLOSS_PENDING, (ulong *)&target->flags); /* FCLNX-GPL-FX-523 */
	}
...
#else
			/* FCLNX-GPL-FX-695 */
			ret = pci_enable_msix_range(pdev, ap->entries, HFC_MSIX_NVEC, HFC_MSIX_NVEC);
			if(ret < 0)
				err = ret;
			else
				err = 0;
...
{
#if defined(BLK_MQ_ONLY)
	hfc_modify_request_timer(cmnd); /* FCLNX-GPL-FX-704 */
...
/* FCLNX-GPL-FX-672 */
pci_ers_result_t hfc_error_detected(struct pci_dev *pdev, pci_channel_state_t state)
{
	pci_ers_result_t ret = PCI_ERS_RESULT_DISCONNECT;
	if( pdev->device == HFC_PCI_DEVICE_ID_3070 ){
		ret = hfc_fx_error_detected( pdev, state );
	}
	return ret;
}
