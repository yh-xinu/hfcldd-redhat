...
int hfc_read_flash(struct adap_info *ap, int offset, int size, uchar *buf)
{
	...
	if (read_reg == 0xffffffff) {
		HFC_DBGPRT("hfc_read_flash() - pci bus error\n");
		return(0);
	}
...
/* FCLNX-GPL-FX-683 */
struct Scsi_Host *
hfc_scsi_host_alloc_new(struct scsi_host_template_new *sht, int privsize)
{
	struct Scsi_Host *addr = NULL;
...
/* req and rq must be Non NULL */
static inline ulong hfc_get_req_timeout(const struct request *req, const struct request_queue *rq) {
	if (likely(req->timeout != 0))
		return req->timeout;

	/* req->timeout is 0 */

	if (likely(rq->rq_timeout != 0))
		return rq->rq_timeout;
...
void hfc_modify_request_timer(struct scsi_cmnd *cmnd) {
	static const ulong EXPANSION = 3600*HZ;
	...
	if (unlikely(!cmnd->request))
		return;

	req = cmnd->request; /* req is Non NULL */

	if (unlikely(!req->q))
		return;

	rq = req->q; /* rq is Non NULL */

	timeout = hfc_get_req_timeout(req, rq); /* timeout is Non 0 */

	/* mod_timer_pending() will not re-activate and modify already deleted timers */
	mod_timer_pending(&rq->timeout, (jiffies + timeout + EXPANSION));
