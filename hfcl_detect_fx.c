...
#if defined(HFC_RHEL7) || defined(HFC_X8664_SLES12)|| defined(HFC_X8664_OEL7)|| defined(HFC_RHEL8)
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
extern uint	hfc_rhel75;		/* FCLNX-GPL-FX-683 */
#endif

extern void print_module_parameter(void);
int hfc_fx_read_hfcbios(struct port_info *pp);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 */
static void hfc_fx_set_irq_hints(struct port_info *pp, struct request_queue *rq); /* FCLNX-GPL-FX-547 */
#endif /* FCLNX-GPL-FX-609 */
#if 0
/* FCLNX-GPL-204 STR */
...
int hfc_fx_allocate_memory(struct port_info *pp, int rid)
{
	...
	int 	i;
	int next_pkt_num	= 0; /* FCLNX-GPL-FX-547 */
	uchar	logdata[16];
...
//			(ulong)pp->target_scan);
	...
	if (HFC_FX_PHYSICAL_PORT(pp)) {
		int total_pkt_num = pp->can_queue + MAX_DEV_CNT; /* FCLNX-GPL-FX-731 */
		for (i=0;i<total_pkt_num;i++) {   /* FCLNX-GPL-FX-547, 731 */
			...
			next_pkt_num = hfc_set_new_hfcp(pp, hfcp); /* FCLNX-GPL-FX-547 */ 
			if (next_pkt_num >= total_pkt_num) /* FCLNX-GPL-FX-547, 731 */
				break;
		}
	}
...
//						(ulong)sgl->page_link);
	...
	if (!HFC_FX_MMODE_CHECK_MLPF(pp))
		return (0); /* FCLNX-GPL-FX-493 */
	
	/* FCLNX-GPL-FX-488 >>> */
	/* allocate scsi_cmnd for mig */
	if(hfc_fx_mlpf_check_state_port(pp, HFC_HG_LPAR_LIVEMIG_SUPPORT_FX, HFC_CHECK_HVM_SUPPORT )){
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_cmnd for mig\n"); 
		pp->mig_cmnd = (struct scsi_cmnd *)hfc_fx_kmalloc(pp, (sizeof(struct scsi_cmnd)), GFP_KERNEL);
		if (pp->mig_cmnd == NULL){
			logdata[0] = 0x09;
			goto attachmem_error_exit;
		}
		memset( pp->mig_cmnd, 0, sizeof(struct scsi_cmnd) );
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_cmnd for mig logical=%lx\n",
//			(ulong)pp->mig_cmnd);
	
		/* allocate scsi_device for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_device for mig\n"); 
		if(pp->mig_cmnd != NULL) {
			if(pp->mig_cmnd->device == NULL){
				pp->mig_cmnd->device = (struct scsi_device *)hfc_fx_kmalloc(pp, sizeof(struct scsi_device), GFP_KERNEL);
				if (pp->mig_cmnd->device == NULL){
					logdata[0] = 0x0a;
					goto attachmem_error_exit;
				}
				memset( pp->mig_cmnd->device, 0, sizeof(struct scsi_device) );
//				HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_device for mig logical=%lx\n",
//					(ulong)pp->mig_cmnd->device);
			}
		}

		/* allocate scsi_cmnd->cmnd area for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_cmnd->cmnd[] area for mig\n"); 
		if(pp->mig_cmnd != NULL) {
			if(pp->mig_cmnd->cmnd == NULL){
				pp->mig_cmnd->cmnd = (uchar *)hfc_fx_kmalloc(pp, 16, GFP_KERNEL);
				if (pp->mig_cmnd->cmnd == NULL){
					logdata[0] = 0x0b;
					goto attachmem_error_exit;
				}
				memset( pp->mig_cmnd->cmnd, 0, 16 );
//				HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_cmnd->cmnd area for mig logical=%lx\n",
//					(ulong)pp->mig_cmnd->cmnd);
			}
		}

		/* allocate request_queue for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_device for mig\n"); 
		if(pp->mig_cmnd != NULL) {
			if(pp->mig_cmnd->device != NULL){
				if(pp->mig_cmnd->device->request_queue == NULL){
					pp->mig_cmnd->device->request_queue = (struct request_queue *)hfc_fx_kmalloc(pp, sizeof(struct request_queue), GFP_KERNEL);
					if (pp->mig_cmnd->device->request_queue == NULL){
						logdata[0] = 0x0c;
						goto attachmem_error_exit;
					}
					memset( pp->mig_cmnd->device->request_queue, 0, sizeof(struct request_queue) );
//					HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate request_queue for mig logical=%lx\n",
//						(ulong)pp->mig_cmnd->device->request_queue);
				}
			}
		}

		/* allocate dev_info_fx for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scsi_device for mig\n");
		if (pp->mig_dev == NULL) {
			pp->mig_dev = (struct dev_info_fx *)hfc_fx_kmalloc(pp, sizeof(struct dev_info_fx), GFP_KERNEL);
			if (pp->mig_dev == NULL){
				logdata[0] = 0x0d;
				goto attachmem_error_exit;
			}
			memset( pp->mig_dev, 0, sizeof(struct dev_info_fx) );
//			HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate dev_info_fx for mig logical=%lx\n",
//				(ulong)pp->mig_dev);
		}
	
		/* allocate scatterlist for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scatterlist for mig\n"); 
		if(pp->mig_cmnd != NULL) {
			if(pp->mig_cmnd->sdb.table.sgl == NULL){
				pp->mig_cmnd->sdb.table.sgl = (struct scatterlist *)hfc_fx_kmalloc(pp, sizeof(struct scatterlist)*HFC_SCATTERLIST_NUM, GFP_KERNEL);
				if (pp->mig_cmnd->sdb.table.sgl == NULL){
					logdata[0] = 0x0e;
					goto attachmem_error_exit;
				}
				memset( pp->mig_cmnd->sdb.table.sgl, 0, sizeof(struct scatterlist)*HFC_SCATTERLIST_NUM );
//				HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate scatterlist for mig logical=%lx\n",
//					(ulong)pp->mig_cmnd->sdb.table.sgl);
			}
		}
		
		/* allocate data buffer (DMA area) for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate data buffer for mig\n"); 
		if(!HFC_FX_MMODE_CHECK_SHADOW(pp)){
			if(pp->mig_cmnd != NULL) {
				sgl = pp->mig_cmnd->sdb.table.sgl;
				for(i=0;i<HFC_SCATTERLIST_NUM;i++){
					if( sgl != NULL ){
						if( sgl->page_link == (unsigned long)0 ){
							sgl->page_link = (unsigned long)hfc_fx_dma_alloc_coherent(pp, &pp->pci_cfginf->dev, HFC_PAGE_SIZE, &sgl->dma_address,GFP_ATOMIC);
							...
							memset( (char *)sgl->page_link, 0, HFC_PAGE_SIZE );
//							HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate data buffer (DMA area) for mig logical=%lx\n",
//								(ulong)sgl->page_link);
						}
						sgl->length = HFC_PAGE_SIZE;
						sgl++;
		...
		/* allocate sense data) for mig */
//		HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate sense data buffer for mig\n"); 
		if (pp->mig_sense == NULL) {
			pp->mig_sense = (uchar *)hfc_fx_kmalloc(pp, 64, GFP_KERNEL);
			if (pp->mig_sense == NULL){
				logdata[0] = 0x0d;
				goto attachmem_error_exit;
			}
			memset( pp->mig_sense, 0, 64 );
//			HFC_DBGPRT(" hfcldd : hfc_fx_allocate_memory - allocate dev_info_fx for mig logical=%lx\n",
//				(ulong)pp->mig_sense);
		}
	}
	...
	/* FCLNX-GPL-FX-582,595 >>>*/
	if(hfc_fx_mlpf_check_state_port(pp, HFC_HG_LPAR_EX_LOG_SUPPORT, HFC_CHECK_HVM_SUPPORT )){ 
		pp->mlpf_xlog = (uchar *)hfc_fx_kmalloc(pp, HFC_ERRLOG_LEN, GFP_KERNEL);
		if (pp->mlpf_xlog == NULL){
			logdata[0] = 0x10;
			goto attachmem_error_exit;
		}
		memset( pp->mlpf_xlog, 0, HFC_ERRLOG_LEN );
	}
	/* >>> FCLNX-GPL-FX-582 */
...
int hfc_fx_free_memory(struct port_info *pp, int rid)
{
	...
	/* FCLNX-GPL-FX-488 >>> */
	/* Release data buffer (DMA area) for mig */
	if(!HFC_FX_MMODE_CHECK_SHADOW(pp)){
		if(pp->mig_cmnd != NULL) {
			sgl = pp->mig_cmnd->sdb.table.sgl;
			for(i=0;i<HFC_SCATTERLIST_NUM;i++){
				if( sgl != NULL ){
					if( sgl->page_link != (unsigned long)0 ){
						...
						hfc_fx_dma_free_coherent(pp, &pp->pci_cfginf->dev, (size_t)HFC_PAGE_SIZE, (void *)sgl->page_link, (dma_addr_t)sgl->dma_address);
						sgl->page_link = (unsigned long)0;
						HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - Release data buffer (DMA area) for mig logical=%lx\n",
							(ulong)sgl->page_link);
					}
					sgl++;
				}
	...
	/* >>> FCLNX-GPL-FX-488 */
	/* release scatterlist for mig */
	if(pp->mig_cmnd != NULL) {
		if(pp->mig_cmnd->sdb.table.sgl != NULL){
			memset( pp->mig_cmnd->sdb.table.sgl, 0, sizeof(struct scatterlist)*HFC_SCATTERLIST_NUM );
			hfc_fx_kfree(pp, pp->mig_cmnd->sdb.table.sgl);
			pp->mig_cmnd->sdb.table.sgl = NULL;
			HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release scatterlist for mig logical=%lx\n",
				(ulong)pp->mig_cmnd->sdb.table.sgl);
		}
		
	}
	
	/* release request_queue for mig */
	if(pp->mig_cmnd != NULL) {
		if(pp->mig_cmnd->device != NULL){
			if(pp->mig_cmnd->device->request_queue != NULL){
				memset( pp->mig_cmnd->device->request_queue, 0, sizeof(struct request_queue) );
				hfc_fx_kfree(pp, pp->mig_cmnd->device->request_queue);
				pp->mig_cmnd->device->request_queue = NULL;
				HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release request_queue for mig logical=%lx\n",
					(ulong)pp->mig_cmnd->device->request_queue);
			}
		}
	}

	/* release dev_info_fx for mig */
	if(pp->mig_dev != NULL) {
		memset( pp->mig_dev, 0, sizeof(struct dev_info_fx) );
		hfc_fx_kfree(pp, pp->mig_dev);
		pp->mig_dev = NULL;
		HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release dev_info_fx for mig logical=%lx\n",
			(ulong)pp->mig_dev);
	}
	
	/* release scsi_cmnd->cmnd area for mig */
	if(pp->mig_cmnd != NULL) {
		if(pp->mig_cmnd->cmnd != NULL){
			memset( pp->mig_cmnd->cmnd, 0, 16 );
			hfc_fx_kfree(pp, pp->mig_cmnd->cmnd);
			pp->mig_cmnd->cmnd = NULL;
			HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release scsi_cmnd->cmnd area for mig logical=%lx\n",
				(ulong)pp->mig_cmnd->cmnd);
		}
	}

	/* release scsi_device for mig */
	if(pp->mig_cmnd != NULL) {
		if(pp->mig_cmnd->device != NULL){
			memset( pp->mig_cmnd->device, 0, sizeof(struct scsi_device) );
			hfc_fx_kfree(pp, pp->mig_cmnd->device);
			pp->mig_cmnd->device = NULL;
			HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release scsi_device for mig logical=%lx\n",
				(ulong)pp->mig_cmnd->device);
		}
	}

	/* release scsi_cmnd for mig */
	if(pp->mig_cmnd != NULL){
		memset( pp->mig_cmnd, 0, sizeof(struct scsi_cmnd) );
		hfc_fx_kfree(pp, pp->mig_cmnd);
		pp->mig_cmnd = NULL;
		HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release scsi_cmnd for mig logical=%lx\n",
			(ulong)pp->mig_cmnd);
	}

	/* release sense data) for mig */
	if(pp->mig_sense != NULL){
		memset( pp->mig_sense, 0, 64 );
		hfc_fx_kfree(pp, pp->mig_sense);
		pp->mig_sense = NULL;
		HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release sense data for mig logical=%lx\n",
			(ulong)pp->mig_cmnd);
	}
	...
	if (HFC_FX_PHYSICAL_PORT(pp)) {
		...
		pp->pkt_top = NULL;  /* FCLNX-GPL-FX-547 */
		pp->pkt_end = NULL; /* FCLNX-GPL-FX-547 */
		atomic64_set(&pp->pkt_next, (uint64_t)NULL); /* FCLNX-GPL-FX-547 */
		HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - HFC_FX_PHYSICAL_PORT(pp)\n");
	}
	...
	/* FCLNX-GPL-FX-598 start ------- */	
	/* FCLNX-GPL-FX-582,595 >>>*/
	if (pp->mlpf_xlog != NULL) {
		memset(pp->mlpf_xlog, 0, HFC_ERRLOG_LEN );
		hfc_fx_kfree(pp, pp->mlpf_xlog);
		pp->mlpf_xlog = NULL;
		HFC_DBGPRT(" hfcldd : hfc_fx_free_memory - release trace area logical=%lx\n",
			(ulong)pp->mlpf_xlog);
	}
...
void hfc_fx_set_fw_init_tbl(struct port_info *pp)
{
	...
	uint	fw_support=0;	/* FCLNX-GPL-FX-488 */
...
//			break;
		...
		/* FCLNX-GPL-FX-488 >>> */
		if(HFC_FX_MMODE_CHECK_SHADOW(pp) ){
			fw_support = hfc_fx_read_reg_ext(pp,( uint )0x328+0x80*core->core_no,( char )0x04);
			HFC_DBGPRT("hfcldd%d hfc_fx_set_fw_init_tbl core#%d fw_support = %08x\n",
						pp->dev_minor, core->core_no, fw_support);
			if(!(fw_support & HFC_FX_FWSP_MIGRATION)){
				pp->hg_migration_enable = HFC_FX_DISABLE_MIGRATION;
			}else{
				if(hfc_fx_mlpf_check_state_port(pp, HFC_HG_LPAR_LIVEMIG_SUPPORT_FX, HFC_CHECK_HVM_SUPPORT ))
...
int hfc_fx_release_adp(struct port_info *pp)
{
	...
	HFC_ALLLOCK_MQ_IRQSAVE(pp, flags); /* FCLNX-GPL-FX-569 */
	...
	if(!test_bit(HFC_PS_ISOL,(ulong *)&pp->pport->status)){
		...
		if (HFC_FX_PHYSICAL_PORT(pp)) {	/* FCLNX-GPL-FX-594 */
			/* Require OFFLINE_MB */
			...
			do {
				...
				HFC_ALLUNLOCK_MQ_IRQRESTORE(pp, flags); /* FCLNX-GPL-FX-569 */
				msleep(1); /* sleep for a while */
				HFC_ALLLOCK_MQ_IRQSAVE(pp, flags); /* FCLNX-GPL-FX-569 */
			} while (wait);
		}
		else if ((!HFC_FX_MQ_VIRTUAL_PORT(pp)) &&
				(test_bit(HFC_PS_CONNECTED, (ulong *)&pp->status)) &&
				(!test_bit(HFC_PS_MCK_RECOVERY, (ulong *)&pp->pport->status))) {	/* FCLNX-GPL-FX-248,272,594 */
			/* Require CANCEL_SCSI */
	...
	HFC_PORTUNLOCK_IRQRESTORE(pp,flags);	/* FCLNX-GPL-FX-569 */
...
#if !(defined(HFC_RHEL7)|| defined(HFC_X8664_SLES12)|| defined(HFC_X8664_OEL7)|| defined(HFC_RHEL8) )	/* FCLNX-GPL-FX-435 */
...
//	}
	...
	HFC_PORTLOCK_IRQSAVE(pp,flags);
	set_bit(HFC_PD_NEED_LINK_INI, (ulong *)&pp->status_detail1);	/* FCLNX-GPL-FX-575 */
...
//				pp->next_gidpn = TRUE;
	...
	&& (pp->mig_login_delay != 0)) {
		hfc_fx_watchdog_enter( pp, core, NULL, NULL, 0, HFC_FX_LOGIN_DELAY_TMR, pp->mig_login_delay, 0 );
	}else{
...
void hfc_fx_wwnverify_gpnft(struct port_info *pp, struct target_info_fx *target, struct core_info *core, uint mb_resp_status)
{
	...
	int		no_target=1, find_target=FALSE, mig_find_target=FALSE;
	int 	detect_own_scsi_id;
	ushort	portnum_by_rcvlen=0;
	uchar	ctl=0;
	unsigned long long wwpn=0, def_wwpn=0;	/* FCLNX-GPL-FX-501 */
	struct payload_fx	*pyload = NULL;
	struct mailbox_fx	*mbox = NULL ;
	...
	if (mb_resp_status) {
		...
		if(target_not_found){																		/* FCLNX-GPL-fX-139 Start */
			...
			memcpy(&core->logdata[2],(char *)&pyload->send_payload.data0[0],2);
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			if(pp->mig_tgtlist->target_cnt == 0x00){
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_NOMAL, 0x0000000a);
			}else{
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_DEGTGT, 0x0000000C);
			}
		}
	...
	for (j=0;j<(int)(pp->max_target);j++) {
		...
		if(target != NULL){
			...
			for(i=0 ; i < HFC_FX_MAX_PORTID ; i++){
				wwpn = hfc_fx_read_val( pyload->receive_payload.type.gxx.sub_type.gpn_ft.portid[i].port_name ) ;
...
//	structdump( 0xee, (uchar *)core->payload, sizeof(struct payload_fx) );
	...
	if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
		if(pp->mig_tgtlist != NULL){
			for (j=0;j<(int)(pp->max_target);j++) {
				def_wwpn = hfc_fx_read_val( pp->mig_tgtlist->t_info[j].srcwwpn);
				if((def_wwpn != 0)&&(def_wwpn != 0xffffffff)){
					mig_find_target = FALSE;
					for(i=0 ; i < HFC_FX_MAX_PORTID ; i++){
						wwpn = hfc_fx_read_val( pyload->receive_payload.type.gxx.sub_type.gpn_ft.portid[i].port_name ) ;
						if(wwpn == def_wwpn){
							mig_find_target = TRUE;
							break;
						}
					}
					if(mig_find_target == FALSE){
						HFC_DBGPRT( "fx_wwnverify_gpnft() - Not find mig target = %llx\n", def_wwpn);
						hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_DEGTGT, 0x0000000E);
						return;
					}
				}
			}
	...
	if (!port_num) {
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_NOMAL, 0x00000005);
		}
		/* FCLNX-GPL-FX-501 End */
	...
	for (i=0;i<port_num;i++) {
		...
		if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			if(pp->mig_tgtlist != NULL){
				...
				for (j=0;j<(int)(pp->max_target);j++) {
					def_wwpn = hfc_fx_read_val( pp->mig_tgtlist->t_info[j].srcwwpn);
					HFC_DBGPRT( "fx_wwnverify_gpnftwwpn is not found. target = %llx, wwpn =%llx\n", def_wwpn, wwpn);
					if(wwpn == def_wwpn){
						mig_find_target = TRUE;
						if( ( pp->mig_req_flag == HFCFX_MIG_TL_REQ_DSCAN_DST )
						||  ( pp->mig_req_flag == HFCFX_MIG_TL_REQ_LOGIN_DST )){
							HFC_DBGPRT( "fx_wwnverify_gpnftwwpn is found. target = %llx, wwpn =%llx\n", def_wwpn, wwpn);
							HFC_8L_TO_8B(pp->mig_tgtlist->t_info[j].dstwwpn, wwpn);
						}
						break;
					}
				}
				if(mig_find_target == FALSE){
					HFC_DBGPRT("wwnverify_gpnft wwpn is not found. port_id[%d] = %08x, wwpn = %llx\n", i, scsi_id, wwpn);
					continue;
				}
			}
		}
		...
		if(pp->scsi_id != scsi_id) {
			...
			if (new == TRUE) {
				for (j=0;j<(int)(pp->max_target);j++) {
					target = pp->target_arg[j];
					if (target != NULL) {
						if ((target->scsi_id == scsi_id) && (!test_bit(HFC_TF_DEVFLG_VALID, (ulong *)&target->flags))) {
							new=FALSE;
							if (!test_bit(HFC_TS_NEED_LOGO_TGT, (ulong *)&target->status)) {
								HFC_DBGPRT("set HFC_NEED_PLOGI_T -2\n");
								set_bit(HFC_TS_NEED_PLOGI, (ulong *)&target->status);
								atomic_set(&pp->check_mbreq, 1);
								no_target = 0;
							}
...
//					hfc_fx_enque_plogi_req(pp,target);
	...
	if(no_target){
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			if(pp->mig_tgtlist->target_cnt == 0x00){
				HFC_DBGPRT("hfcldd%d : hfc_fx_wwnverify_plogi 9 \n", pp->dev_minor);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_NOMAL, 0x00000009);
			}else{
				HFC_DBGPRT("hfcldd%d : hfc_fx_wwnverify_plogi 9 \n", pp->dev_minor);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_DEGTGT, 0x0000000D);
			}
		}
		/* FCLNX-GPL-FX-501 End */
	}
...
//		hfc_fx_clear_target_info_fx( pp, target, TRUE );			/* FCLNX-GPL-FX-446 */
	...
	if (mb_resp_status != 0) {
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_DEGTGT, 0x0000000A);
		}
			/* FCLNX-GPL-FX-501 End */
	}
							...
							uint mb_resp_status, uint64_t ww_name)
{
	...
	case 0 :
		...
		if ( !test_bit(HFC_TF_DEVFLG_VALID, (ulong *)&target->flags) ){
			...
			if (empty_hit) {
				target->device_flags   = (ushort)hfc_fx_read_val( pyload->receive_payload.type.prli.prli_param_resp) ;	/* FCLNX-GPL-FX-488 */
				set_bit(HFC_TF_DEVFLG_VALID, (ulong *)&target->flags);
				/* FCLNX-GPL-FX-488 >>> */
				if ((HFC_FX_MMODE_CHECK_SHARED(pp))&&(!HFC_FX_MMODE_CHECK_SHADOW(pp))) {	
					if(pp->hg_migration_enable == HFC_FX_ENABLE_MIGRATION){
...
#ifdef SYSFS_SUPPORT
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,16)
		if ( !HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
...
//		hfc_fx_clear_target_info_fx( pp, target, TRUE );			/* FCLNX-GPL-FX-446 */
	...
	if (!hit){
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			if( (pp->mig_req_flag == HFCFX_MIG_TL_REQ_DSCAN_SRC )
			||	(pp->mig_req_flag == HFCFX_MIG_TL_REQ_DSCAN_DST )){
				HFC_DBGPRT("hfcldd%d :hfc_fx_wwnverify_prli8 \n", pp->dev_minor);
				for (j=0;j<MAX_TARGET_PROBE;j++) {
					target = hfc_fx_hash_target_info(pp, j);	/* Not allocated */
					if(target != NULL){
						HFC_DBGPRT("hfcldd%d :hfc_fx_wwnverify_prli8 target_id=%d\n", pp->dev_minor,target->target_id );
						set_bit(HFC_TS_NEED_RLUNS_MIG,	(ulong *)&target->status);
						target->mig_scsi_retry = 0;	/* FCLNX-GPL-FX-495 */
					}
				}
...
//				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_NOMAL, 0x00000003);
			}else{
				HFC_DBGPRT("hfcldd%d : hfc_fx_wwnverify_prli9 \n", pp->dev_minor);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_NOMAL, 0x00000004);
			}
		}
		/* FCLNX-GPL-FX-488 >>> */
		if(pp->initialize != 0){
...
void hfc_fx_change_portstat_linkup(struct port_info *pp, struct core_info *core)
{
	...
	/* FCLNX-GPL-FX-488 >>> */
	if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
		HFC_DBGPRT("hfcldd%d: hfc_fx_change_portstat_linkup - skip\n", pp->dev_minor);
		return;
	}
	/* >>> FCLNX-GPL-FX-488 */
	...
	||  (pp->mck_linkup == HFC_LINKUP_MCK)){
		/* FCLNX-GPL-FX-603 */
		if(pp->log_after_receive_frame != HFC_FX_RECEIVE_FLOGI_LOG)
...
struct target_info_fx *hfc_fx_add_target_info_fx( struct port_info *pp, uint64_t scsi_id )
{
	...
	struct scsi_cmnd		*dummy_cmnd[HFC_FX_MAX_MQ_NUM][MAX_CORE_PROBE_FX];
	struct hfc_pkt_fx		*hfcp=NULL, *hfcp_wk=NULL;
	uint pseq;
	int  i, j;
	...
	for(i=0;i<MAX_CORE_PROBE_FX;i+=MAX_CORE_PROBE_FX/pp->core_num){
		dummy_cmnd[0][i] = hfc_fx_get_new_cmnd(pp);	/* FCLNX-GPL-FX-560 */
	}
	/* FCLNX-GPL-FX-560 >>> */
	if (HFC_FX_MQ_VALID(pp)) {
		for (i=1; i<=pp->max_vport_count; i++) {
			...
			for(j=0;j<MAX_CORE_PROBE_FX;j+=MAX_CORE_PROBE_FX/pp->core_num){
				dummy_cmnd[i][j] = hfc_fx_get_new_cmnd(vpp);  
			}
		}
	}
	/* FCLNX-GPL-FX-560 <<< */
	...
	if (HFC_FX_MQ_VALID(pp)) {
		for (i=1; i<=pp->max_vport_count; i++) {
			...
			if (vpp == NULL)
				continue;
			/* FCLNX-GPL-FX-556 start */
			hfcp_wk = (struct hfc_pkt_fx *)hfc_fx_kmalloc(vpp, sizeof(struct hfc_pkt_fx)*MAX_CORE_PROBE_FX*3, 0);
			if (hfcp_wk == NULL){
				uchar logdata[16];
			...
			/* FCLNX-GPL-FX-556 start */
			if (hfcp_wk != NULL) {
				memset( hfcp_wk, 0, sizeof(struct hfc_pkt_fx)*MAX_CORE_PROBE_FX*3 );
				target_wk->reset_pkt = hfcp_wk;
			}
			/* FCLNX-GPL-FX-556 end */			
	...
	for(i=0;i<MAX_CORE_PROBE_FX;i+=MAX_CORE_PROBE_FX/pp->core_num){
		target->dummy_cmnd[i] = dummy_cmnd[0][i];	/* FCLNX-GPL-FX-560 */
	}
	/* FCLNX-GPL-FX-560 >>> */
	if (HFC_FX_MQ_VALID(pp)) {
		for (i=1; i<=pp->max_vport_count; i++) {
			...
			if (vpp->target_arg[pseq] != NULL) {
				for(j=0;j<MAX_CORE_PROBE_FX;j+=MAX_CORE_PROBE_FX/pp->core_num){
					vpp->target_arg[pseq]->dummy_cmnd[j] = dummy_cmnd[i][j];
				}
			}
		}
	}
...
//	set_bit(HFC_WAIT_TARGET_STOP, (ulong *)&target->status);
	...
	if ((HFC_FX_MMODE_CHECK_SHARED(pp))&&(!HFC_FX_MMODE_CHECK_SHADOW(pp))) {	
		if(pp->hg_migration_enable == HFC_FX_ENABLE_MIGRATION){
			if((test_bit(HFC_TF_WWN_VALID, (ulong *)&target->flags))
			&& (test_bit(HFC_TF_DEVFLG_VALID, (ulong *)&target->flags))) /* FCLNX-GPL-FX-493 */
				hfc_fx_mlpf_clear_target_listinfo(pp, target->target_id);
		}
	}
	/* >>> FCLNX-GPL-FX-488 */
	clear_bit(HFC_TF_WWN_VALID, (ulong *)&target->flags); /* temp */	/* FCLNX-GPL-FX-493 */
	target->login_seq_retry_cnt = pp->login_seq_retry_cnt;	/* FCLNX-GPL-FX-0647 */

	/* FCLNX-GPL-FX-0611 >>> */
	if ( test_bit(HFC_KTHREAD_RUN, (ulong *)&pp->pport->kthread_status) 
	&& !HFC_FX_MMODE_CHECK_SHADOW(pp)){	/* FCLNX-GPL-FX-501 */
	...
	if ( !test_bit(HFC_TF_DEVFLG_VALID, (ulong *)&target->flags) ){
		/* FCLNX-GPL-FX-0647 >>> */
		HFC_DBGPRT("hfcldd%d hfc_fx_clear_target_info_fx HFC_TF_DEVFLG_VALID OFF,  pseq = %02x\n",
						pp->dev_minor, target->pseq);
		if ((pp->connect_type == HFC_FX_SWITCH ) /* FC-SW ? */
		|| ((pp->connect_type == HFC_FX_AL) && (pp->scsi_id & 0x00ffff00))) {
			HFC_DBGPRT("hfcldd%d hfc_fx_clear_target_info_fx FC-SW,  pseq = %02x\n",
							pp->dev_minor, target->pseq);
			if (hfc_fx_check_seq_num(pp, target->pseq)) {
				HFC_DBGPRT("hfcldd%d hfc_fx_clear_target_info_fx release_seq_num,  pseq = %02x\n",
							pp->dev_minor, target->pseq);
...
//			SPRINTF ("MSI-X Multiqueue Mode");
	...
	if( HFC_FX_MMODE_CHECK_SHADOW(pp) ){
		if(pp->hg_migration_enable == HFC_FX_ENABLE_MIGRATION)
			SPRINTF ("    HBA Live Migration Support    = On\n");
	...
	if ( HFC_FX_MMODE_CHECK_SHARED(pp) ){
		...
		SPRINTF ("    Entended Log SUPPORT        Status:%d\n",pp->hvm_extended_log);/* FCLNX-GPL-FX-582,595 */
	}
	/* FCLNX-GPL-393 */
	...
	if ( HFC_FX_MMODE_CHECK_MLPF(pp) ) {
		...
		if(pp->mig_fwalist != NULL){
			SPRINTF ("-- mig fw if addr list --virtual address=%llx --phsyical address=%llx\n",(uint64_t)pp->mig_fwalist,(uint64_t)pp->padr_mig_fwalist);
			SPRINTF ("   version          =  0x%x\n", pp->mig_fwalist->rev);
			SPRINTF ("   size             =  0x%x\n", pp->mig_fwalist->tbl_size);
			for(i=0;i<MAX_CORE_PROBE_FX;i+=MAX_CORE_PROBE_FX/pp->core_num){
				SPRINTF ("   nextXOBA         =  0x%llx\n", pp->mig_fwalist->next_addr[i].xob);
				SPRINTF ("   nextXRBA         =  0x%llx\n", pp->mig_fwalist->next_addr[i].xrb);
				SPRINTF ("   init_addr        =  0x%llx\n", pp->mig_fwalist->init_addr[i]);
				SPRINTF ("   mbox_addr        =  0x%llx\n", pp->mig_fwalist->mbox_addr[i]);
			}
			for(i=0;i<8;i++){
				SPRINTF ("   xob_addr         =  0x%llx\n", pp->mig_fwalist->xob_addr[i]);
			}
			for(i=0;i<32;i++){
				SPRINTF ("   xrb_addr         =  0x%llx\n", pp->mig_fwalist->xrb_addr[i]);
			}
			for(i=0;i<64;i++){
				SPRINTF ("   slog_addr        =  0x%llx\n", pp->mig_fwalist->slog_addr[i]);
			}
			SPRINTF ("   payload_saddr    =  0x%llx\n", pp->mig_fwalist->payload_saddr);
			SPRINTF ("   payload_gaddr    =  0x%llx\n", pp->mig_fwalist->payload_gaddr);
			for(i=0;i<5;i++){
				SPRINTF ("   seg_saddr        =  0x%llx\n", pp->mig_fwalist->seg_spaddr[i]);
				SPRINTF ("   seg_gaddr        =  0x%llx\n", pp->mig_fwalist->seg_gpaddr[i]);
			}
			SPRINTF ("   login_delay      =  0x%x\n", pp->mig_fwalist->login_delay);

			SPRINTF ("-- mig fw if addr list start -- \n");
			
			size = sizeof(struct mig_fwifaddr_list);
			tmp_cca = NULL;
			tmp_cca = (uchar *)pp->mig_fwalist;
			
			if(tmp_cca != NULL){
				for ( j = 0; j < ( size / 8 ); j+=8 ) {
					SPRINTF ("   %02x%02x%02x%02x%02x%02x%02x%02x\n",
						tmp_cca[j],tmp_cca[j+1],tmp_cca[j+2],tmp_cca[j+3],
						tmp_cca[j+4],tmp_cca[j+5],tmp_cca[j+6],tmp_cca[j+7]);
				}
			}
		
			SPRINTF ("-- mig fw if addr list end -- \n");
		}
		if(pp->mig_tgtlist != NULL){
			SPRINTF ("-- mig target addr list --virtual address=%llx --phsyical address=%llx\n",(uint64_t)pp->mig_tgtlist,(uint64_t)pp->padr_mig_tgtlist);
			SPRINTF ("   version          =  0x%x\n", pp->mig_tgtlist->rev);
			SPRINTF ("   rid              =  0x%x\n", pp->mig_tgtlist->rid);
			SPRINTF ("   sw_state         =  0x%x\n", pp->mig_tgtlist->sw_state);
			SPRINTF ("   size             =  0x%x\n", pp->mig_tgtlist->tbl_size);
			SPRINTF ("   req_flag         =  0x%x\n", pp->mig_tgtlist->req_flag);
			SPRINTF ("   target_cnt       =  0x%x\n", pp->mig_tgtlist->target_cnt);
			SPRINTF ("   update_cnt       =  0x%x\n", pp->mig_tgtlist->update_cnt);
			SPRINTF ("   hvm_used         =  0x%x\n", pp->mig_tgtlist->hvm_used);
			SPRINTF ("   dmywwpn          =  0x%llx\n", pp->mig_tgtlist->dmywwpn);
			SPRINTF ("   port_statis_addr =  0x%llx\n", pp->mig_tgtlist->port_statis_addr);
			SPRINTF ("   gstwwpn          =  0x%llx\n", pp->mig_tgtlist->gstwwpn);
			SPRINTF ("   result           =  0x%x\n", pp->mig_tgtlist->result);
			SPRINTF ("   datail           =  0x%x\n", pp->mig_tgtlist->detail);
			
			SPRINTF ("-- mig_targetinfo Start -- \n");
			for(i=0;i<255;i++){
				size = sizeof(struct mig_targetinfo);
				tmp_cca = NULL;
				tmp_cca = (uchar *)&pp->mig_tgtlist->t_info[i];
			...
			SPRINTF ("-- mig_targetinfo End -- \n");

			SPRINTF ("-- hg_mig_tgtlist start -- \n");
			
			size = sizeof(struct hg_mig_tgtlist);
			tmp_cca = NULL;
			tmp_cca = (uchar *)pp->mig_tgtlist;
...
//	hfc_fx_set_filter_target(pp);		/* Set filter target			pp->filter_target */	/* FCLNX-GPL-491 */
	...
	hfc_fx_set_xlog_func(pp);/* FCLNX-GPL-FX-582,595 */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 start */
#if !defined(BLK_MQ_ONLY)
	/* FCLNX-GPL-FX-547: blk-mq & scsi-mq support start */
	if (shost_use_blk_mq(pp->hosts))
#endif
	{
		/* register the number of LLD queues to scsi-mid layer */
		switch (pp->msi_enable) {
			case HFC_INT_TYPE_MSIX:
				if((HFC_FX_MQ_ENABLE(pp)) && (pp->topology != HFC_FX_AL)){
					pp->hosts->nr_hw_queues = HFC_FX_MSIX_MULTIQUEUE;
				}else{
					pp->hosts->nr_hw_queues = pp->core_num; /* except share vector for non-xrb */
				}
				break;
			...
			case HFC_INT_TYPE_INTX:
			default:
				pp->hosts->nr_hw_queues = HFC_FX_NVEC_ONE;
				break;
		}
		/* FCLNX-GPL-FX-704 start */
#if defined(BLK_MQ_ONLY)
		if (hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable()) {
			pp->hosts->nr_hw_queues = 1;
		}
#endif /* defined(BLK_MQ_ONLY) */
		/* FCLNX-GPL-FX-704 end */
	}
	/* FCLNX-GPL-FX-547: blk-mq & scsi-mq support end */
#endif /* FCLNX-GPL-FX-609 end */
...
int hfc_fx_slave_alloc(struct scsi_device *sdev)
{
	...
	struct region_info	*rp = NULL;
	struct hfc_pkt_fx	*hfcp;
	struct pci_dev		*pdev = NULL;	/* FCLNX-GPL-FX-495 */
	ulong				flags = 0;
	uchar				logdata[16];
	uchar				no_dev = 0;
	int					i, alloc_cnt = 0;
	int					next_pkt_num	= 0; /* FCLNX-GPL-FX-547 */
	uchar				free_mig = 0;	/* FCLNX-GPL-FX-495 */
...
//		sdev->host->host_no, sdev->channel, sdev->id, sdev->lun);
	...
	if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() )
	{
		...
		pdev = pp->pci_cfginf;	/* FCLNX-GPL-FX-495 */
		...
		for (i=0;i<alloc_cnt;i++) {
			...
			HFC_ALLLOCK_MQ_IRQSAVE(pp,flags); /* FCLNX-GPL-FX-569 */
			next_pkt_num = hfc_set_new_hfcp(pp, hfcp); /* FCLNX-GPL-FX-547 */
			if (next_pkt_num >= pp->pport->can_queue) {   /* FCLNX-GPL-FX-547 */
				HFC_ALLUNLOCK_MQ_IRQRESTORE(pp,flags); /* FCLNX-GPL-FX-569 */
				break;
			}
		...
		if( target != NULL){
			if ((struct dev_info_fx *)hfc_fx_get_dev_info_fx(target, sdev->lun) == NULL) {
				...
				/* FCLNX-GPL-FX-495 >>> */
				if (target->mig_luid_info != NULL) {
					HFC_DBGPRT(" hfcldd : hfc_fx_slave_alloc - free mlpf cca \n");
...
void hfc_fx_slave_destroy(struct scsi_device *sdev)
{
	...
	struct dev_info_fx		*temp_dev2 = NULL;	/* FCLNX-GPL-FX-614 */
	struct target_info_fx	*target = NULL;
	struct region_info	*rp = NULL;
	struct pci_dev		*pdev = NULL;	/* FCLNX-GPL-FX-495 */
	ulong				flags = 0;
		...
		if( dev != NULL ){
			if( !test_bit(HFC_DEVINF_VALID, (ulong *)&dev->flags) ){
				...
				if( target != NULL){
					...
					while( temp_dev != NULL ){
						if( temp_dev == dev ){
							HFC_DBGPRT("hfc_fx_slave_destroy - 1\n");
							break;
						}
					...
					if( temp_dev != NULL){
						if( temp_dev == target->dev ){
							...
							/* FCLNX-GPL-FX-614 >>> */
							temp_dev2 = temp_dev->next;
							if( temp_dev2 != NULL){
								HFC_DBGPRT("hfc_fx_slave_destroy - 7\n");
								temp_dev2->prev = NULL;
							}
							/* FCLNX-GPL-FX-614 <<< */

							HFC_DBGPRT("hfc_fx_slave_destroy - 2\n");
							...
							HFC_DBGPRT("hfc_fx_slave_destroy - 3\n");
							if( temp_dev != NULL){
								/* FCLNX-GPL-FX-614 >>> */
								temp_dev->next = dev->next;
								HFC_DBGPRT("hfc_fx_slave_destroy - 4\n");
								temp_dev2 = dev->next;
								if( temp_dev2 != NULL){
									temp_dev2->prev = temp_dev;
									HFC_DBGPRT("hfc_fx_slave_destroy - 5\n");
								}
								/* FCLNX-GPL-FX-614 <<< */
							}
						}
					}
					HFC_DBGPRT("hfc_fx_slave_destroy - 6\n");
					/* FCLNX-GPL-FX-495 >>> */
					if (target->mig_luid_info != NULL) {
						HFC_DBGPRT(" hfcldd : hfc_fx_slave_destroy - free mlpf cca \n");
						memset((char *)target->mig_luid_info, 0, target->mig_lidlist_size);
						target->mig_luid_info = NULL;
						target->mig_lidlist_size = 0;
						free_mig = 1;
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
	struct scsi_host_template_new	*shost_tmp=NULL;	/* FCLNX-GPL-FX-683 */
	int		alloc_policy=0;
	
	/* FCLNX-GPL-FX-683 */
	if( hfc_rhel75 ){
		shost_tmp = (struct scsi_host_template_new *)sdev->host->hostt;
		alloc_policy = shost_tmp->tag_alloc_policy;
	}
#endif
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
		hfc_scsi_activate_tcq(sdev, queue_depth, alloc_policy);	/* FCLNX-GPL-FX-683 */
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 */	
	if (!hfc_manage_info.hfcldd_mp_mod) {	/* FCLNX-GPL-FX-625 >>> */
#if !defined(BLK_MQ_ONLY)
		/* FCLNX-GPL-FX-547: blk-mq & scsi-mq support start */
		if (shost_use_blk_mq(pp->hosts))
#endif
		{
			/* TODO: do this routine ONCE AT EACH FUNCTION */
			hfc_fx_set_irq_hints(pp, rq);
		}
	/* FCLNX-GPL-FX-547: blk-mq & scsi-mq support end */
	}										/* <<< FCLNX-GPL-FX-625 */
#endif /* FCLNX-GPL-FX-609 */
...
#ifdef HFC_RHEL7
	/* FCLNX-GPL-FX-683 */
	struct scsi_host_template_new	*shost_tmp=NULL;
	int		alloc_policy=0;
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
			hfc_scsi_activate_tcq(sdev, sdev->queue_depth, alloc_policy);
#else
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
	/* FCLNX-GPL-565 start *//* FCLNX-GPL-575 start *//* FCLNX-GPL-FX-683 */
	if(!hfc_rhel75){
		if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ){
		...
		if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ){
			host = hfc_fx_scsi_host_alloc_new(hfc_manage_info.hfcldd_driver_template_mp_rhel75, sizeof(struct port_info));
		} 
		else {
			if( hfc_rport_lu_scan != 1){
				host = hfc_scsi_host_alloc_new(hfc_manage_info.hfcldd_driver_template_mp_rhel75, sizeof(struct port_info));
			}
			else if( hfc_rport_lu_scan == 1){
				host = hfc_fx_scsi_host_alloc_new(hfc_manage_info.hfcldd_driver_template_rhel75, sizeof(struct port_info));
			}
		}
	}
...
/* FCLNX-GPL-FX-689 */
		...
		if(!hfc_rhel75){
			...
			vrt_host_alloc = hfc_manage_info.npubp->hfc_fx_mp_scsi_host_alloc_new(hfc_manage_info.hfcldd_driver_template_platform_bus_rhel75,
															host, pp, &hsd_host, &pfb_host);	/* FCLNX-GPL-473 */
		}
#else
		vrt_host_alloc = hfc_manage_info.npubp->hfc_fx_mp_scsi_host_alloc(hfc_manage_info.hfcldd_driver_template_platform_bus, host, pp, &hsd_host, &pfb_host);	/* FCLNX-GPL-473 */
#endif
...
//	if((hfc_manage_info.hfcplus_enable)&&(HFC_FX_MMODE_CHECK_BASIC(pp))){	/* FCLNX-0506 */
	...
	/* FCLNX-GPL-FX-488 >>> */
	if(HFC_FX_MMODE_CHECK_MLPF(pp)){
		if(pp->hg_migration_enable == HFC_FX_ENABLE_MIGRATION){	/* FCLNX-GPL-488 >>> */
			HFC_DBGPRT("hfcldd : hfc_fx_probe_one - call hfc_fx_mlpf_alloc_target_list\n");
			if(hfc_fx_mlpf_alloc_target_list(pp)){
				pp->hg_migration_enable = HFC_FX_DISABLE_MIGRATION;
			}
			HFC_DBGPRT("hfcldd : call hfc_fx_mlpf_alloc_target_list\n");
			hfc_fx_mlpf_setup_target_list(pp);
			
			HFC_DBGPRT("hfcldd : hfc_fx_probe_one - call hfc_fx_mlpf_alloc_fwifaddr_list\n");
			if(hfc_fx_mlpf_alloc_fwifaddr_list(pp)){
				pp->hg_migration_enable = HFC_FX_DISABLE_MIGRATION;
			}
			HFC_DBGPRT("hfcldd : call hfc_fx_mlpf_setup_fwifaddr_list\n");
			hfc_fx_mlpf_setup_fwifaddr_list(pp);
		}	/* >>> FCLNX-GPL-488 */
//		HFC_DBGPRT("set pp->hg_mem_base_addr mmio_hg\n");
//		structdump( 0xec, (uchar *)(ulong)pp->hg_mem_base_addr, 0x400 );
	}																			/* FCLNX-GPL-494 */
	/* FCLNX-GPL-FX-488 >>> */
...
/* FCLNX-GPL-FX-689 */
#ifdef HFC_RHEL7
	/* FCLNX-GPL-FX-683 */
	hfc_manage_info.hfcldd_driver_template_rhel75->can_queue = pp->can_queue;
	hfc_manage_info.hfcldd_driver_template_rhel75->sg_tablesize = pp->sg_tblsize;
#endif
	...
	/* FCLNX-GPL-FX-601 start ------ */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
	/* second kernel only HFC_INT_TYPE_INTX */
	if(is_kdump_kernel()){
		msi_enable = HFC_INT_TYPE_INTX;
	}else{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 start */
		if ( (HFC_FX_MQ_ENABLE(pp)) && (pp->topology != HFC_FX_AL) ) {	/* FCLNX-GPL-FX-246,272 */
			if (msi_enable == HFC_INT_TYPE_MSIX) {		/* FCLNX-GPL-FX-203 */
				...
				if ( hfc_fx_mq_attach(pp) ) { /* FCLNX-GPL-FX-562 */
					msi_enable = HFC_INT_TYPE_MSIX;
				}
#if !defined(BLK_MQ_ONLY)
				if ( !(shost_use_blk_mq(pp->hosts)) ) { /* FCLNX-GPL-FX-562 */
					msi_enable = HFC_INT_TYPE_MSIX;
				}
#endif
			}
	...
	if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ){
		...
		scsi_scan_host(host); /* FCLNX-GPL-FX-523 start */
		hfc_manage_info.npubp->hfc_fx_mp_scsi_host_rescan(); /* FCLNX-GPL-204 */
	} else {
		if (hfc_rport_lu_scan == 1) {
			unsigned long start = jiffies;
			hfc_fx_scan_start(host);
			while (!hfc_fx_scan_finished(host, jiffies - start))
				msleep(10);
		} else {
			scsi_scan_host(host);
		}
...
struct fc_host_statistics *hfc_fx_get_statistics(struct Scsi_Host *host)
{
	...
	{	/* FCLNX-GPL-FX-552 >>> */
		struct fc_host_statistics	*port_stat = &pp->port_statistics;
		struct fw_init_tbl_fx		*fw_init_p = core[pp->master_core_no]->fw_init_p;
		if ((!HFC_FX_MMODE_CHECK_SHARED(pp)) && HFC_FX_PHYSICAL_PORT(pp)) {
			port_stat->tx_frames	=(uint64_t)hfc_fx_read_val(fw_init_p->portstatistics.tx_frames);
			port_stat->tx_words	=(uint64_t)hfc_fx_read_val(fw_init_p->portstatistics.tx_words);
                        port_stat->rx_frames	=(uint64_t)hfc_fx_read_val(fw_init_p->portstatistics.rx_frames);
                        port_stat->rx_words	=(uint64_t)hfc_fx_read_val(fw_init_p->portstatistics.rx_words);
		} else { /* Virtual Port */
			for (i=0; i<MAX_CORE_PROBE_FX; i+=MAX_CORE_PROBE_FX/pp->core_num) {
				port_stat->tx_frames	+=(uint64_t)pp->tx_frames[i];
				port_stat->tx_words	+=(uint64_t)pp->tx_words[i];
				port_stat->rx_frames	+=(uint64_t)pp->rx_frames[i];
				port_stat->rx_words	+=(uint64_t)pp->rx_words[i];
			}
		}
	}	/* >>> FCLNX-GPL-FX-552 */
...
void hfc_fx_reset_statistics(struct Scsi_Host *host)
{
	...
	HFC_ALLLOCK_MQ_IRQSAVE(pp,flags); /* FCLNX-GPL-FX-569 */
	/* FCLNX-GPL-FX-552 >>> */
	memset(pp->tx_frames,   0, (MAX_CORE_PROBE_FX * sizeof(uint64_t)));
	memset(pp->tx_words,    0, (MAX_CORE_PROBE_FX * sizeof(uint64_t)));
	memset(pp->rx_frames,   0, (MAX_CORE_PROBE_FX * sizeof(uint64_t)));
	memset(pp->rx_words,    0, (MAX_CORE_PROBE_FX * sizeof(uint64_t)));
	if ((!HFC_FX_MMODE_CHECK_SHARED(pp)) && HFC_FX_PHYSICAL_PORT(pp)) {
		/* Physical Port Only */
		for(i=0;i<MAX_CORE_PROBE_FX;i+=MAX_CORE_PROBE_FX/pp->core_num){
			core[i] = rp->core_arg[i];
...
void hfc_fx_rport_add(struct port_info *pp, struct target_info_fx *target)
{
	...
	/* FCLNX-GPL-FX-523 start */
	if (target->rport) {
		if (test_bit(HFC_TF_DEVLOSS_PENDING, (ulong *)&target->flags)) {
			/* The fc_remote_port_delete was done, */
			/* however, the dev_loss_tmo callback entry has NOT called. */
			/* In this case, the fc_remote_port_add will return same rport pointer. */
			target->rport = NULL;
		} else {
			/* The fc_remote_port_delete was NOT called. */
			/* This rport is ready. */
			HFC_DBGPRT("hfc_fx_rport_add() - fc_remote_port is ready()\n");
			return;
		}
	}
	/* FCLNX-GPL-FX-523 end */
	...
	clear_bit(HFC_TF_DEVLOSS_PENDING, (ulong *)&target->flags); /* FCLNX-GPL-FX-523 */
	target->rport = rport;
...
void hfc_fx_rport_delete(struct target_info_fx *target)
{
	...
	if ((target->rport != NULL) && 	!test_bit(HFC_PD_WAIT_CLOSE, (ulong *)&target->pp->status_detail2)) {
		...
		set_bit(HFC_TF_DEVLOSS_PENDING, (ulong *)&target->flags); /* FCLNX-GPL-FX-523 */
	}
}
...
//	fc_host_max_npiv_vports(host) = pp->max_vport_count;
	fc_host_max_npiv_vports(host) = pp->max_vport_num; /* FCLNX-GPL-FX-558 */
	...
	if( type == HFC_INT_TYPE_MSIX_MULTI) {
		...
		if(pos == 0){ /* This HBA don't support MSI-X.*/
				...
				if(HFC_FX_MMODE_CHECK_MLPF(pp)) {
					...
					type = HFC_INT_TYPE_MSI; /* FCLNX-GPL-671 VMDirect Path I/O Support */
				}
		}else{
			...
			if ((type == HFC_INT_TYPE_INTX) || (type == HFC_INT_TYPE_MSIX)) {
				...
				hfc_fx_write_reg_ext( pp, 0x23c, 0x1, chr_wk );
				/* MSI-X Table configuration (vector number) */
				dat_wk = 0x0DC00060 | (0x00000400 * PCI_FUNC(pp->pci_cfginf->devfn) ); /* FCLNX-GPL-FX-565 */
				hfc_fx_write_indarea(
					pp,
					pp->master_core_no, /* core_no */
					dat_wk,     /* offset */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 start */
/* FCLNX-GPL-FX-547: blk-mq & scsi-mq support start */
static void hfc_fx_set_irq_hints(struct port_info *pp, struct request_queue *rq)
{
	...
	HFC_DBGPRT("hfcldd%d %s start\n", pp->dev_minor, __FUNCTION__);

	if(pp->hosts == NULL)
		return;

	if(rq == NULL) /* FCLNX-GPL-FX-602 */
		return;

/* FCLNX-GPL-FX-597 start ------ */
#if defined(HFC_RHEL7_SCSI_MQ)
	if(pp->hosts->tag_set->tags == NULL) /* FCLNX-GPL-FX-602 */
		return;

	tags = pp->hosts->tag_set->tags;
#else
	if(pp->hosts->tag_set.tags == NULL) /* FCLNX-GPL-FX-602 */
		return;
	...
	tags = pp->hosts->tag_set.tags;
#endif
/* ------ FCLNX-GPL-FX-597 end */
	if(tags == NULL)
		return;

#if defined(HFC_RHEL7_SCSI_MQ)
	for(i=0; i < pp->hosts->tag_set->nr_hw_queues; i++){ /* FCLNX-GPL-FX-602 */
#else
	for(i=0; i < pp->hosts->tag_set.nr_hw_queues; i++){ /* FCLNX-GPL-FX-602 */
#endif
		if(tags[i] == NULL)
			continue;
		if(pp->intr_entries[i].pp == NULL)
			continue;

#if defined(HFC_X8664_SLES12SP3) || defined(HFC_RHEL8) || ((LINUX_VERSION_CODE < KERNEL_VERSION(4, 2, 0)) || (LINUX_VERSION_CODE > KERNEL_VERSION(4, 8, 0))) /* FCLNX-GPL-FX-609 */ /* FCLNX-GPL-FX-611 */
		if(rq->queue_hw_ctx[i] == NULL) /* FCLNX-GPL-FX-602 */
			continue;

                irq_set_affinity_hint(pp->intr_entries[i].vector,rq->queue_hw_ctx[i]->cpumask);
#endif

#if !defined(HFC_X8664_SLES12SP3) && !defined(HFC_RHEL8) && ((LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0)) && (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 8, 0))) /* FCLNX-GPL-FX-609 */ /* FCLNX-GPL-FX-611 */
		irq_set_affinity_hint(pp->intr_entries[i].vector,blk_mq_tags_cpumask(tags[i]));
#endif
	}

	HFC_DBGPRT("hfcldd%d %s end\n", pp->dev_minor, __FUNCTION__);
...
#else
		/* FCLNX-GPL-FX-695 start */
		ret = pci_enable_msix_range(pdev, pp->entries, nvec, nvec);
		if( ret < 0 )
			err = ret;
#endif /* FCLNX-GPL-FX-695 end */
...
#else
		/* FCLNX-GPL-FX-695 start */
		ret = pci_alloc_irq_vectors_affinity(pdev, (int)pp->core_num * 2, (int)pp->core_num * 2,
				PCI_IRQ_MSI, NULL);
		if( ret < 0 )
			err = ret;
#endif /* FCLNX-GPL-FX-496 end */ /* FCLNX-GPL-FX-695 end */
...
#else
		/* FCLNX-GPL-FX-695 start */
		ret = pci_alloc_irq_vectors_affinity(pdev, (int)nvec, (int)nvec, PCI_IRQ_MSI, NULL);
		if(ret < 0)
			err = ret;
#endif /* FCLNX-GPL-FX-496 end */ /* FCLNX-GPL-FX-695 end */
	}
...
//			hfc_fx_logout(pp, (uint)0xf1, HFC_ERRLOG_TYPE_IMLLOG);
		...
		/* Reset UTL Register */
		hfc_fx_reset_start(pp, HFC_UTL_REG_CLEAR); /* FCLNX-GPL-FX-670 */
...
int hfc_fx_do_rport(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		/* FCLNX-GPL-FX-488 >>> */
		if(HFC_FX_MMODE_CHECK_SHADOW(pp) ){
			if ( test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
				HFC_DBGPRT(" hfcldd : hfc_do_rpot - call hfc_fx_mlpf_remap_mig_fwif_addr.\n");
				hfc_fx_mlpf_remap_mig_fwif_addr(pp);
				if ( (pp->mig_req_flag == HFCFX_MIG_TL_REQ_LOGOUT_SRC)
				||   (pp->mig_req_flag == HFCFX_MIG_TL_REQ_LOGOUT_DST)){
					set_bit(HFC_PD_NEED_OFFLINE_MB, (ulong *)&pp->status_detail2);
				}else{
					set_bit(HFC_PD_NEED_CORE_START, (ulong *)&pp->status_detail1 );
				}
...
void hfc_fx_dev_loss_tmo_callbk(struct Scsi_Host *host, struct fc_rport *rport)
{
	...
	if (target) {
		clear_bit(HFC_TF_WAITING_DEV_LOSS_TMO, (ulong *)&target->flags);
		clear_bit(HFC_TF_DEVLOSS_PENDING, (ulong *)&target->flags); /* FCLNX-GPL-FX-523 */
		target->rport = NULL; /* FCLNX-GPL-FX-523 */
	}
	HFC_ALLUNLOCK_MQ_IRQRESTORE(pp,flags); /* FCLNX-GPL-FX-569 */
...
void hfc_fx_assign_core_no(struct port_info *pp, struct dev_info_fx *dev)
{
	...
	if( pp->core_num == 1 ){
		dev->curr_core = pp->master_core_no;
		atomic_set(&pp->curr_core, pp->master_core_no);
		return;
	}
	
	/* Count online_core_num */
	/* 1-port:4-loop, 2-port:2-loop */
	for(i=0; i<MAX_CORE_PROBE_FX; i+=MAX_CORE_PROBE_FX/pp->core_num){
		if(!hfc_fx_check_cs_disable(pp, rp->core_arg[i])){	/* FCLNX-GPL-FX-438 */
			online_core_num++;
		}
	}
	/* 1,2-port adapter Common */
	if(online_core_num == 1){
		/* Available core only 1  */
		dev->curr_core = pp->master_core_no;
		atomic_set(&pp->curr_core, pp->master_core_no); /* FCLNX-GPL-FX-547 :Lockless */
		return;
	}
	/* FCLNX-GPL-FX-547 start -------- */
	if(pp->core_num == 2) {
	/* 2-port adapter Only */
	/* All Core(2 Core) are Alive. */
		core_no = atomic_read(&pp->curr_core) + (MAX_CORE_PROBE_FX/pp->core_num);
		core_no %= MAX_CORE_PROBE_FX;
	}
	else { /* 1-port adapter Only  */
		/* search next online core */
		for (i=0; i< MAX_CORE_PROBE_FX; i++) {
			core_no = atomic_read(&pp->curr_core) + 1 + i; /* (MAX_CORE_PROBE_FX/pp->core_num) == 1 */
			core_no %= MAX_CORE_PROBE_FX;
			if(!hfc_fx_check_cs_disable(pp, rp->core_arg[i])){	/* FCLNX-GPL-FX-438 */
				break;
			}	
	...
	atomic_inc(&pp->curr_core); /* FCLNX-GPL-FX-547 :Lockless */
}
...
int hfc_fx_mq_attach(struct port_info *pp)
{
	...
	for (i=1; i<=pp->max_vport_count; i++)
	{
		...
		vpp->multiple_portid	= pp->multiple_portid;	/* FCLNX-PGL-FX-594 */
		for (j=0; j<8; j++) {
		...
		vpp->max_vport_count = pp->mq_num -1;   /* FCLNX-GPL-FX-644 */
	...
	if (HFC_FX_MQ_VALID(pp) && HFC_FX_PHYSICAL_PORT(pp)) {	/* FCLNX-GPL-FX-270,275 */
		for (i=1; i<=pp->max_vport_count; i++) {
			...
			if (test_bit(HFC_TF_WWN_VALID, (ulong *)&target->flags)) {
				...
				vpp->tid_map[target->target_id] = target->pseq;	/* FCLNX-GPL-FX-574 */
			}
			else {
			...
			/* FCLNX-GPL-FX-679 */
			vpp->target_arg[target->pseq]->scsi_id = target->scsi_id;
		}
	}
...
/* FCLNX-GPL-FX-672 */
pci_ers_result_t
hfc_fx_error_detected(struct pci_dev *pdev, pci_channel_state_t state)
{
	struct Scsi_Host *host = pci_get_drvdata(pdev);
	...
	pp = (struct port_info *)host->hostdata;
	if (pp == NULL) {
		HFC_DBGPRT( "hfcldd : %s - pp is NULL.\n", __function__);
		return ret;
	}
