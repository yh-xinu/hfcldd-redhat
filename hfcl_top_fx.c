		...
		if( core_tmp != NULL ){
			if( core_tmp->core_no != core->core_no ){
				...
				&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
					if(core_tmp->mig_fw_init_p != NULL){
						memcpy( &core_tmp->mig_fw_init_p->fw_iocinfo.connect_type, &core->mig_fw_init_p->fw_iocinfo.connect_type, 64);
						memcpy( core_tmp->mig_fw_init_p->pos_map, core->mig_fw_init_p->pos_map, 160);
						memcpy( core_tmp->mig_fw_init_p->active_alpa, core->mig_fw_init_p->active_alpa, 32);
						memcpy( &core_tmp->mig_fw_init_p->sfp_info.sfp_status, &core->mig_fw_init_p->sfp_info.sfp_status, 0x40); /* FCLNX-GPL-FX-149 */
					}
				}else{
...
/* FCLNX-GPL-FX-647 >>> */
/*
 * Function:    hfc_fx_check_seq_num
 *
 * Purpose:     Check sequence number for target
 *
 * Arguments:   
 *  pp         - port_info 
 *  num        - seq#
 ...
 * Notes:       
 */
uint hfc_fx_check_seq_num( struct port_info *pp, int num )
{
	return (pp->seq_num_tbl[num / 32] & ( 1 << (num % 32)));
}
/* FCLNX-GPL-FX-647 <<< */
...
//	HFC_DBGPRT(" hfcldd%d : hfc_fx_read_flash : pkg.type=%d.\n",pp->instance, pp->pkg.type);
	...
	if (read_reg == 0xffffffff) {
		HFC_DBGPRT("hfc_fx_read_flash() - pci bus error\n");
		return(0);
	}
...
int hfc_fx_write_indarea(struct port_info *pp, int core_no, int offset, uint data, int rammask)
{
	...
	read_reg = hfc_fx_read_reg_ext( pp, (uint)0, (char)0x4) ;
	if (read_reg == 0xffffffff) {
		HFC_DBGPRT("hfc_fx_write_indarea() - pci bus error\n");
		return(0);
	}
			...
			for(lp=0 ; lp<MAX_TARGET_PROBE ; lp++)
			{
				...
				if(( target != NULL )&&(test_bit(HFC_TS_NEED_CANCEL_SCSI_WITHOUT_DMA, (ulong *)&target->status ))){	/* FCLNX-GPL-FX-014 */
					...
					if (HFC_FX_MQ_VALID(pp)) {
						( void )hfc_fx_mq_all_cancel_scsi(pp,target,NULL,NULL,HFC_CANCEL_ITNEXUS,HFC_CANCEL_WITHOUT_DMA);
					}
					else {
		...
		case HFC_NMB_CANCEL_SCSI_T_WAIT_DMA:
			for(lp=0 ; lp<MAX_TARGET_PROBE ; lp++)
			{
				...
				if(( target != NULL )&&(test_bit(HFC_TS_NEED_CANCEL_SCSI_WAIT_DMA, (ulong *)&target->status ))){
					/* FCLNX-GPL-FX-556 start */
					if (HFC_FX_MQ_VALID(pp)) {
						( void )hfc_fx_mq_all_cancel_scsi(pp,target,NULL,NULL,HFC_CANCEL_ITNEXUS,HFC_CANCEL_WAIT_DMA);
					}
					else {
				...
				if(target != NULL){
					...
					while(dev != NULL){
						if(test_bit(HFC_DS_NEED_CANCEL_SCSI_WITHOUT_DMA, (ulong *)&dev->lustat )){
							/* FCLNX-GPL-FX-556 start */
							if (HFC_FX_MQ_VALID(pp)) {
								( void )hfc_fx_mq_all_cancel_scsi(pp,target,dev,NULL,HFC_CANCEL_ITLNEXUS,HFC_CANCEL_WITHOUT_DMA);
							}
							else {
				...
				if(target != NULL){
					...
					while(dev != NULL){
						if(test_bit(HFC_DS_NEED_CANCEL_SCSI_WAIT_DMA, (ulong *)&dev->lustat )){
							/*FCLNX-GPL-FX-556 start*/
							if (HFC_FX_MQ_VALID(pp)) {
								( void )hfc_fx_mq_all_cancel_scsi(pp,target,dev,NULL,HFC_CANCEL_ITLNEXUS,HFC_CANCEL_WAIT_DMA);
							}
							else {	
...
int hfc_fx_mailbox_response( struct port_info *pp, struct core_info *core, uint *fsb_error_code )
{
	...
	if ( hfc_fx_read_val(mbox->mb_resp.flag) != 0x80 ) /* valid bit off? */
		return MAILBOX_FATAL;

    *fsb_error_code = 0;
    fsb = (uchar) hfc_fx_read_val( mbox -> mb_resp.fsb );

	if ( fsb != 0 ) {
		*fsb_error_code = ( fsb << 24 ) +
			( (uint) hfc_fx_read_val( mbox->mb_resp.err_code[0] ) << 16 ) +
			( (uint) hfc_fx_read_val( mbox->mb_resp.err_code[1] ) << 8 ) +
			  (uint) hfc_fx_read_val( mbox->mb_resp.err_code[2] );
		rtn = MAILBOX_ERROR;
		HFC_DBGPRT("hfcldd%d : hfcl_top, hfc_fx_mailbox_response -  MAILBOX_ERROR fsb_errcode=%0x\n", pp->dev_minor, *fsb_error_code);
//		pp -> link_dead_cnt = 0;
	}

	hfc_fx_write_val( mbox->mb_resp.flag, 0 ); /* Clear response */
...
int hfc_fx_mb_passthrough(struct port_info *pp, struct core_info *core, ushort timer_id, ulong restart, int retry, uint caller)
{
	...
	memset( &pyload->receive_payload, 0, (uint)HFC_RECV_PAYLOADL_MAX );	/* FCLNX-GPL-FX-488*/
...
int hfc_fx_mb_passthrough_rsp(struct port_info *pp, struct core_info *core, uint caller)
{
	...
	if ( ( rtn = hfc_fx_mailbox_response( pp, core, 
		...
		{
			if (HFC_MBCMD_SNDRCV != hfc_fx_read_val(mbox->mb_init.mb_code)) {	/* FCLNX-GPL-FX-139 Start */
	     ...
	     || pp->mailbox_force_retry)
	{
		...
		if((target!=NULL)&&(test_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target->status))){
			...
			|| (((mb_code & 0xffff0000) == HFC_MBCMD_SNDRCV)&&(HFC_PRLI_REQDATA0 == (uchar) hfc_fx_read_val( pyload->send_payload.data0[0]))))
			return (HFC_MBPASS_RETRY_OVER);
		...
		|| ((mblog_fx.fsb_error_code & 0x00ffffff) == HFC_FSB_ERR_CLASS_NOT_SUPPORT2)){
			...
			|| ((mb_code & 0xffff0000) == HFC_MBCMD_PDISC)){
				fc_class = hfc_fx_read_val( mbox-> mb_init.type.cmn_ctl.fc_class);
				if(fc_class == HFC_FC_CLASS3){
					hfc_fx_write_val( mbox->mb_init.type.cmn_ctl.fc_class, HFC_FC_CLASS2);
					HFC_DBGPRT("hfcldd%d : hfcl_top, hfc_fx_mb_passthrough_rsp - Change the fc_class to HFC_FC_CLASS2\n", pp->dev_minor);
				}else{
					hfc_fx_write_val( mbox->mb_init.type.cmn_ctl.fc_class, HFC_FC_CLASS3);
		...
		if( (mblog_fx.fsb_error_code & 0x00ffffff) == 0x00d30000) {
			if(HFC_FX_MMODE_CHECK_SHADOW(pp)
...
void hfc_fx_initialize_failed(struct port_info * pp, struct core_info *core, struct target_info_fx *target)
{
	...
	if( (HFC_FX_MMODE_CHECK_SHARED(pp)) && (!HFC_FX_MMODE_CHECK_SHADOW(pp) ) ){	/* FCLNX-GPL-FX-670 */
		set_bit( HFC_PD_MLPF_WAIT_LINKEND, (ulong *)&pp->status_detail2 );
	}
	
	/* FCLNX-GPL-FX-679 >>> */
	if (HFC_FX_MQ_VALID(pp) && !HFC_FX_MQ_VIRTUAL_PORT(pp)) {
		...
		for (i=1; i<=pp->max_vport_count; i++) {
			...
			HFC_DBGPRT("hfcldd%d %s -1 rid=%d status=0x%08x\n",
					vpp->dev_minor, __FUNCTION__, vpp->rid, vpp->status);
			
			for (lp=0;lp<(int)vpp->max_target;lp++) {
				target_work = vpp->target_arg[lp];
				if(target_work != NULL) {
					target_work->status = HFC_NON_STATUS;
					HFC_DBGPRT("hfcldd%d %s -2 rid=%d tid=%d status=0x%08x\n", 
						vpp->dev_minor, __FUNCTION__, vpp->rid, target_work->target_id, target_work->status);
				}
			}
		}
		
		for (lp=0;lp<(int)pp->max_target;lp++) {
			target_work = pp->target_arg[lp];
			if(target_work != NULL) {
				/* Cancel pending SCSI processes for MQ port */
				hfc_fx_mq_cancel_scsi_cmd(pp, target_work, 0, NULL, SCS_LINKUP_TO, HFC_CSCSI_ERROR,
					TRUE, FALSE, FALSE, TRUE, TRUE, HFC_FLASH_TARGET);
				
				/* Clear HFC_TF_WWN_VALID for MQ port */
				hfc_fx_mq_change_target_info(pp, target_work);
				HFC_DBGPRT("hfcndd%d %s -3 rid=%d tid=%d status=0x%08x\n", 
						pp->dev_minor, __FUNCTION__, pp->rid, target_work->target_id, target_work->status);
			}
		}
	}
	/* FCLNX-GPL-FX-679 <<< */
...
//	HFC_DBGPRT("hfc_fx_issue_core_start_all() - 2");
	...
	for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
		...
		if(HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
		&& (!(pp->mig_available_core & (0x80)>>core->pcore_no))){
			HFC_DBGPRT("hfc_fx_issue_core_start_all() - core start() NG.core_no=%d\n",core->core_no);
			continue;	/* FCLNX-GPL-FX-438 */
		}
...
//	HFC_DBGPRT(" hfcldd : hfc_fx_top 2; hfc_fx_issue_core_start - entry \n"); 
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.core_start.self_wwpn, (pp -> mig_wwpn));
		hfc_fx_write_val( mb -> mb_init.type.core_start.self_wwnn, (pp -> mig_wwpn +1));
	}else{
...
//			}
	...
	if (success_add_portid == 0) {
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, HFC_MBCMD_ADDPORTID & 0x00000004);
		}
...
int hfc_fx_issue_add_portid( struct port_info *pp, struct core_info *core )
{
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		sid = ( uint )( pp -> mig_scsi_id & 0x00ffffff );
	}else{
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.add_port_id.self_wwpn, (pp -> mig_wwpn));
	}else{
...
//			}
	...
	if (success_del_portid == 0) {
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, HFC_MBCMD_DELPORTID & 0x00000004);
		}
...
int hfc_fx_issue_del_portid( struct port_info *pp, struct core_info *core)
{
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.delete_port_id.self_wwpn, (pp -> mig_wwpn));
	}else{
...
//	}/* FCLNX-GPL-FX-067 */
	...
	if(hfcp != NULL){
		...
		if(HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
			core = rp->core_arg[ pp->mig_master_core_no ];
		}else{
...
//			}
	...
	if(!HFC_FX_MQ_VIRTUAL_PORT(pp)) { /* FCLNX-GPL-FX-556 */
		if(cancel_nexus == HFC_CANCEL_ITLNEXUS){
			...
			HFC_DBGPRT("hfc_fx_all_cancel_scsi() - cscsi_mb_num=%d", pp->pport->cscsi_mb_num); /* FCLNX-GPL-FX-556 */
		}
...
/* FCLNX-GPL-FX-556 start */
void hfc_fx_mq_all_cancel_scsi(struct port_info *pp, struct target_info_fx *target, struct dev_info_fx *dev, struct hfc_pkt_fx *hfcp, uchar cancel_nexus, uchar cancel_ctl)
{
	struct port_info		*vpp;
//	struct port_info		*mqp=pp;
	struct target_info_fx	*target_wk = NULL;
	int i;
	
	HFC_ENTRY("hfc_fx_mq_all_cancel_scsi");
...
//			}
	...
	HFC_EXIT("hfc_fx_mq_all_cancel_scsi");
}
/* FCLNX-GPL-FX-556 end */
...
//	HFC_DBGPRT(" hfcldd : hfc_fx_top 0; hfc_fx_issue_linkini - 2 \n");
	...
	if (((pp->topology == HFC_FX_PT2PT) && (pp->multiple_portid))){ 
		mb->mb_init.type.link_init.connect_type = HFC_FX_F_PORT;
		/* FCLNX-GPL-FX-594 >>> */
		if( HFC_FX_MQ_VIRTUAL_PORT(pp) ){
			mb->mb_init.type.link_init.pidctl |= HFC_MBREQ_NO_REG_PORTID;
		}
		/* >>> FCLNX-GPL-FX-594 */
	}
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.link_init.self_wwpn, (pp -> mig_wwpn));
		hfc_fx_write_val( mb -> mb_init.type.link_init.self_wwnn, (pp -> mig_wwpn+1));
	}else{
...
//			}
	...
	if (success_offline_mb == 0) {
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, HFC_MBCMD_OFFLINEMB	 & 0x00000004);
		}
...
int hfc_fx_issue_offline_mb( struct port_info *pp, struct core_info *core)
{
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.offline_mb.self_wwpn, (pp -> mig_wwpn));
		hfc_fx_write_val( mb -> mb_init.type.offline_mb.self_wwnn, (pp -> mig_wwpn+1));
	}else{
...
int hfc_fx_issue_flogi( struct port_info *pp )
{
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.flogi.self_wwpn, (pp -> mig_wwpn));
		hfc_fx_write_val( mb -> mb_init.type.flogi.self_wwnn, (pp -> mig_wwpn+1));
	}else{
...
//	HFC_DBGPRT("hfc: hfc_fx_issue_plog start 5\n");
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.plogi.self_wwpn, (pp -> mig_wwpn));
		hfc_fx_write_val( mb -> mb_init.type.plogi.self_wwnn, (pp -> mig_wwpn+1));
	}else{
	...
	struct target_info_fx *target, uint scsi_id, uchar rctl )
{
	...
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		hfc_fx_write_val( mb -> mb_init.type.frmsndrcv.payload, 
				(uint64_t)pp->mig_payload_gpaddr  );		/* Bus address of Send payload part  */
	
		receive_pyld_add = (uint64_t)(pp->mig_payload_gpaddr+(uint)HFC_SEND_PAYLOADL_MAX);
	}else{
		hfc_fx_write_val( mb -> mb_init.type.frmsndrcv.payload, 
...
//	core -> mb_retry_cnt  = pp->els_retry ;		/* FCLNX-0523 */
	...
	memset( &pyload->send_payload, 0, (uint)HFC_SEND_PAYLOADL_MAX+HFC_RECV_PAYLOADL_MAX );
...
//	core -> mb_retry_cnt  = pp->els_retry ;		/* FCLNX-0523 */
	...
	/* Setup mailbox Send_Payload */
	hfc_fx_write_val( pyload->send_payload.data0[0], HFC_LOGO_REQDATA0 );	/* TBD */
	/* FCLNX-GPL-FX-488 >>> */
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		nport_id = ( uint )( pp -> mig_scsi_id & 0x00ffffff );
	}else{
	...
	hfc_fx_write_val( pyload->send_payload.data0[5], id);			/* FCLNX-GPL-FX-069 */
	id = (uchar)(nport_id >> 8 );											/* FCLNX-GPL-FX-069 */
	hfc_fx_write_val( pyload->send_payload.data0[6], id);			/* FCLNX-GPL-FX-069 */
	id = (uchar)nport_id;													/* FCLNX-GPL-FX-069 */
	hfc_fx_write_val( pyload->send_payload.data0[7], id);			/* FCLNX-GPL-FX-069 */
	hfc_fx_write_val( core -> payload->send_payload.type.logo.nport_name, pp->ww_name );	/* FCLNX-GPL-FX-646 */
...
//	core -> mb_retry_cnt  = pp->els_retry ;		/* FCLNX-0523 */
	...
	if( pyload != NULL ){
		...
		if(HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
			port_id = ( uint )( pp -> mig_scsi_id & 0x00ffffff );
		}else{
			port_id = ( uint )( pp -> scsi_id & 0x00ffffff );
		}
		/* >>> FCLNX-GPL-FX-488*/
...
//	core -> mb_retry_cnt  = pp->els_retry ;		/* FCLNX-0523 */
	...
	if( pyload != NULL ){
		...
		hfc_fx_write_val( pyload->send_payload.data0[5], 0x02);
		hfc_fx_write_val( pyload->send_payload.type.gxx.data1[0], 0x02);
		hfc_fx_write_val( pyload->send_payload.type.gxx.data1[1], 0x1f);
		hfc_fx_write_val( pyload->send_payload.type.gxx.data1[3], 0x01);
		/* FCLNX-GPL-FX-488 >>> */
		if(HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
			HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_make_rff_id - 6 \n");
			port_id = ( uint )( pp -> mig_scsi_id & 0x00ffffff );
		}else{
			port_id = ( uint )( pp -> scsi_id & 0x00ffffff );
		}
		/* >>> FCLNX-GPL-FX-488*/
		hfc_fx_write_val( pyload->send_payload.type.gxx.sub_type.rff_id.port_identifier, port_id );

		hfc_fx_write_val( pyload->send_payload.type.gxx.sub_type.rff_id.data2[2], 0x02);	/* payload +0x16 */
		hfc_fx_write_val( pyload->send_payload.type.gxx.sub_type.rff_id.data2[3], 0x08);	/* payload +0x17 */
	}
			...
			if(pp->region_arg[pp->rid]->core_arg[i] != NULL){
				...
				if (core == NULL){
					...
					if( !hfc_fx_check_cs_disable(pp, core) ){	/* FCLNX-GPL-FX-438 */
						...
						&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
						&& (!(pp->mig_available_core & (0x80)>>core->pcore_no))){
							HFC_DBGPRT("hfc_fx_issue_change_state() - change state() NG.core_no=%d\n",core->core_no);
							continue;	/* FCLNX-GPL-FX-438 */
						}
...
//					d_time = (pp->lun_reset_tmo * HZ);
			...
			case HFC_FX_MLPF_ISOLEND_TMR:
				...
				break;
			/* FCLNX-GPL-FX-488 >>> */
			case HFC_FX_MIG_SCCMND_TMR:			/* for MLPF */
				d_time = (HFC_FX_MIG_SCCMND_TO * HZ);
				w_timer = &pp->mig_sccmnd_wdog; 
				break;
			/* >>> FCLNX-GPL-FX-488 */
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0) /* FCLNX-GPL-FX-699 --> 717 */
...
{
	wait_event_timeout(*event, atomic_read(condition), HFC_MAX_SLEEP); /* FCLNX-GPL-FX-705 */
...
/* FCLNX-GPL-FX-683 */
struct Scsi_Host *
hfc_fx_scsi_host_alloc_new(struct scsi_host_template_new *sht, int privsize)
{
	struct Scsi_Host *addr = NULL;
	...
	addr = scsi_host_alloc((struct scsi_host_template *)sht, privsize);
