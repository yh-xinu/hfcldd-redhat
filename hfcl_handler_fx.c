...
hfc_fx_intr_xrb(int irq, void *intr_entry)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,17,0) /* FCLNX-GPL-FX-699 --> 717 */
		time = rdtsc_ordered();
#else
		rdtscll(time);
#endif
	...
	if (HFC_FX_VPORT_EXIST(pp)) {
		...
		mpint.l = (uint)hfc_fx_read_reg_core(pp, core_no, HFC_IOSPACE_MPINTAC,
											 0x04, HFC_FX_CORE_OFFSET10);
//		HFC_DBGPRT("hfcldd :  hfc_fx_intr_xrb, rid=0x%08x\n", mpint.l);
		mpint.c[0] = 0x00;
		/* >>> FCLNX-GPL-FX-488 */
	}
	else {
	...
	if(hfc_manage_info.hfcldd_mp_mod) {
		HFC_CORELOCK(core);/* FCLNX-GPL-FX-593 */
	} else {
		HFC_CORELOCK_IRQSAVE(core, flags);/* FCLNX-GPL-FX-547 */
	}		
	if ((!core->tskmgm_cmd_num) && (!core->preserve_cmd_num)){	/* FCLNX-FX-031 */
		...
		if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ){
			HFC_PORTUNLOCK(pp);
			HFC_SYSLOCK(pp);
//			HFC_PORTLOCK(pp); /* FCLNX-GPL-FX-627 */
		}
		HFC_PORTLOCK(pp); /* FCLNX-GPL-FX-547 */
		HFC_ALLCORELOCK_MQ(pp); /* FCLNX-GPL-FX-569 */
		portlock = 1;
		
		if((!core->tskmgm_cmd_num) && (!core->preserve_cmd_num)){
			HFC_ALLCOREUNLOCK_MQ(pp); /* FCLNX-GPL-FX-569 */
			if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ){
				HFC_SYSUNLOCK(pp);
			}
			HFC_CORELOCK(core);
	...
	if (vpp == NULL) {
		...
		if( HFC_FX_MMODE_CHECK_SHADOW(pp) ){
			/* FCLNX-GPL-FX-488 >>> */
			rtn = hfc_fx_xrb_resp_shadow(vpp, rp, core, time, cpuno, &wait_iodone_hfcp);
			/* >>> FCLNX-GPL-FX-488 */
		}else{
...
//								HFC_ALLCOREUNLOCK(rp); /* FCLNX-GPL-FX-569 comment out*/
								
								/* all core_lock for rid0 */
//								HFC_ALLCORELOCK(pp->region_arg[0]); /* FCLNX-GPL-FX-569 comment out*/
								if (hfc_fx_check_errcount(pp)) {
									/* isolate done */
									rp = pp->region_arg[0];
									goto intr_xrb_exit;
								}
//								HFC_ALLCOREUNLOCK(pp->region_arg[0]); /* FCLNX-GPL-FX-569 comment out*/
								
//								HFC_ALLCORELOCK(rp); /* FCLNX-GPL-FX-569 comment out*/
							}
						}
						else {
							if (hfc_fx_check_errcount(pp)) {
...
//				structdump( 0xcc, (uchar *)(wait_iodone_hfcp), sizeof(struct hfc_pkt_fx) );
		...
		HFC_ALLCOREUNLOCK_MQ(pp);	/* FCLNX-GPL-FX-584 */
		if (!hfc_manage_info.hfcldd_mp_mod) {
...
hfc_fx_intr_share(int irq, void *intr_entry)
{
	...
	uchar					portlock;
	uchar					irqsave = 0; /* FCLNX-GPL-FX-547 */
	uchar					trace = 0;
	...
	if( int_vector.l == 0 ) /* Check whether this int. is for handler or not */
	{
		...
		return IRQ_HANDLED; /*FCLNX-GPL-FX-547 */
	}
	...
	{
		if(!hfc_manage_info.hfcldd_mp_mod) {	/* FCLNX-GPL-FX-593 */
			HFC_PORTLOCK_IRQSAVE(pp,flags); /* FCLNX-GPL-FX-547 */
		}
		HFC_ALLCORELOCK_MQ(pp);			/* FCLNX-GPL-FX-584 */
		...
		HFC_ALLCOREUNLOCK_MQ(pp);			/* FCLNX-GPL-FX-584 */
		/*FCLNX-GPL-FX-547:Bottom HFC_PORTUNLOCK_IRQRESTORE(pp,flags); */
	}
		...
		for(i=0; i<4; i+=4/pp->core_num)
		{
			...
			if( int_vector.c[3-i] & 0x40 )
			{
				...
				if (HFC_FX_VPORT_EXIST(pp)) {
					...
					mpint.l = (uint)hfc_fx_read_reg_core(pp, i, HFC_IOSPACE_MPINTAC,
														 0x04, HFC_FX_CORE_OFFSET10);
					pp->mig_mbint_rid = 0x00;
					/* >>> FCLNX-GPL-FX-488 */
//					HFC_DBGPRT("hfcldd : hfcl_intr_share - Mailbox response received, rid=0x%08x\n", mpint.l);
				}else {
					mpint.l = 0;
				}
				if(HFC_FX_MMODE_CHECK_SHADOW(pp) && (mpint.c[1] == pp->mig_rid)){
					/* FCLNX-GPL-FX-488 >>> */
					int_rid_of_core[i] = 0x00;
					pp->mig_mbint_rid = pp->mig_rid;
//					HFC_DBGPRT("hfcldd : hfcl_intr_share - Mailbox response received, rid=%d\n", mpint.c[1]);
					/* >>> FCLNX-GPL-FX-488 */
				}else if ( mpint.c[1] == 0x80) {
					/* physical port */
					int_rid_of_core[i] = 0x00;
				}else if(HFC_FX_MMODE_CHECK_MLPF(pp) && !(HFC_FX_MMODE_CHECK_SHADOW(pp) )){
					int_rid_of_core[i] = pp->rid;
	...
	else if( int_vector.l & 0x80808080 )
	{
		...
		for(i=0; i<4; i+=4/pp->core_num)
		{
			...
			if( int_vector.c[3-i] & 0x80 )
			{
				...
				if(HFC_FX_MMODE_CHECK_SHADOW(pp) && (mpint.c[2] != 0x00) && (mpint.c[2] != 0x80)){
					/* FCLNX-GPL-FX-488 >>> */
					int_rid_of_core[i] = 0x00;
					pp->mig_mbint_rid = mpint.c[2];
					/* >>> FCLNX-GPL-FX-488 */
				}else if ( mpint.c[2] == 0x80) {
					/* physical port */
					int_rid_of_core[i] = 0x00;
				}else if(HFC_FX_MMODE_CHECK_MLPF(pp) && !(HFC_FX_MMODE_CHECK_SHADOW(pp) )){
					int_rid_of_core[i] = pp->rid;
	...
	else if( int_vector.l & 0x20202020 )
	{
		...
		for(i=0; i<4; i+=4/pp->core_num)
		{
			...
			{
				/* Read RID */
				if(HFC_FX_MMODE_CHECK_MLPF(pp) && HFC_FX_MMODE_CHECK_SHADOW(pp) ){
					/* FCLNX-GPL-FX-488 >>> */
					mpint.l = (uint)hfc_fx_read_reg_core(pp, i, HFC_IOSPACE_MPINTAC,
											 0x04, HFC_FX_CORE_OFFSET10);
					pp->mig_mbint_rid = 0x00;
				...
				if(HFC_FX_MMODE_CHECK_SHADOW(pp) && (mpint.c[0] == pp->mig_rid)){
					/* FCLNX-GPL-FX-488 >>> */
					int_rid_of_core[i] = 0x00;
					pp->mig_mbint_rid = pp->mig_rid;
					/* >>> FCLNX-GPL-FX-488 */
				}else if ( mpint.c[0] == 0x80) {
					/* physical port */
					int_rid_of_core[i] = 0x00;
				}else if(HFC_FX_MMODE_CHECK_MLPF(pp) && !(HFC_FX_MMODE_CHECK_SHADOW(pp) )){
					int_rid_of_core[i] = pp->rid;
		...
		for(i=0; i<4; i+=4/pp->core_num)
		{
			if( int_rid_of_core[i] != 0xff )
			{
				...
				/*FCLNX-GPL-FX-547 start ---------------*/
				if (!portlock) { /* FCLNX-GPL-FX-607 */
					if((irqsave == 0) && (!hfc_manage_info.hfcldd_mp_mod)){	/* FCLNX-GPL-FX-593 */
						HFC_CORELOCK_IRQSAVE(core, flags);
						irqsave = 1;
					}else{
						HFC_CORELOCK(core);
					}
				} else {
					HFC_ALLCORELOCK_MQ(pp);
				}
				/*---------------- FCLNX-GPL-FX-547 end */

				if ((!core->tskmgm_cmd_num) && (!core->preserve_cmd_num) && (!pp->pport->cscsi_mb_num)){ /* FCLNX-GPL-FX-556 */
					/* task management command none in core or not master core */
				//	HFC_PORTUNLOCK(pp); /*FCLNX-GPL-FX-547 comment out/
					if (hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() && (!portlock)) { /* FCLNX-GPL-FX-593 */
						HFC_PORTUNLOCK(pp);	/* FCLNX-GPL-FX-593 */
						HFC_SYSUNLOCK(pp);
					}
				//	portlock = 0;	/*FCLNX-GPL-FX-547 comment out/
				}
				else {
					/* task management command exists in core and master core */
					/*FCLNX-GPL-FX-547 start -----*/
				...
				if (!rtn) {
					...
					if (!portlock) {
						if (xrb_factor_num == 1) {
							...
							if (hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable()) {
								HFC_SYSLOCK(pp);
								HFC_PORTLOCK(pp);	/* FCLNX-GPL-FX-593 */
								portlock = 1;		/* FCLNX-GPL-FX-623 */
							}
						}
					}
					else {
...
//											HFC_ALLCORELOCK(rp);
					...
					if (rtn & (HFC_XRBRESP_TMCMD_NORMAL | HFC_XRBRESP_TMCMD_ERR)) {
						/* FCLNX-GPL-FX-556 start ------ */
						if (HFC_FX_MQ_VALID(pp)) {
							for (i=0; i<=vpp->pport->max_vport_count; i++) {
								wkpp = vpp->pport->vport_ptr[i].vport_arg;
								if (wkpp == NULL)
									continue;
								
								HFC_DBGPRT("hfcndd : hfcn_intr_share HFC_XRBRESP_TMCMD 1 rid=%d, sub_rid=%d\n", wkpp->rid, wkpp->sub_rid);
								if((atomic_read(&wkpp->check_mbreq)) && !test_bit(HFC_PD_LOGIN_DELAYI, (ulong *)&vpp->status_detail2 )) 
								{
									HFC_DBGPRT("hfcndd : hfcn_intr_share HFC_XRBRESP_TMCMD 2\n");
									start_fx_next_mailbox(wkpp, NULL);
								}
								HFC_DBGPRT("hfcndd : hfcn_intr_share HFC_XRBRESP_TMCMD 3\n");
                                                	}								
						}
						else{
							if((atomic_read(&vpp->check_mbreq)) && !test_bit(HFC_PD_LOGIN_DELAYI, (ulong *)&vpp->status_detail2 )) {
								HFC_DBGPRT("hfcndd : hfcn_intr_share HFC_XRBRESP_TMCMD 4\n");
								start_fx_next_mailbox(vpp, NULL);
							}
						}
						HFC_DBGPRT("hfcndd : hfcn_intr_share HFC_XRBRESP_TMCMD 5\n");
						/* ------ FCLNX-GPL-FX-556 end*/
					}
...
//intr_share_exit: /* FCLNX-GPL-547 intr_share_exit :defined but not uesd :comment out*/
	if (!hfc_manage_info.hfcldd_mp_mod) {
		HFC_PORTUNLOCK_IRQRESTORE(pp,flags);
	}else{
		HFC_PORTUNLOCK(pp);
...
void hfc_fx_mb_resp(struct port_info *pp, struct core_info *core, struct region_info *rp)
{
	...
	struct payload_fx	*pyload = NULL;	/* FCLNX-GPL-FX-488 */
...
//	HFC_DBGPRT("*hfcldd : entry %s()\n)","hfc_fx_mb_resp");
	...
	else
	{
		/* FCLNX-GPL-FX-488 >>> */
		if(HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
//			HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_mb_resp - 3 \n");
			mbox = core->mig_mb;
			pyload = pp->mig_payload_vsaddr;	/* TBD */
//			if(test_bit( HFC_PD_WAIT_CORE_START, (ulong *)&pp->status_detail1 ))
//				structdump_err( 0xfe, (uchar *)core->mig_mb, sizeof(struct mailbox_fx) );
		}else{
			mbox = core->mb;
			pyload = core->payload;
		}
		...
		if((mb_code & 0xffff0000) == HFC_MBCMD_SNDRCV){
			payload_cmd = (uchar) hfc_fx_read_val( pyload->send_payload.data0[0]);	/* FCLNX-GPL-FX-488 */
...
void hfc_fx_add_port_id_resp(struct port_info *pp, struct core_info *core )
{
	...
	struct mailbox_fx	*mbox = NULL;	/* FCLNX-GPL-FX-488 */
	uint				i;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_add_port_id - 3 \n");
		mbox = core->mig_mb;
	}else{
		mbox = core->mb;
	}
	...
	case HFC_MB_FATAL :
		...
		/* FCLNX-GPL-FX-502 Start */
		if(HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
			if (core->mb_results == 0x02d30000){
				for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
					...
					clear_bit(HFC_CS_WAIT_MAILBOX, (ulong *)&core_wk->status);
					hfc_fx_watchdog_enter( pp , core_wk, NULL, NULL, 0, HFC_FX_MB_RSP_TMR, 0, TRUE);
				}
				clear_bit(HFC_PD_WAIT_ADD_PORTID, (ulong *)&pp->status_detail1);
				hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
				unlock_fx_mailbox(pp);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_ADDPORTID | 0x00000005));	/* FCLNX-GPL-FX-517 */
				return;
			}
		}
		/* FCLNX-GPL-FX-502 End */
	...
	for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
		...
		/* FCLNX-GPL-FX-680 >>> */
		if (core_wk->mb_results != HFC_MBPASS_SUCCESS) {
		    if (core_wk->core_no == pp->master_core_no) {
	...
	if( tmp_linkdown_occurred == 1 ){	/* FCLNX-GPL-FX-058,174  */
		unlock_fx_mailbox(pp);
		/* FCLNX-GPL-FX-517 >>> */
		if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000002);
		}
		/* >>> FCLNX-GPL-FX-517 */
		return;
	}	/* FCLNX-GPL-FX-058 */
	...
	if (failed_master_core) {
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_ADDPORTID | 0x00000002));	/* FCLNX-GPL-FX-517 */
		
		/* FCLNX-GPL-FX-680 */
		HFC_DBGPRT("add_port_id_resp, failed_master_core\n");
		unlock_fx_mailbox(pp);
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_ADDPORTID | 0x00000003));	/* FCLNX-GPL-FX-517 */
		
		/* FCLNX-GPL-FX-680 */
		HFC_DBGPRT("add_port_id_resp, failed_slave_core\n");
		unlock_fx_mailbox(pp);
	...
	case HFC_MB_FATAL :
		...
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
			if (core->mb_results == 0x02d30000){
				...
				hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
				unlock_fx_mailbox(pp);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_DELPORTID | 0x00000005));	/* FCLNX-GPL-FX-517 */
				return;
			}
		}
		/* FCLNX-GPL-FX-502 End */
...
void hfc_fx_mihlog_resp(struct port_info *pp, struct core_info *core )
{
	...
	uint				zero[HFC_FX_MAX_MQ_NUM] = {0,0,0,0,0,0,0,0};	/*FCLNX-GPL-FX-556*/
...
//	struct hfc_pkt_fx	*hfcp_end_p;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
//		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_mihlog_resp - 3 \n");
		mbox = core->mig_mb;
	}else{
		mbox = core->mb;
	}
	...
	case HFC_MB_FATAL :
		...
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
			if (core->mb_results == 0x02d30000){
				...
				hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
				unlock_fx_mailbox(pp);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_MIHLOG | 0x00000005));	/* FCLNX-GPL-FX-517 */
				return;
			}
		}
		/* FCLNX-GPL-FX-502 End */
		...
		{	
			if( hfcp_wk != NULL ){
				...
				if (find_hfcp) {
					...
					if( HFC_FX_MMODE_CHECK_SHARED(pp) && (!HFC_FX_MMODE_CHECK_SHADOW(pp)) && pp->hvm_extended_log){ /* FCLNX-GPL-FX-582,595 >>>*/
						pp->mlpf_xlog_flag = 1;
						hfc_fx_errlog( pp, core, hfcp_to->target, hfcp_wk, HFC_ERRLOG_TYPE_TOUTLOG, ERRID_HFCP_ERRA, 0x24, hfcp_wk->core->logdata, 16 ); /* FCLNX-GPL-FX-091 */
						hfc_fx_mlpf_indacc(pp, core, hfcp_to->tout_slog_ssn / 4, hfcp_to->tout_slog_ssn % 4, HFC_ERRLOG_TYPE_TOUTLOG);/* >>>FCLNX-GPL-FX-582 */
					}
				}
				else {
	...
	if(hfcp_wk != NULL){
		...
		||((test_bit(CFLAG_TARGET_RESET, (ulong *)&hfcp_wk->cmd_flags))&&(test_bit( HFC_PD_WAIT_BUSRSP, (ulong *)&pp->status_detail2 )))){
		...
		}else if((target != NULL)&&(dev != NULL)&&(test_bit(HFC_DC_WAIT_MIHLOG, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]))){	/* FCLNX-GPL-FX-178 *//* FCLNX-GPL-FX-282, 288 */
			clear_bit(HFC_DC_WAIT_MIHLOG, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);	/* FCLNX-GPL-FX-556 */
			HFC_DBGPRT( "hfc_fx_mihlog_resp - 1\n");
			if ( (memcmp(dev->dev_core_stat.all, zero, sizeof(uint)*HFC_FX_MAX_MQ_NUM) == 0)&&(test_bit(HFC_DS_NEED_ABORT, (ulong *)&dev->lustat) )){	/* FCLNX-GPL-FX-178 */ /* FCLNX-GPL-FX-556 */
				HFC_DBGPRT( "hfc_fx_mihlog_resp - 2\n");			
				if((!test_bit(HFC_DS_FAIL_ABORT, (ulong *)&dev->lustat))&&(!test_bit(HFC_DS_FAIL_LUN_RST, (ulong *)&dev->lustat))){
					...
					if (HFC_FX_MQ_VALID(pp)) {
						HFC_DBGPRT( "hfc_fx_mihlog_resp - 3\n");
						...
						HFC_DBGPRT( "hfc_fx_mihlog_resp - 4\n");
						hfc_fx_issue_devrst_cscsi(
			...
			clear_bit(CFLAG_SCMD_SLOGV, (ulong *)&hfcp_wk->cmd_flags);
			hfc_fx_deque_we_que(pp, hfcp_wk->target, hfcp_wk);
			clear_bit(HFC_DC_WAIT_LUN_RESET_OR_ABORT, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);/*FCLNX-GPL-FX-556*/
		}		/* FCLNX-GPL-FX-320 End */
	}
...
{
	uint                rc_passthrouh = 0xffffffff;
	struct port_info	*vpp=NULL;	/* FCLNX-GPL-FX-556 */
	struct region_info	*rp=NULL;
	struct target_info_fx  *target=NULL;
	struct mailbox_fx	*mbox;
	struct core_info 	*core_wk=NULL;
	uint				i=0, j=0,wait_mb_core=0, did=0;					/* FCLNX-GPL-FX-112 */ /* FCLNX-GPL-FX-556 */
...
//	HFC_DBGPRT("hfc_fx_cancel_scsi_resp");
	...
	if (HFC_FX_MQ_VALID(pp)) {	
		...
		for(j=0; j<=pp->pport->max_vport_count;j++){		/* FCLNX-GPL-FX-556 */
			vpp = pp->pport->vport_ptr[j].vport_arg;	/* FCLNX-GPL-FX-556 */	
			if(vpp == NULL)					/* FCLNX-GPL-FX-556 */
				continue;				/* FCLNX-GPL-FX-556 */

			for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
				core_wk = vpp->region_arg[vpp->rid]->core_arg[i];	/* FCLNX-GPL-FX-556 */
				if ( core_wk == NULL
					 || core_wk == core
					 || hfc_fx_check_cs_disable(vpp, core_wk) )	/* FCLNX-GPL-FX-438 */
					continue;
				if ( test_bit(HFC_CS_WAIT_MAILBOX, (ulong *)&core_wk->status) ){
					wait_mb_core = 1;
					break;
			...
			if(wait_mb_core) /* FCLNX-GPL-FX-556 */
				break;
		}
	}
	else{ /* FCLNX-GPL-FX-556 start ------ */
		/* check if cancel_scsi of all cores finished or not. */
		for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
			core_wk = pp->region_arg[pp->rid]->core_arg[i];
			if ( core_wk == NULL
				 || core_wk == core
				 || test_bit(HFC_CS_CHK_STOP, (ulong *)&core_wk->status) )
				continue;
			if ( test_bit(HFC_CS_WAIT_MAILBOX, (ulong *)&core_wk->status) ){
				wait_mb_core = 1;
				break;
	...
	/* cancel_scsi of other cores have not finished yet. */
	if (wait_mb_core) {
		hfc_fx_hand2_trace(
			HFC_FX_TRC_CSCSIRSP, 0x03, pp, core->rp, core, NULL, NULL,
			(uint64_t)rc_passthrouh, 0, 0 );
		HFC_DBGPRT("cancel_scsi of other cores have not finished yet. rid=%d, core_no=%x",pp->rid, core->core_no);
		return;
	}
	pp->pport->cscsi_mb_num--; /* FCLNX-GPL-FX-556 */
	HFC_DBGPRT("hfc_fx_cancel_scsi_resp - cscsi_mb_num=%d", pp->pport->cscsi_mb_num);	
	...
	if(target != NULL){
		...
		if(mbox -> mb_init.type.cscsi.cnexus == HFC_CANCEL_ITLNEXUS){
			...
			if(dev != NULL){
				...
				if (HFC_FX_MQ_VALID(pp)) { /* FCLNX-GPL-FX-572 */
			...
			hfc_fx_hand2_trace(
				...
				(uint64_t)rc_passthrouh, 0, 0 );
			if(HFC_FX_MQ_VALID(pp)) {	/* FCLNX-GPL-FX-556 */
				clear_bit(HFC_TS_WAIT_CANCEL_SCSI_WAIT_DMA, (ulong *)&pp->pport->target_arg[target->pseq]->status);	/* FCLNX-GPL-FX-556 */
			}
			else{
			...
			for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
				if ((core_wk = rp->core_arg[i]) == NULL)
					...
					/* SoftLo Errlog output */
				if(HFC_FX_MQ_VALID(pp)) {       /* FCLNX-GPL-FX-556 */
					hfc_fx_watchdog_enter(
						pp->pport,
						pp->pport->region_arg[0]->core_arg[core_wk->core_no],
						pp->pport->target_arg[target->pseq],
						NULL,
						0,
						HFC_FX_TOTAL_TGTRST_TMR,
						0,
						TRUE);
					clear_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&pp->pport->target_arg[target->pseq]->status);			
				}
				else{
				...
				while(dev != NULL) {			/* FCLNX-GPL-FX-190 Start */
					dev->lustat = 0x00;
					dev->dev_core_stat.all[pp->vport_id] = 0; /* FCLNX-GPL-FX-556 */
					if (HFC_FX_MQ_VALID(pp)) {  /* FCLNX-GPL-FX-556 */
						hfc_fx_mp_watchdog_enter(
							pp->pport,
							pp->pport->region_arg[0]->core_arg[core_wk->core_no],
							pp->pport->target_arg[target->pseq],
							NULL,
			...
			/* FCLNX-GPL-FX-573 start ------- */
			if (HFC_FX_MQ_VALID(pp)) {
				if (test_bit(HFC_TS_WAIT_TARGET_STOP, (ulong *)&pp->pport->target_arg[target->pseq]->status)) {
					clear_bit(HFC_TS_WAIT_TARGET_STOP, (ulong *)&pp->pport->target_arg[target->pseq]->status);
					...
					hfc_fx_clear_target_info_fx( pp->pport, pp->pport->target_arg[target->pseq], TRUE );
					hfc_fx_mq_change_target_info(pp->pport, pp->pport->target_arg[target->pseq]);
				}
			}
			else{
				/* FCLNX-GPL-FX-554 start */
				if (test_bit(HFC_TS_WAIT_TARGET_STOP, (ulong *)&target->status)) {
					clear_bit(HFC_TS_WAIT_TARGET_STOP, (ulong *)&target->status);
					/* Discards XRB when SCN_WLINKUP timer timed out */
					for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
						if ((core_wk = rp->core_arg[i]) == NULL)
							continue;
						hfc_fx_cancel_scsi_cmd(pp, core_wk, target, 0, NULL, SCS_SCN_LINKDOWN,          /* FCLNX-GPL-038 */
				...
				/* FCLNX-GPL-FX-554 end */
			}
			/* ------ FCLNX-GPL-FX-573 end*/
		}
	}
	...
	if (HFC_FX_MQ_VALID(pp)) {
		for (j=0; j<=pp->pport->max_vport_count; j++) {
			...
			/* STOP mailbox retry timer per port */
			hfc_fx_w_stop(vpp, NULL, HFC_FX_MB_RETRY_TMR);
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
						...
						hfc_fx_watchdog_enter( pp , wk_core, NULL, NULL, 0, HFC_FX_MB_RSP_TMR, 0, TRUE);
					}
					clear_bit(HFC_PD_WAIT_CORE_START, (ulong *)&pp->status_detail1);
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_CORESTART | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
...
//	HFC_DBGPRT("clear_bit HFC_CS_WAIT_MAILBOX core_no%d", core->core_no);
	...
	for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
		...
		if(hfc_fx_check_cs_disable(pp, wk_core))
			continue;	/* FCLNX-GPL-FX-438 */
		if (test_bit(HFC_CS_WAIT_MAILBOX, (ulong *)&wk_core->status) ) {
			wait_mb_core = 1;
			break;
		}
		if (wk_core->mb_results == HFC_MBPASS_SUCCESS) {
			available_pcore |= (0x80 >> wk_core->pcore_no);
		}
	}
...
//			hfc_fx_hw_chk_stop(pp);		/* TBD */
		...
		if(pp->initialize != 0){
			if (!( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() )
			|| (!pp->after_isolrec)){
				if((!pp->after_isolrec) && (!pp->before_linkini))
					clear_bit(HFC_PD_NEED_LINK_INI, (ulong *)&pp->status_detail1);	/* FCLNX-GPL-FX-575,605 */
...
void hfc_fx_offline_mb_resp(struct port_info *pp, struct core_info *core )
{
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
//		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_offline_mb_resp - 3 \n");
		mbox = core->mig_mb;
	}else{
		mbox = core->mb;
	}
	...
	case HFC_MB_FATAL :
		...
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
			if (core->mb_results == 0x02d30000){
				...
				hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
				unlock_fx_mailbox(pp);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_OFFLINEMB | 0x00000001));	/* FCLNX-GPL-FX-517 */
				return;
			}
		}
		/* FCLNX-GPL-FX-502 End */
...
void hfc_fx_link_resp(struct port_info *pp, struct core_info *core )
{
	...
	struct fw_init_tbl_fx	*fw_init_p=NULL;	/* FCLNX-GPL-FX-488 */
	uint				sysrev = 0;	/* FCLNX-GPL-FX-556  */
	uint				lpar_support = 0x00000307; /* LPAR support FW vwrsion */ /* FCLNX-GPL-FX-556 */
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
//		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_link_resp - 3 \n");
		mbox = core->mig_mb;
		fw_init_p = core->mig_fw_init_p;
	}else{
		mbox = core->mb;
		fw_init_p = core->fw_init_p;
	}
	/* >>> FCLNX-GPL-FX-488*/
		...
		if(	(rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_LINKINIT | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
...
//		HFC_DBGPRT("link_resp, success %d \n",rc_passthrouh);
		...
		if( HFC_FX_MMODE_CHECK_SHARED(pp) && !(HFC_FX_MMODE_CHECK_SHADOW(pp) ) ) 
			fw_init_p->fw_iocinfo.connect_type = hfc_fx_read_val( mbox->mb_resp.type.linkini.connect_type );	/* FCLNX-GPL-FX-488 */
		else
			fw_init_p->fw_iocinfo.connect_type = hfc_fx_read_val( mbox->mb_resp.type.linkini.link_con_type );	/* FCLNX-GPL-FX-488 */
		
		fw_init_p->fw_iocinfo.trans_rate = hfc_fx_read_val( mbox->mb_resp.type.linkini.trans_rate );	/* FCLNX-GPL-FX-488 */
		
		fw_init_p->fw_iocinfo.configure_flag = hfc_fx_read_val( mbox->mb_resp.type.linkini.link_config_flag );	/* FCLNX-GPL-FX-488 */
		flag = fw_init_p->fw_iocinfo.configure_flag;	/* FCLNX-GPL-FX-488 */
...
//			flag);
		...
		switch( core->fw_init_p->fw_iocinfo.connect_type ){
			...
			case HFC_FX_MULTI_ALPA:
				fw_init_p->fw_iocinfo.alpa_count = hfc_fx_read_val( mbox->mb_resp.type.linkini.alpa_count );	/* FCLNX-GPL-FX-488 */
				if( flag & HFC_FX_POSMAP_VALID ){
					count = (uint) hfc_fx_read_val(mbox->mb_resp.type.linkini.position_map[0]);	/* FCLNX-GPL-FX-488 */
//					core->fw_init_p->fw_iocinfo.assign_alpa = hfc_fx_read_val( mbox->mb_resp.type.linkini.assign_alpa );	/* FCLNX-GPL-FX-488 */
					if( count > 0){
						fw_init_p->pos_map[0] = mbox->mb_resp.type.linkini.position_map[0];	/* FCLNX-GPL-FX-488 */
						for (i=1;i<=count; i++) {
							fw_init_p->pos_map[i] = mbox->mb_resp.type.linkini.position_map[i];	/* FCLNX-GPL-FX-488 */
							if( fw_init_p->pos_map[i] == 0x00 ){	/* FCLNX-GPL-FX-488 */
								loop_direct = 0;
							}
						}
					}
				...
				if( flag & HFC_FX_ALPA_VALID ){
					fw_init_p->fw_iocinfo.assign_alpa = hfc_fx_read_val(mbox->mb_resp.type.linkini.assign_alpa );	/* FCLNX-GPL-FX-488 */

					/* FCLNX-GPL-FX-488 >>> */
					if(HFC_FX_MMODE_CHECK_SHADOW(pp)
					&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
						HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_link_resp - 5 \n");
						pp->mig_scsi_id = (uint64_t)hfc_fx_read_val(mbox->mb_resp.type.linkini.assign_alpa );
					}else{
						pp->scsi_id = (uint64_t)hfc_fx_read_val(mbox->mb_resp.type.linkini.assign_alpa );
					}
					/* >>> FCLNX-GPL-FX-488*/

					fw_init_p->fw_iocinfo.self_port_id[0] = 0;
					fw_init_p->fw_iocinfo.self_port_id[1] = 0;
					fw_init_p->fw_iocinfo.self_port_id[2] = (uchar)hfc_fx_read_val(mbox->mb_resp.type.linkini.assign_alpa );
					fw_init_p->fw_iocinfo.configure_flag |= HFC_FX_PID_VALID;
				}
				if( fw_init_p->fw_iocinfo.connect_type == HFC_MULTI_ALPA ){
					count = mbox->mb_resp.type.linkini.acquired_alpa[0];
					if( count > 0){
						fw_init_p->active_alpa[0] = mbox->mb_resp.type.linkini.acquired_alpa[0];	/* FCLNX-GPL-FX-488 */
						for (i=1;i<=count; i++) {
							fw_init_p->active_alpa[i] = mbox->mb_resp.type.linkini.acquired_alpa[i];	/* FCLNX-GPL-FX-488 */
						}
					}
				}
...
//	HFC_DBGPRT("hfcldd : hfcl_intr - hfc_fx_link_resp copy iocinfo port_info = %lx\n",(ulong)pp);
	...
	if (mb_resp_status == 0) {
		if( loop_direct == 1 )
		{
			...
			if( pp->connect_type == HFC_FX_F_PORT ){
				...
				if(( (HFC_FX_MMODE_CHECK_SHARED(pp)) && (!HFC_FX_MMODE_CHECK_SHADOW(pp) ) )
				|| ( (HFC_FX_MMODE_CHECK_SHADOW(pp)) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2))	){	/* FCLNX-GPL-FX-386 Start */
					/* FCLNX-GPL-FX-488 >>> */
					if(HFC_FX_MMODE_CHECK_SHADOW(pp)
					&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
						HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_link_resp - 5 \n");
						pp->mig_scsi_id = 0;
						domain = (uchar)pp->mig_rid + 0x80;
						pp->mig_scsi_id |= (uint64_t)(domain << 16);
					}else{
						pp->scsi_id = 0;
					...
					/* FCLNX-GPL-FX-488 >>> */
					fw_init_p->fw_iocinfo.p2p_tgt_port_id[0] = 0x01;
					fw_init_p->fw_iocinfo.p2p_tgt_port_id[1] = 0;
					fw_init_p->fw_iocinfo.p2p_tgt_port_id[2] = 0;
					/* >>> FCLNX-GPL-FX-488 */
					
					/* Set self_port_id of Init_Table */
					/* FCLNX-GPL-FX-488 >>> */
					fw_init_p->fw_iocinfo.self_port_id[0] = domain;
					fw_init_p->fw_iocinfo.self_port_id[1] = 0;
					fw_init_p->fw_iocinfo.self_port_id[2] = 0;
					/* >>> FCLNX-GPL-FX-488 */
				...
				if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
					 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_LINKINIT | 0x00000002));	/* FCLNX-GPL-FX-517 */
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			if ((core->mb_results == 0x02FF1004)&&(pp->mig_tgtlist != NULL)&&(pp->mig_tgtlist->target_cnt == 0x00)){
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_NOMAL, 0x00000006);
			}else{
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_LINKINIT | 0x00000003));	/* FCLNX-GPL-FX-513 */
			}
		}
	}
	...
	if (HFC_FX_PHYSICAL_PORT(pp)) {
		...
		if ((sysrev & 0x0000ffff) >= lpar_support) {
			HFC_DBGPRT("%s - FIVE-FX LPAR support \n", __FUNCTION__);
			pp->lpar_support = 1;
		}
		else {
			HFC_DBGPRT("%s - FIVE-FX LPAR not support \n", __FUNCTION__);
		}
	}
	/* ------ FCLNX-GPL-FX-556 end */
...
int hfc_fx_check_linkresp_param(struct port_info *pp, struct core_info *core){
	...
	if(hfc_fx_read_val( mbox->mb_resp.type.linkini.position_map[0] ) & HFC_FX_FABRIC_VALID){
		pp->switch_exist = HFC_SWITCH_EXIST;
	}else{
		pp->switch_exist = 0;
	}
	
	switch(hfc_fx_read_val( mbox->mb_resp.type.linkini.trans_rate )){	/* FCLNX-GPL-FX-488 */
		case	HFC_FX_TRUNKN	: case	HFC_FX_1GBPS	:  
		case	HFC_FX_10GBPS	: case	HFC_FX_40GBPS	:
			HFC_DBGPRT("hfcldd%d: hfc_fx_check_linkresp_param - trans_rate0 = %llx\n", pp->dev_minor, hfc_fx_read_val( mbox->mb_resp.type.linkini.trans_rate ));
			break;
	...
	switch(hfc_fx_read_val( mbox->mb_resp.type.linkini.link_con_type )){	/* FCLNX-GPL-FX-488 */
		case	HFC_FX_CTUNKN	:
			HFC_DBGPRT("hfcldd%d: hfc_fx_check_linkresp_param - con_type0 = %llx\n", pp->dev_minor, hfc_fx_read_val( mbox->mb_resp.type.linkini.link_con_type ));
			break;
		case	HFC_FX_PT2PT	:
		case	HFC_FX_SWITCH	:
		...
		case	HFC_FX_MULTI_ALPA	:
			...
			if((!hfc_fx_read_val( mbox->mb_resp.type.linkini.alpa_count ))
			||(!hfc_fx_read_val( mbox->mb_resp.type.linkini.position_map[0] ))){
				retry_linkini = TRUE;
			}
			break;
		case	HFC_FX_F_PORT	:
...
//	uchar				flogi_config_flag=0;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_flogi_resp - 3 \n");
		mbox = core->mig_mb;
		fw_init_p = core->mig_fw_init_p;
	}else{
		mbox = core->mb;
	...
	case HFC_MB_FATAL :
		...
		if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
			...
			/* FCLNX-GPL-FX-517 >>> */
			if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000003);
			}
			/* >>> FCLNX-GPL-FX-517 */
			return;
		}		/* FCLNX-GPL-FX-058 */
		...
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
			if (core->mb_results == 0x02d30000){
				...
				hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
				unlock_fx_mailbox(pp);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_FLOGI | 0x00000005));	/* FCLNX-GPL-FX-517 */
				return;
			}
		}
		/* FCLNX-GPL-FX-502 End */
	...
	if (mb_resp_status != 0) {	
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_FLOGI | 0x00000002));	/* FCLNX-GPL-FX-517 */
	}
	else{
		...
		fw_init_p->fw_iocinfo.fabric_param = 
			(pp->flogi_rsp_param & mbox->mb_init.type.flogi.flogi_param);	/* FCLNX-GPL-FX-488 */
		...
		if( pp->flogi_config_flag & HFC_FL_PID_VALID ){
			...
			if(HFC_FX_MMODE_CHECK_SHADOW(pp)
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
				HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_flogi_resp - 5 \n");
				pp->mig_scsi_id=0;
				pp->mig_scsi_id = ( (uint64_t) hfc_fx_read_val( mbox->mb_resp.type.flogi.assign_portid[0] ) << 16 ) +
			...
			fw_init_p->fw_iocinfo.self_port_id[0] = mbox->mb_resp.type.flogi.assign_portid[0];
			fw_init_p->fw_iocinfo.self_port_id[1] = mbox->mb_resp.type.flogi.assign_portid[1];
			fw_init_p->fw_iocinfo.self_port_id[2] = mbox->mb_resp.type.flogi.assign_portid[2];
			fw_init_p->fw_iocinfo.configure_flag |= HFC_FX_PID_VALID;
		}
		...
		if( pp->flogi_config_flag & HFC_FL_FABRIC_EXIST ){
			...
			if(pp ->connect_type != HFC_FX_AL){
				...
				fw_init_p->fw_iocinfo.connect_type = HFC_FX_SWITCH;	/* FCLNX-GPL-FX-171,488  */
			}
			
			/* FCLNX-GPL-FX-488 >>> */
			fw_init_p->fw_iocinfo.configure_flag |= HFC_FX_FABRIC_VALID;
			fw_init_p->fw_iocinfo.configure_flag &= ~HFC_FX_P2P_PID_VALID;
			/* >>> FCLNX-GPL-FX-488 */
			...
			else if( mbox -> mb_init.type.flogi.flogi_param & FLOGI_PARAM_VF ){
				hfc_fx_hand2_trace(
...
//								pp->mb_timer[ HFC_MBTIME_ADD_PORTID ].delay ,FALSE);
			...
			if( fw_init_p->fw_iocinfo.connect_type == HFC_FX_AL ){
...
//			node_name);
	...
	switch (rc_passthrouh)
	{
		...
		case HFC_MB_FATAL :
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000004);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_PLOGI | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
	...
	if( mb_resp_status != 0 )
	{
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_FLOGI | 0x00000003));	/* FCLNX-GPL-FX-517 */
	}
	else{
		/* PLOGI succeeded */
...
//	int                         scsito = 0;
	...
	if (!(test_bit(HFC_TS_WAIT_PLOGI, (ulong *)&target->status))) {
		...
		clear_bit(HFC_CS_WAIT_MAILBOX, (ulong *)&core->status);
		clear_bit(HFC_CS_MB_RETRY_DELAY, (ulong *)&core->status);	/* FCLNX-GPL-FX-610 */
		...
		hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
		hfc_fx_w_stop(pp, core, HFC_FX_MB_RETRY_DELAY_TMR);			/* FCLNX-GPL-FX-610 */
	...
	switch (rc_passthrouh)
	{
		...
		case HFC_MB_FATAL :
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000005);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_PLOGI | 0x00000006));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
...
#pragma GCC diagnostic ignored "-Wimplicit-fallthrough"
	...
	/* WWN is mismatched */
		...
		/* FCLNX-GPL-FX-488 >>> */
		if ((HFC_FX_MMODE_CHECK_SHARED(pp))&&(!HFC_FX_MMODE_CHECK_SHADOW(pp))) {	
			if(pp->hg_migration_enable == HFC_FX_ENABLE_MIGRATION){
				hfc_fx_mlpf_clear_target_listinfo(pp, target->target_id);
			}
		}
		/* >>> FCLNX-GPL-FX-488 */
		clear_bit(HFC_TF_WWN_VALID, (ulong *)&target->flags);	/* FCLNX-GPL-FX-493 */
		if (HFC_FX_MQ_VALID(pp))
			hfc_fx_mq_change_target_info(pp, target);
	...
	case SCS_CANCEL_RESP :
		...
		if(test_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target->status)){	/* FCLNX-GPL-FX-112 Start */
			...
			if((!(pp->flogi_config_flag & HFC_FL_FABRIC_EXIST) ) && (pp->connect_type == HFC_FX_PT2PT)){
				...
				if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
				 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_FLOGI | 0x00000004));	/* FCLNX-GPL-FX-517 */
				return;																
			}
		}
	...
	case 0 :
		...
		if( test_bit(HFC_PS_WAIT_INITIALIZE, (ulong *)&pp->status) ) {	/* FCLNX-GPL-FX-066 */
			if(( pp->connect_type == HFC_FX_PT2PT )&&(!(pp->flogi_config_flag & HFC_FL_FABRIC_EXIST))){
				...
				/* Set p2p_tgt_port_id of Init_Table */
				/* FCLNX-GPL-FX-488 >>> */
				fw_init_p->fw_iocinfo.p2p_tgt_port_id[0] = (uchar)hfc_fx_read_val( mbox->mb_resp.type.plogi.recv_s_id[0] );
				fw_init_p->fw_iocinfo.p2p_tgt_port_id[1] = (uchar)hfc_fx_read_val( mbox->mb_resp.type.plogi.recv_s_id[1] );
				fw_init_p->fw_iocinfo.p2p_tgt_port_id[2] = (uchar)hfc_fx_read_val( mbox->mb_resp.type.plogi.recv_s_id[2] );
				/* >>> FCLNX-GPL-FX-488 */
				
				/* Set self_port_id of Init_Table */
				/* FCLNX-GPL-FX-488 >>> */
				fw_init_p->fw_iocinfo.self_port_id[0] = (uchar)hfc_fx_read_val( mbox->mb_resp.type.plogi.recv_d_id[0] );
				fw_init_p->fw_iocinfo.self_port_id[1] = (uchar)hfc_fx_read_val( mbox->mb_resp.type.plogi.recv_d_id[1] );
				fw_init_p->fw_iocinfo.self_port_id[2] = (uchar)hfc_fx_read_val( mbox->mb_resp.type.plogi.recv_d_id[2] );
				/* >>> FCLNX-GPL-FX-488 */
		
				/* Set configure_flag of Init_Table  */
				fw_init_p->fw_iocinfo.configure_flag = ( HFC_FL_PID_VALID | HFC_FL_P2P_PID_VALID );
...
//				hfc_fx_hw_chk_stop(pp);		/* TBD */
		...
		(!hfc_fx_mlpf_check_state_port(pp,HFC_HG_LPRSTATUS_LINKDOWN , HFC_CHECK_LPAR_STATE))){
			...
			set_bit(HFC_PS_CONNECTED, (ulong *)&pp->status);
			hfc_fx_hand2_trace(
				HFC_FX_TRC_SHADOWUPRSP, 0x05, pp, core->rp, core, NULL, NULL,
				(uint64_t)rc_passthrouh, 0, 0 );
		}
...
//	ushort				payload_length=0;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_prli_resp - 3 \n");
		mbox = core->mig_mb;
		pyload = pp->mig_payload_vsaddr;	/* TBD */
	}else{
		mbox = core->mb;
	...
	{/* When target_info_fx is found */
		...
		case HFC_MB_FATAL :
			...
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000006);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV | (HFC_SNDRCV_PRLI << 8) | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
		...
		case HFC_MBPASS_SUCCESS :
			...
			/* FCLNX-GPL-FX-681 >>> */
			type_code = (uchar)hfc_fx_read_val( pyload->receive_payload.type.prli.data1[4]) ;
			if (type_code != 0x08) {	/* Invalid Type Code */
				mb_resp_status =  SCS_NO_DEV_RESP;
				break;
			}
			...
			if ((service_param & 0x0f) != 0x01) {	/* Invalid Response Code */
				mb_resp_status =  SCS_NO_DEV_RESP;
				break;
			}
			
			prli_resp = (ushort)hfc_fx_read_val(pyload->receive_payload.type.prli.prli_param_resp);
			if (!(prli_resp & 0x0010)) {	/* Not TARGET FUNCTION */
				mb_resp_status =  SCS_NO_DEV_RESP;
			}
			/* FCLNX-GPL-FX-681 <<< */
			break;
...
//	structdump( 0xee, (uchar *)core->payload, 4096 );
	...
	case 0 :
		...
		if(!test_bit(HFC_TF_WWN_VALID, (ulong *)&target->flags)){
			...
			if ((HFC_FX_MMODE_CHECK_SHARED(pp))&&(!HFC_FX_MMODE_CHECK_SHADOW(pp))) {	
				if(pp->hg_migration_enable == HFC_FX_ENABLE_MIGRATION){
					hfc_fx_mlpf_setup_target_listinfo(pp, target->target_id);
				}
			}
		}
		/* >>> FCLNX-GPL-FX-488 */
		...
		if(test_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target->status)){	/* FCLNX-GPL-FX-112 Start */
			...
			while( dev != NULL){
				dev->lustat = 0x00;
				memset(dev->dev_core_stat.all, 0, sizeof(uint)*HFC_FX_MAX_MQ_NUM);	/* FCLNX-GPL-FX-014 *//* FCLNX-GPL-FX-073 *//* FCLNX-GPL-FX-556 */
				dev = dev->next;
			}
		}																	/* FCLNX-GPL-FX-112 End */
...
//	ushort				payload_length=0;
	...
	switch (rc_passthrouh)
	{
		...
		case HFC_MB_FATAL :
			...
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000007);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV | (HFC_SNDRCV_SCR << 8) | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
	...
	if( mb_resp_status != 0 ){
		...
		if ( HFC_FX_MMODE_CHECK_SHADOW(pp) && 
		 test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
			HFC_DBGPRT("hfcldd%d :hfc_fx_scr_resp8 \n", pp->dev_minor);
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV | (HFC_SNDRCV_SCR << 8) | 0x00000002));	/* FCLNX-GPL-FX-517 */
		}
	}
	else{
...
//	ushort				payload_length=0;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_logo_resp - 3 \n");
		mbox = core->mig_mb;
	}else{
		mbox = core->mb;
	}
	...
	case HFC_MB_FATAL :
		...
		&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
			if (core->mb_results == 0x02d30000){
				...
				hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
				unlock_fx_mailbox(pp);
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV | (HFC_SNDRCV_LOGO << 8) | 0x00000005));	/* FCLNX-GPL-FX-517 */
				return;
			}
		}
		/* FCLNX-GPL-FX-502 End */
	...
	switch (rc_passthrouh)
	{
		...
		case HFC_MB_FATAL :
			...
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000008);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV | (HFC_SNDRCV_RFT_ID << 8) | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
...
//	ushort				payload_length=0;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_rff_id_resp - 3 \n");
		mbox = core->mig_mb;
		pyload = pp->mig_payload_vsaddr;
	}else{
		mbox = core->mb;
	...
	switch (rc_passthrouh)
	{
		...
		case HFC_MB_FATAL :
			...
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000009);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV | (HFC_SNDRCV_RFF_ID << 8) | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
...
//	structdump( 0xee, (uchar *)core->payload, 4096 );
	...
	if ( !HFC_FX_MMODE_CHECK_SHADOW(pp)
	 || test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )) {
		hfc_fx_wwnverify_linkup(pp, NULL, core, 0, 0);	/* FCLNX-GPL-FX-615 */
	}
	else {
		if(pp->initialize != 0){
			hfc_fx_wake_up(&pp->init_event,&pp->int_a_poll);
...
//	ushort				payload_length=0;
	...
	if(HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_gpn_ft_resp - 3 \n");
		mbox = core->mig_mb;
		pyload = pp->mig_payload_vsaddr;	/* TBD */
	}else{
		mbox = core->mb;
	...
	switch (rc_passthrouh)
	{
		...
		case HFC_MB_FATAL :
			...
			if(((rc_passthrouh == HFC_MBPASS_RETRY_OVER)	||
				...
				/* FCLNX-GPL-FX-517 >>> */
				if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x0000000a);
				}
				/* >>> FCLNX-GPL-FX-517 */
				return;
			}	/* FCLNX-GPL-FX-058 */
			...
			&& (test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))){	/* FCLNX-GPL-FX-502 */
				if (core->mb_results == 0x02d30000){
					...
					hfc_fx_w_stop(pp, NULL, HFC_FX_MB_RETRY_TMR);
					unlock_fx_mailbox(pp);
					hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_SNDRCV_GIDFT | (HFC_SNDRCV_GPN_FT << 8) | 0x00000005));	/* FCLNX-GPL-FX-517 */
					return;
				}
			}
			/* FCLNX-GPL-FX-502 End */
...
void hfc_fx_mb_intreq(struct port_info *pp, struct core_info *core, struct region_info *rp)
{
	...
	if( HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (core->mig_mb != NULL)
	&& (pp->mig_mbint_rid != 0x00) && (pp->mig_mbint_rid != 0x80)){
		mbox = core->mig_mb;
	}else{
		mbox = core->mb;
	}
	...
	if( mb_int0 == 0xa0 )
	{
		...
		if( HFC_FX_MMODE_CHECK_SHADOW(pp)
		&& (pp->mig_mbint_rid != 0x00) && (pp->mig_mbint_rid != 0x80)){
			mb_resp = hfc_fx_frame_a_data(mbox, pp->pkg.type, (ushort)mb_code, (uchar)pp->mig_mbint_rid);
		}else{
...
//		hfc_fx_write_reg_ext( pp, ( uint )hfc_framea_of_core[core->core_no], ( char )0x4, ( int )mb_resp );
		hfc_fx_write_reg_core(pp, core->core_no, (uint)HFC_IOSPACE_FRAMEA,
							(char)0x4, (int)mb_resp, HFC_FX_CORE_OFFSET40);
		/* FCLNX-GPL-FX-603 >>> */
		if(pp->log_after_receive_frame == HFC_FX_LINKUP_LOG){
			hfc_fx_errlog(pp,core,NULL,NULL,HFC_ERRLOG_TYPE_MBINT,ERRID_HFCP_EVNT2,0x1b,core->logdata,16) ;
			pp->log_after_receive_frame = 0;
		}else if(pp->log_after_receive_frame == HFC_FX_RECEIVE_FLOGI_LOG){
			hfc_fx_errlog(pp,core,NULL,NULL,HFC_ERRLOG_TYPE_NONE,ERRID_HFCP_EVNT1,0x15,core->logdata,16) ;
			pp->log_after_receive_frame = 0;
		}
		/* >>> FCLNX-GPL-FX-603 */
	}
...
//			memcpy(&core->logdata[0], &(mbox->mb_intreq.type.resvx[16]), 16);
...
void hfc_fx_comp_online_update_intreq(struct port_info *pp, struct core_info *core)
{
	...
	&& (core->mig_mb != NULL)
	&& (pp->mig_mbint_rid != 0x00) && (pp->mig_mbint_rid != 0x80)){
		HFC_DBGPRT( "hfcldd :  hfc_fx_top 0; hfc_fx_comp_online_update_intreq - 3 \n");
		mbox = core->mig_mb;
	}else{
		mbox = core->mb;
	}
	...
	uchar	mig)
{
	...
	if( HFC_FX_MMODE_CHECK_SHADOW(pp) && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 )){
		if(!test_bit(HFC_MAILBOX_BUSY, (ulong *)&( pp->region_arg[pp->rid]->mb_lock ) ))
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBINT, 0x00000001);
	}
	/* >>> FCLNX-GPL-FX-517 */
...
//						hfc_manage_info.npubp->hfc_fx_all_clear_dev_info_fx(pp, dev);					/* FCLNX-GPL-0343	*/
	...
	hfc_fx_copy_master_to_slave( pp, core );	/* FCLNX-GPL-FX-18 */
	hfc_fx_issue_change_state(pp, HFC_FX_CHANGE_STATE_LINKDOWN );	/* FCLNX-GPL-FX-622 */
	memset(core->logdata,0,16);
	hfc_fx_hand2_trace(
	...
	if( !test_bit(HFC_DIAG_END , (ulong *)&pp->status) )
	{
		if( !test_bit(HFC_PS_WAIT_INITIALIZE, (ulong *)&pp->status) ){
			...
			if(pp->connect_type != HFC_FX_F_PORT){
				...
				pp->log_after_receive_frame = HFC_FX_LINKUP_LOG;
			}
		}
		else{
	...
	switch( core->fw_init_p->fw_iocinfo.connect_type ){
		...
		case HFC_FX_MULTI_ALPA:
			...
			if( flag & HFC_FX_ALPA_VALID ){
				/* FCLNX-GPL-FX-488 >>> */
				core->fw_init_p->fw_iocinfo.assign_alpa = hfc_fx_read_val(mbox->mb_intreq.type.linkup.assign_alpa );
				pp->scsi_id = (uint64_t)hfc_fx_read_val(mbox->mb_intreq.type.linkup.assign_alpa );

				core->fw_init_p->fw_iocinfo.self_port_id[0] = 0;
				core->fw_init_p->fw_iocinfo.self_port_id[1] = 0;
				core->fw_init_p->fw_iocinfo.self_port_id[2] = (uchar)hfc_fx_read_val(mbox->mb_intreq.type.linkup.assign_alpa );
			...
			if( core->fw_init_p->fw_iocinfo.connect_type == HFC_MULTI_ALPA ){
				/* FCLNX-GPL-FX-488 >>> */
				count = mbox->mb_intreq.type.linkup.acquired_alpa[0];	/* FCLNX-GPL-FX-417 */
				if( count > 0){
					core->fw_init_p->active_alpa[0] = mbox->mb_intreq.type.linkup.acquired_alpa[0];	/* FCLNX-GPL-FX-417 */
					for (i=1;i<=count; i++) {
						core->fw_init_p->active_alpa[i] = mbox->mb_intreq.type.linkup.acquired_alpa[i];	/* FCLNX-GPL-FX-417 */
					}
				}
				/* >>> FCLNX-GPL-FX-488 */
			}
...
//					set_bit(HFC_COMP_RECEIVE_FLOGI, (ulong *)&pp->status1);
		...
		||  (pp->mck_linkup == HFC_LINKUP_MCK)){
			pp->log_after_receive_frame = HFC_FX_RECEIVE_FLOGI_LOG;	/* FCLNX-GPL-FX-603 */
		}
		hfc_fx_change_portstat_linkup(pp, core);
	...
	struct mailbox_fx       *mbox)
{
	...
	/*==== Check Multi AL-PA ==================================================*/
	if ((pp->connect_type == HFC_FX_MULTI_ALPA) || (pp->connect_type == HFC_FX_AL)){ /* FCLNX-GPL-FX-606 */
		/*---- Reject, cause Multi AL-PA(Request no-reply)--*/
		HFC_DBGPRT("hfcldd%d hfc_fx_receive_plogi_intreq reject case Multi AL-PA\n",pp->dev_minor);
		reject_flag = 1;
		Reason		= HFC_MBX_LS_RC_LOGERR;
		Reason_ex	= HFC_MBX_LS_RE_NOADD;
	}
	...
	for (tid=0; tid < MAX_TARGET_PROBE; tid++) {
		 ...
		 *     Check Multi AL-PA
		 *====================================================================*/
		if ((pp->connect_type == HFC_FX_MULTI_ALPA)||(pp->connect_type == HFC_FX_AL)) { break; } /* FCLNX-GPL-FX-606 */
...
//						target->login_seq_retry_cnt--;	/* FCLNX-GPL-FX-476 */
	...
	/* Reject responce send case */
		...
		hfc_fx_write_val( mbox -> mb_intresp.payload[6], Reason_ex);
																	...
																	/* @MLPF STR */
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) &&
		((pp->mig_mbint_rid == 0x00) || (pp->mig_mbint_rid == 0x80)) ){ /* FCLNX-GPL-FX-528 */
...
//	uchar					rsp				= FALSE;
	...
	/* FCLNX-GPL-FX-488 >>> */
	if( HFC_FX_MMODE_CHECK_SHADOW(pp)
	&& (pp -> mig_rcvpayload_gpaddr != 0 )
	&& (pp->mig_mbint_rid != 0x00) && (pp->mig_mbint_rid != 0x80)){
		HFC_DBGPRT("hfc_fx_receive_frame_intreq - 1\n");
		hfc_fx_write_val( mbox -> mb_intresp.self_wwpn, (pp -> mig_rcvpayload_gpaddr));
	}else{
	...
	/*==== Check Distnation Port ID ==========================================*/
	if ((Rdid != pp->scsi_id) && (!HFC_FX_MMODE_CHECK_SHARED(pp))){	/* FCLNX-GPL-FX-516 */
		/*---- Reject, cause FC-PH Header Error(Request no-reply) ------------*/
		reject_flag = 1;
		Reason		= HFC_MBX_LS_RC_LOGERR;
		Reason_ex	= HFC_MBX_LS_RE_NOADD;
	...
	&& (pp->mig_mbint_rid != 0x00) && (pp->mig_mbint_rid != 0x80)){
		HFC_DBGPRT("hfc_fx_receive_frame_intreq - 2 %x \n", els_command);
		rcvfrm_pyload = pp->mig_rcvpayload_vsaddr;
	}else{
...
//				if (!reject_flag) {
//					res_payload_len = (vmk_uint16)HFC_PRLI_RLENGTH;
//				} else {
//					res_payload_len = (vmk_uint16)HFC_LS_RJT_LENGTH;
//				}
//				break;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,17,0) /* FCLNX-GPL-FX-699 --> 717 */
				pp->pm_pkt_pool[hfcp->pm_pkt_no].tsc_iodone = rdtsc_ordered();
#else
...
/* FCLNX-GPL-FX-488 >>> */
/*
 * Function:    hfc_fx_xrb_resp_shadow
 *
 * Purpose:     This routine deals with XRB Response interrupt
 *
 * Arguments:
	...
	struct hfc_pkt_fx		*(*wait_iodone_hfcp))
{
	...
	if(pp->mig_xrb_p == NULL) return(rtn);

	/* Check xrbs while xrb valid bit is enabled */
	while(pp->mig_xrb_p[core->drv_next_xrb].xcrbchk.valid & HFC_XRB_VALID)
	{
		HFC_DBGPRT("hfc_fx_xrb_resp_shadow \n");
		/* Set xrb_no */
		xrb_no = core->drv_next_xrb ;
		memcpy(&xrb, &pp->mig_xrb_p[xrb_no], sizeof(xrb));
		...
		if (pp == NULL) {
			/* Dequeue XRB */
			pp->mig_xrb_p[core->drv_next_xrb].xcrbchk.valid &= ~HFC_XRB_VALID ;     /* Reset xrb valid flag */
			xrb_cnt++ ;
			core->drv_next_xrb++ ;
			if( core->drv_next_xrb >= HFC_MIG_XOB_MAX)
				core->drv_next_xrb = 0 ;	/* FCLNX-GPL-FX-332 */
		...
		if( (xrb.xcrb.err_code[0] == 0xd3 )
		&&  (xrb.xcrb.err_code[1] == 0x00 )
		&&  (xrb.xcrb.err_code[2] == 0x00 ) ){
			hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_SCSIERR, 0x00000001);
			break;
		}
		...
		{	/* If xrb valid flag is zero */
			...
			/* Dequeue XRB */
			pp->mig_xrb_p[core->drv_next_xrb].xcrbchk.valid &= ~HFC_XRB_VALID ;     /* Reset xrb valid flag */
			pp->mig_xrb_p[core->drv_next_xrb].xcrb.skip &= ~HFC_XRB_SKIP;
			xrb_cnt++ ;
			core->drv_next_xrb++ ;
			if( core->drv_next_xrb >= HFC_MIG_XOB_MAX)
				core->drv_next_xrb = 0 ;
		...
		if(pp->mig_xrb_p == NULL){
			HFC_DBGPRT("hfcldd%d : hfc_fx_xrb_resp - hfcp->cmd_pkt->result",pp->dev_minor);
			break;
		}
		pp->mig_xrb_p[core->drv_next_xrb].xcrbchk.valid &= ~HFC_XRB_VALID ;     /* Reset xrb valid flag */
		xrb_cnt++ ;
	...
	struct xrb_fx			*xrb)
{
	...
	uint					result;
	uint					cmd_flags;
	uint					zero[HFC_FX_MAX_MQ_NUM] = {0,0,0,0,0,0,0,0};
	...
	/* FCLNX-GPL-FX-556 >>> */
	if (HFC_FX_MQ_VIRTUAL_PORT(pp)) {
		/* use physical target for status check */
...
//			hfc_fx_errlog(pp,core,target,hfcp,HFC_ERRLOG_TYPE_XRB,ERRID_HFCP_ERR6,0x20,core->logdata,16) ;
		...
		if (test_bit(CFLAG_CSCSI_TGT_WAIT_DMA, (ulong *)&hfcp->cmd_flags)) {
			...
			/* FCLNX-GPL-FX-556 start ------ */
			if (HFC_FX_MQ_VALID(pp)) {
				if(test_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target_wk->status)){
					hfc_fx_watchdog_enter(
						pp->pport,
						pp->pport->region_arg[0]->core_arg[core->core_no],
						target_wk,
						NULL,
						0,
						HFC_FX_TOTAL_TGTRST_TMR,
		...
		/* FCLNX-GPL-FX-556 >>> */
		set_bit(HFC_TF_FAIL_TARGET_RESET, (ulong *)&target_wk->flags);		/* FCLNX-GPL-FX-014 */
		clear_bit(HFC_TS_NEED_TARGET_RESET, (ulong *)&target_wk->status);	/* FCLNX-GPL-FX-085 */
		clear_bit(HFC_TS_WAIT_TARGET_RESET, (ulong *)&target_wk->status);	/* FCLNX-GPL-FX-085 */
		/* FCLNX-GPL-FX-556 <<< */
		...
		/* FCLNX-GPL-FX-556 >>> */
		if (hfc_fx_test_tgt_core_stat(pp, target) == 0){	/* FCLNX-GPL-FX-014 */
			HFC_DBGPRT("hfcldd : hfc_fx_link_chk - 10 \n");
			clear_bit(HFC_TS_WAIT_TARGET_RESET, (ulong *)&target_wk->status);
		}	/* FCLNX-GPL-FX-014 */
		/* FCLNX-GPL-FX-556 <<< */
...
//			hfc_fx_errlog(pp,core,target,hfcp,HFC_ERRLOG_TYPE_XRB,ERRID_HFCP_ERR6,0x21,core->logdata,16) ;
		...
		/* FCLNX-GPL-FX-014 Start */  /* FCLNX-GPL-FX-556 start  */
		if ( test_bit(CFLAG_ABORT, (ulong *)&hfcp->cmd_flags) 
		  || test_bit(CFLAG_LUN_RESET, (ulong *)&hfcp->cmd_flags)){
			clear_bit(HFC_DC_WAIT_LUN_RESET_OR_ABORT, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);
			HFC_DBGPRT("hfcldd : hfc_fx_link_chk - 12 core_no=%d, vport_id=%d\n", core->core_no, pp->vport_id);
		}
		if (test_bit(CFLAG_CSCSI_LU_WAIT_DMA, (ulong *)&hfcp->cmd_flags)){
			clear_bit(HFC_DC_WAIT_CSCSI_LU_WAIT_DMA, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);
			HFC_DBGPRT("hfcldd : hfc_fx_link_chk - 13 core_no=%d, vport_id=%d\n", core->core_no, pp->vport_id);
		}
		if (test_bit(CFLAG_CSCSI_LU_WITHOUT_DMA, (ulong *)&hfcp->cmd_flags)) {
			clear_bit(HFC_DC_WAIT_CSCSI_LU_WITHOUT_DMA, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);
			HFC_DBGPRT("hfcldd : hfc_fx_link_chk - 14 core_no=%d, vport_id=%d\n", core->core_no, pp->vport_id);
		}
		/* FCLNX-GPL-FX-014 End */ /* FCLNX-GPL-FX-556 end  */
			...
			if(hfc_fx_check_cmnd_timeout(pp,core_wk,target,&hfcp->lun_id)){	/* FCLNX-GPL-FX-014 */
				...
				if(pp->rt_err_enable){
					...
					if(pp->tgtrst_restrain){
						...
						/* FCLNX-GPL-FX-556 >>> */
						if( (test_bit(HFC_TS_WAIT_TARGET_RESET, (ulong *)&target_wk->status ) ) ||
						(test_bit(HFC_TS_NEED_TARGET_RESET, (ulong *)&target_wk->status ) ) )
							break ;
						/* FCLNX-GPL-FX-556 <<< */
						...
						set_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target_wk->status);
						...
						target->target_reset_cmnd = hfcp->cmd_pkt;
			 			...
			 			if(hfc_fx_issue_tgtrst_cscsi(pp, target, target->dev, (0x00000001 << CFLAG_CSCSI_TGT_WAIT_DMA))){
							clear_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target_wk->status);
							break;
						}
						/* FCLNX-GPL-FX-112 End */
		...
		if (memcmp(dev->dev_core_stat.all, zero, sizeof(uint)*HFC_FX_MAX_MQ_NUM) == 0){	/* FCLNX-GPL-FX-014 */ /*FCLNX-GPL-FX-556 */
			if (test_bit(CFLAG_LUN_RESET, (ulong *)&hfcp->cmd_flags) ) {
				clear_bit(HFC_DS_WAIT_LUN_RESET, (ulong *)&dev->lustat);
		...
		HFC_DBGPRT("hfcldd : hfc_fx_link_chk - 22 dev_core_stat.all=\n");
		HFC_DBGPRT("%08x%08x%08x%08x%08x%08x%08x%08x\n",
						dev->dev_core_stat.all[0],dev->dev_core_stat.all[1],dev->dev_core_stat.all[2],dev->dev_core_stat.all[3],
						dev->dev_core_stat.all[4],dev->dev_core_stat.all[5],dev->dev_core_stat.all[6],dev->dev_core_stat.all[7]);
		HFC_DBGPRT("%08x%08x%08x%08x%08x%08x%08x%08x\n",
						dev->dev_core_stat.all[8],dev->dev_core_stat.all[9],dev->dev_core_stat.all[10],dev->dev_core_stat.all[11],
						dev->dev_core_stat.all[12],dev->dev_core_stat.all[13],dev->dev_core_stat.all[14],dev->dev_core_stat.all[15]);
		HFC_DBGPRT("%08x%08x%08x%08x%08x%08x%08x%08x\n",
						dev->dev_core_stat.all[16],dev->dev_core_stat.all[17],dev->dev_core_stat.all[18],dev->dev_core_stat.all[19],
						dev->dev_core_stat.all[20],dev->dev_core_stat.all[21],dev->dev_core_stat.all[22],dev->dev_core_stat.all[23]);
		HFC_DBGPRT("%08x%08x%08x%08x%08x%08x%08x%08x\n",
						dev->dev_core_stat.all[24],dev->dev_core_stat.all[25],dev->dev_core_stat.all[26],dev->dev_core_stat.all[27],
						dev->dev_core_stat.all[28],dev->dev_core_stat.all[29],dev->dev_core_stat.all[30],dev->dev_core_stat.all[31]);			
		hfc_fx_set_cmnd_res(pp, core, Scmd, hfcp, result);
		if (!hfc_manage_info.hfcldd_mp_mod) {
			hfc_fx_iodone(pp, core, Scmd, hfcp);
		}
...
{	/* FCLNX-GPL-FX-332 */
	...
	{	/* Always Count up */ /* FCLNX-GPL-FX-552 */	
	...
	if (pp->hg_cca_p) { /* FCLNX-GPL-FX-552 */		
	...
	struct xrb_fx			*xrb)
{
	...
	{/* Response of target reset */
		...
		if (test_bit(CFLAG_CSCSI_TGT_WAIT_DMA, (ulong *)&hfcp->cmd_flags)) {
			clear_bit(HFC_TC_WAIT_CSCSI_TGT_WAIT_DMA, (ulong *)&target->tgt_core_stat.core[ core->core_no ]);
			if(test_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target_wk->status)){	/* FCLNX-GPL-FX-112 */
		...
		/* FCLNX-GPL-FX-014 Start */
		if (hfc_fx_test_tgt_core_stat(pp, target) == 0){ /* FCLNX-GPL-FX-556 */
			if(!test_bit(HFC_TF_FAIL_TARGET_RESET, (ulong *)&target_wk->flags)){
				if (need_next_reset) {
					...
					if(need_next_reset == 1){									/* FCLNX-GPL-FX-112 */
						if (HFC_FX_MQ_VALID(pp)) {
							hfc_fx_issue_tgtrst_cscsi(
								...
								pp->pport->target_arg[target->pseq],
								dev,
								((0x00000001 << CFLAG_TARGET_RESET) | (0x00000001 << CFLAG_CSCSI_TGT_WAIT_DMA)));
						}	
						else{
							hfc_fx_issue_tgtrst_cscsi(
						...
						set_bit(HFC_TS_NEED_PLOGI, (ulong *)&target_wk->status );	/* FCLNX-GPL-FX-112 */
						if (HFC_FX_MQ_VALID(pp)) {
							atomic_set(&pp->pport->check_mbreq, 1);			/* FCLNX-GPL-FX-112 */
						}else{
							atomic_set(&pp->check_mbreq, 1);
						}
					}
				}else { /* Target Reset succeeded */
					HFC_DBGPRT("hfcldd : hfc_fx_task_mgm_chk - Target Reset succeeded 1\n");
					clear_bit(HFC_TS_WAIT_TARGET_RESET, (ulong *)&target_wk->status);
					/* Cancellation of XOB and response waiting queue */
					for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
						if ((core_wk = pp->region_arg[pp->rid]->core_arg[i]) == NULL)
						...
						/* FCLNX-GPL-FX-556 end  */
						clear_bit(HFC_TS_CANCEL_SCSI_TARGET, (ulong *)&target_wk->status);							/* FCLNX-GPL-FX-190 */
						hfc_fx_notify_tout(pp, core_wk, target, 0, HFC_FLASH_TARGET);	/* FCLNX-GPL-596 */
						hfc_fx_cancel_weque(pp, core_wk, target, 0, NULL, SCS_CMD_RESET, HFC_CSCSI_ERROR, HFC_FLASH_TARGET);	/* FCLNX-GPL-FX-112 */
						wk_dev = target->dev;
						while(wk_dev != NULL) {			/* FCLNX-GPL-FX-190 Start */
							wk_dev->lustat = 0x00;
							/* FCLNX-GPL-FX-556 start */
							memset(dev->dev_core_stat.all, 0, sizeof(uint)*HFC_FX_MAX_MQ_NUM);		
							if (HFC_FX_MQ_VALID(pp)) {
								hfc_fx_mp_watchdog_enter(
									pp->pport,
									pp->pport->region_arg[0]->core_arg[core_wk->core_no],
							...
							HFC_DBGPRT("hfcldd : hfc_fx_task_mgm_chk - Target Reset succeeded 2\n");
							/* FCLNX-GPL-FX-556 end */
							wk_dev = wk_dev->next;
						}								/* FCLNX-GPL-FX-190 End */
					}
		...
		if( pp->scsi_reset_delay ){	/* FCLNX-GPL-FX-112 */
			...
			if (HFC_FX_MQ_VALID(pp)) {
				...
				rtn = hfc_fx_watchdog_enter(
					...
					0,
					FALSE);
				if( !rtn ) set_bit(HFC_TS_SCSI_DELAY, (ulong *)&target_wk->status);
			}
			else{	
	...
	{	/* Response of Abort Task Set or LUN Reset --*/
		...
		if ( test_bit(CFLAG_ABORT, (ulong *)&hfcp->cmd_flags) 
		  || test_bit(CFLAG_LUN_RESET, (ulong *)&hfcp->cmd_flags)){
			clear_bit(HFC_DC_WAIT_LUN_RESET_OR_ABORT, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);
			HFC_DBGPRT("hfcldd : hfc_fx_task_mgm_chk - Response of Abort Task Set or LUN Reset 1 core_no=%d, vport_id=%d\n",
									core->core_no, pp->vport_id);
		}
		if (test_bit(CFLAG_CSCSI_LU_WAIT_DMA, (ulong *)&hfcp->cmd_flags)){
			clear_bit(HFC_DC_WAIT_CSCSI_LU_WAIT_DMA, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);
			HFC_DBGPRT("hfcndd : hfc_fx_task_mgm_chk - Response of Abort Task Set or LUN Reset 2 core_no=%d, vport_id=%d\n",
									core->core_no, pp->vport_id);
		}
		if (test_bit(CFLAG_CSCSI_LU_WITHOUT_DMA, (ulong *)&hfcp->cmd_flags)) {
			clear_bit(HFC_DC_WAIT_CSCSI_LU_WITHOUT_DMA, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);
			HFC_DBGPRT("hfcndd : hfc_fx_task_mgm_chk - Response of Abort Task Set or LUN Reset 3 core_no=%d, vport_id=%d\n",
									core->core_no, pp->vport_id);
			need_next_reset = 1;
		}
		/* FCLNX-GPL-FX-556 end */
		...
		/* FCLNX-GPL-FX-014 Start */
		if (memcmp(dev->dev_core_stat.all, zero, sizeof(uint)*HFC_FX_MAX_MQ_NUM) == 0){	/* FCLNX-GPL-FX-556 */
					...
					if (HFC_FX_MQ_VALID(pp)) {
						if(test_bit(HFC_DS_NEED_LUN_RESET, (ulong *)&dev->lustat)){
							hfc_fx_issue_devrst_cscsi(
									...
									pp->pport->target_arg[target->pseq],
									dev,
									(((0x00000001 << CFLAG_LUN_RESET) | (0x00000001 << CFLAG_CSCSI_LU_WAIT_DMA))));
						}else if(test_bit(HFC_DS_NEED_ABORT, (ulong *)&dev->lustat)){
							hfc_fx_issue_devrst_cscsi(
									pp->pport,
									pp->pport->target_arg[target->pseq],
									dev,
									(((0x00000001 << CFLAG_ABORT) | (0x00000001 << CFLAG_CSCSI_LU_WAIT_DMA))));
						}
					}
					else{
						if(test_bit(HFC_DS_NEED_LUN_RESET, (ulong *)&dev->lustat)){
				...
				if( pp->lun_reset_delay ){
					...
					if (HFC_FX_MQ_VALID(pp)) {
						hfc_fx_mp_watchdog_enter(
							...
							dev,
							hfcp->lun_id,
							HFC_FX_DELAY_TMR_DEV,
							0,
							TRUE);
						rtn = hfc_fx_mp_watchdog_enter(
							pp->pport,
							pp->pport->region_arg[0]->core_arg[core->core_no],
							target_wk,
							hfcp,
		...
		if( dev != NULL ){
			HFC_DBGPRT("hfcldd : hfc_fx_task_mgm_chk - dev_core_stat.all=\n");
			HFC_DBGPRT("%08x%08x%08x%08x%08x%08x%08x%08x\n",
							dev->dev_core_stat.all[0],dev->dev_core_stat.all[1],dev->dev_core_stat.all[2],dev->dev_core_stat.all[3],
							dev->dev_core_stat.all[4],dev->dev_core_stat.all[5],dev->dev_core_stat.all[6],dev->dev_core_stat.all[7]);
			HFC_DBGPRT("%08x%08x%08x%08x%08x%08x%08x%08x\n",
	...
	uint                    rc_passthrouh)
{
	...
	uchar                   type=0;
	struct mailbox_fx       *mbox = NULL;	/* FCLNX-GPL-FX-517 */
...
//		if (hfc_fx_issue_linkini(pp, core) != 0) {	/* TBD */
		if (hfc_fx_issue_linkini(pp) != 0) {		/* TBD */
			...
			if ( HFC_FX_MMODE_CHECK_SHADOW(pp)
			 && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_LINKINIT | 0x00000001));	/* FCLNX-GPL-FX-517 */
			return;
		}
	}
		...
		if (hfc_fx_issue_flogi(pp) != 0) {
			...
			if ( HFC_FX_MMODE_CHECK_SHADOW(pp)
			 && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, (HFC_MBCMD_FLOGI | 0x00000001));	/* FCLNX-GPL-FX-517 */
			return;
		}
	}
...
//		}
		...
		if (hfc_fx_issue_frmsndrcv(pp, NULL, 0, HFC_SNDRCV_SCR) != 0) {
			...
			if ( HFC_FX_MMODE_CHECK_SHADOW(pp)
			 && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, HFC_MBCMD_SNDRCV & (HFC_SNDRCV_SCR << 8) & 0x00000001);
			return;
		}
	}
		...
		if (hfc_fx_issue_plogi(pp, NULL) != 0) {
			...
			if ( HFC_FX_MMODE_CHECK_SHADOW(pp)
			 && test_bit( HFC_PD_MLPF_MIG_LOGINPROC, (ulong *)&pp->status_detail2 ))
				hfc_fx_mlpf_migration_login_complete(pp, HFCFX_MIG_TL_RESULT_MBERR, HFC_MBCMD_PLOGI & 0x00000001);
			return;
		}
	}
}
