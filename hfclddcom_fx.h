...
#define HFC_MBREQ_CONTYPE_FPORT	0xF0			/* F_PORT				*/
				uchar	pidctl ;				/* +B PIDCTL			*//* FCLNX-GPL-FX-594 */
#define HFC_MBREQ_NO_REG_PORTID	0x80			/* Don't Notify PortID	*//* FCLNX-GPL-FX-594 */
				uchar	resv1[8] ;				/* +C - +13				*/
				eth_hdr16b_t eth_hdr ;			/* +14 Ethernet Header	*/
				uchar	resv2[4] ;				/* +24 - +27			*/
...
typedef struct hfcfx_hvm_mig_fwifaddr_naddr {
	uint64_t	xob;								/* nXOB[C*0x0010 + 0x0010]*/
	uint64_t	xrb;								/* nXRB[C*0x0010 + 0x0018]*/
} mig_naddr;

struct mig_fwifaddr_list {
	/*---- FW Interface Address List Header Block ----------------------------*/
	uint		rev;								/* Table Revision [0x0000]*/
#define HFCFX_MIG_FI_REV0		0x00000000	/* Rev 0                          */
	uint		tbl_size;							/* Table Size	  [0x0004]*/
#define HFCFX_MIG_FI_TSIZE		0x00001000	/* Size of FW Interface List      */
	uint64_t	port_id_addr;						/* Reserved       [0x0008]*/
	/*---- FW Interface Address List Core Block ------------------------------*/
	mig_naddr	next_addr[4];						/* nXOB and nXRB[C*0x0010 + 0x0010]*/
	uint64_t	init_addr[4];						/* INIT[C*0x0008 + 0x0050]*/
	uint64_t	mbox_addr[4];						/* MB  [C*0x0008 + 0x0070]*/
	/*---- FW Interface Address List XOB/XRB Queue Block ---------------------*/
	uint64_t	xob_addr[8];						/* XOB [N*0x0008 + 0x0090]*/
	uint64_t	xrb_addr[32];						/* XOB [N*0x0008 + 0x00D0]*/
	/*---- FW Interface Address List Soft Log Block --------------------------*/
	uint64_t	slog_addr[64];						/* SLog[N*0x0008 + 0x01D0]*/
	/*---- FW Interface Address List Receive Frame Payload Address of Shadow -*/
	uint64_t	rcv_payload_saddr;					/* Payload Address[0x03D0]*/
	/*---- FW Interface Address List Receive Frame Payload Address of Guest --*/
	uint64_t	rcv_payload_gaddr;					/* Payload Address[0x03D8]*/
	/*---- FW Interface Address List Seg Address		 ------------------------*/
	uint64_t	seg_spaddr[5];						/* SegI[N*0x0008 + 0x03E0]*/
	/*---- FW Interface Address List Login Delay -----------------------------*/
	ushort		login_delay;						/* Login Delay    [0x0408]*/
	/*---- FW Interface Address List Reserved Locaton ------------------------*/
	uchar		rsv1[2];							/* Reserved       [0x040A]*/
	uint		port_id;
	/*---- FW Interface Address List Receive Frame Payload Address of Shadow -*/
	uint64_t	payload_saddr;						/* Payload Address[0x0410]*/
	/*---- FW Interface Address List Receive Frame Payload Address of Guest --*/
	uint64_t	payload_gaddr;						/* Payload Address[0x0418]*/
	/*---- FW Interface Address List Receive Frame Payload Address of Guest --*/
	uint64_t	seg_gpaddr[5];						/* Payload Address[0x0420]*/
	/*---- FW Interface Address List Reserved Locaton ------------------------*/
	uchar		rsv2[3000];							/* Reserved       [0x0448]*/
};

#define HFCFX_MIG_LL_IDATAMAX	0x0018		/* Max Size of Identifier Data    */

struct mig_luid_info {
	ushort	id_type;							/*LU#n Identifier Type[0x0000]*/
	uchar	rsr[6];								/*Reserved Location   [0x0002]*/
	uchar	id_data[HFCFX_MIG_LL_IDATAMAX];		/*LU#n Identifier Data[0x0008]*/  
};

struct mig_targetinfo{
	uint64_t	pid_addr;		/* +000 - +007	*//* Physical Address for PORT_ID   */
	uint		dst_pid;		/* +008 - +00B	*//* Destnation PORT_ID             */
	uint		rsv0;			/* +00C - +00F	*//* PORT_ID Work                   */
	uint64_t	srcwwpn;		/* +010 - +017	*//* Source Target WWPN             *//* FCLNX-GPL-FX-498 */
	uint64_t	dstwwpn;		/* +018 - +01F	*//* Destnation  Target WWPN        *//* FCLNX-GPL-FX-498 */
	ushort		lunum;			/* +020 - +021  *//* LU Num                         */
	ushort		rsv1;			/* +022 - +023  *//* Reserved Location(0x0022-0003) */
	uint		list_size;		/* +024 - +027  *//* Size of LUID_LIST              */
	uint64_t	list_addr;		/* +028 - +02F  *//* Physical Address for LUID_LIST */
};

/* FCLNX-GPL-FX-583,595 >>> */
typedef struct hfc_mig_expire_log{
	ushort	result;
	union {
		struct {
			uchar    scsi_retry;				/* +0x02 */
			uchar    xob_cdb0;					/* +0x03 */
			uint     xob_fcp_dl;				/* +0x04 */
			uchar    xrb_scsi_status;			/* +0x08 */
			uchar    sense_key;					/* +0x09 */
			uchar    sense_asc;					/* +0x0a */
			uchar    sense_ascq;				/* +0x0b */
			uchar    xrb_fsb;					/* +0x0c */
			uchar    xrb_errcode[3];			/* +0x0d */
		} scsi_error;
		struct {
			uchar    scsi_retry;				/* +0x02 */
			uchar    xob_cdb0;					/* +0x03 */
			...
			uchar    xob_flag;					/* +0x09 */
			uchar    hfcp_lun;					/* +0x0a */
			uchar    hfcp_target_id;			/* +0x0b */
			uint     hfcp_tout_value;			/* +0x0c */
		} scsi_timeout;
		struct {
			uchar    mb_retry;					/* +0x02 */
			uchar    rsv;						/* +0x03 */
			uchar    mb_code[4];				/* +0x04 */
			uchar    esw;						/* +0x08 */
			uchar    ssn;						/* +0x09 */
			ushort   sbc;						/* +0x0a */
			uchar    fsb;						/* +0x0c */
			uchar    errcode[3];				/* +0x0d */
		} type_other;
	} uni;
} HFC_MIG_EXPIRE_LOG;	
/*  >>> FCLNX-GPL-FX-583 */

struct hg_mig_tgtlist {     /* FCLNX-GPL-FX-488 Migration for FIVE-FX */
	ushort		rev;			/* +000 - +001	*/
#define HFCFX_MIG_TL_REV0		0x0000		/* Rev 0                          */
	uchar		rid;				/* +002 - +002	*/
	uchar		sw_state;			/* +003 - +003	*/
	uint		tbl_size;			/* +004 - +007	*/
#define HFCFX_MIG_TL_TSIZE		0x3000		/* Size of Target List            */
	uchar		req_flag;			/* +008 - +008	*/
#define HFCFX_MIG_TL_REQ_DSCAN_SRC	0x01		/* Login/LU Scan for Shadow Driver at Source  */
#define HFCFX_MIG_TL_REQ_LOGOUT_SRC	0x02		/* Target Logout for Shadow Driver at Source  */
#define HFCFX_MIG_TL_REQ_LOGIN_SRC	0x03		/* Target Login for Shadow Driver at Source   */
#define HFCFX_MIG_TL_REQ_DELRSV_SRC	0x04		/* Delete Reserve for Shadow Driver at Source */
#define HFCFX_MIG_TL_REQ_SETLI_SRC	0x05		/* Set LUID Info for Guest Driver at Source   */
#define HFCFX_MIG_TL_REQ_DSCAN_DST	0x11		/* Login/LU Scan for Shadow Driver at Dest    */
#define HFCFX_MIG_TL_REQ_LOGOUT_DST	0x12		/* Target Logout for Shadow Driver at Dest    */
#define HFCFX_MIG_TL_REQ_LOGIN_DST	0x13		/* Target Login for Shadow Driver at Dest     */
#define HFCFX_MIG_TL_REQ_DELRSV_DST	0x14		/* Delete Reserve for Shadow Driver at Dest   */
	uchar		target_cnt;			/* +009 - +009	*/
	ushort		update_cnt;				/* +00A - +00B	*/
	uint		hvm_used;			/* +00C - +00F	*/
	uint64_t	dmywwpn;			/* +010 - +017	*/
	uint64_t	port_statis_addr;	/* +018 - +01F	*/
	uint64_t	gstwwpn;			/* +020 - +027	*/
	ushort		result;				/* +028 - +029	*/
#define HFCFX_MIG_TL_RESULT_NOMAL	0x0001	/* Nomal End(No Error)            */
#define HFCFX_MIG_TL_RESULT_DEGTGT	0x0002	/* Desagree target                  */
#define HFCFX_MIG_TL_RESULT_DISLNUM	0x0003	/* Disagreement LU Num            */
#define HFCFX_MIG_TL_RESULT_DISLUID	0x0004	/* Disagreement 1st LU ID         */
#define HFCFX_MIG_TL_RESULT_DISPS	0x0005	/* Disagreement Port Status       */
#define HFCFX_MIG_TL_RESULT_MBERR	0x0006	/* Mailbox Error(Retryout)        */
#define HFCFX_MIG_TL_RESULT_MBTOUT	0x0007	/* Mailbox Timeout                */
#define HFCFX_MIG_TL_RESULT_SCSIERR	0x0008	/* SCSI Command Error(Retryout)   */
#define HFCFX_MIG_TL_RESULT_SCSITO	0x0009	/* SCSI Command Timeout           */
#define HFCFX_MIG_TL_RESULT_MCK		0x000A	/* Detect MCK                     */
#define HFCFX_MIG_TL_RESULT_MBINT	0x000B	/* Detect Mailbox Interrupt       */
	ushort		rsv1;			/* +02A - +02B	*/
	uint		detail;			/* +02C - +02F	*/
	struct	mig_targetinfo	t_info[255];	/* +030	- +xxx	*/
};
