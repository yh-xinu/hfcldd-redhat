...
/* FCLNX-GPL-FX-689 */
	...
	if( !hfc_rhel75 ){
		...
		vhost = scsi_host_alloc((struct scsi_host_template *)host->hostt, sizeof(struct port_info));
	}
#else
	vhost = scsi_host_alloc(host->hostt, sizeof(struct port_info));
#endif
	...
	*(struct port_info **)fc_vport->dd_data = vpp;
	...
	HFC_ALLLOCK_IRQSAVE(pp,rp,flags); /* FCLNX-GPL-FX-569 */ /* FCLNX-GPL-FX-596 */
	rid = vpp->rid;
	...
	HFC_ALLUNLOCK_IRQRESTORE(pp,rp,flags); /* FCLNX-GPL-FX-569 */ /* FCLNX-GPL-FX-596 */
