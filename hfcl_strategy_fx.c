...
//static struct core_info *hfc_fx_choose_core(struct port_info *pp, struct hfc_pkt_fx *hfcp);	/* FCLNX-GPL-FX-645 */
...
/* FCLNX-GPL-FX-609 >>> */
int hfc_fx_blk_mq_unique_tag(struct request *rq){
	struct request_queue *q = rq->q;
	struct blk_mq_hw_ctx *hctx;
	struct hfc_blk_mq_ctx *mq_ctx;
	int hwq = 0;

	if (q->mq_ops) {
		mq_ctx=(struct hfc_blk_mq_ctx *)rq->mq_ctx;
//		hctx = q->mq_ops->map_queue(q, mq_ctx->cpu);
		hctx = q->queue_hw_ctx[q->mq_map[mq_ctx->cpu]];	/* FCLNX-GPL-FX-683 */
		hwq = hctx->queue_num;
	}
...
int hfc_fx_strategy_pg(struct scsi_cmnd *cmnd, void (*iodone)(struct scsi_cmnd *))
{
	...
	struct	port_info		*org_pp; /* FCLNX-GPL-FX-609 */
#endif
	...
	int		func_rc = 0;
	uint	result, ioctl_mode = 0, mig_mode=0;	/* FCLNX-GPL-FX-488 */
	uint	timeout=60*HZ;
	...
	uint			queue_no = 0;
#endif
	int		core_no_int=0;						/* FCLNX-GPL-FX-593 */

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,37)  /* FCLNX-GPL-FX-547 */
	struct Scsi_Host  *s_host = cmnd->device->host;		/* FCLNX-GPL-FX-445 */
#endif	
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 */
	uint	tag; /* FCLNX-GPL-FX-547 */
	ushort	hwq; /* FCLNX-GPL-FX-547 */
#endif /* FCLNX-GPL-FX-609 */
#if defined(BLK_MQ_ONLY) || defined(HFC_RHEL7_SCSI_MQ)
	bool use_blk_mq = true;
#endif /* defined(BLK_MQ_ONLY) || defined(HFC_RHEL7_SCSI_MQ) */

	pp = (struct port_info *) CMND_HOSTDATA(cmnd);
	cmnd->scsi_done = iodone;
	if(( iodone == (void *) hfc_fx_ioctl_iodone )
	||( cmnd->scsi_done == (void *) hfc_fx_mlpf_mig_iodone ))
	 ioctl_mode=1;
	cmnd->result = 0;

	if( cmnd->scsi_done == (void *) hfc_fx_mlpf_mig_iodone ){
		mig_mode=1;
	}
	if( pp == NULL ) 
	{
	...
	if (CMND_DEV(cmnd) == NULL) /* FCLNX-GPL-FX-620 >>> */
	{
		if(ioctl_mode == 1) func_rc=EIO;
		result = DID_NO_CONNECT;
		cmnd->result |= ( result << 16);
		HFC_DBGPRT(" hfcldd : hfc_fx_strategy - dev==NULL-error\n");
	...
	if (HFC_FX_MQ_VALID(pp) && ioctl_mode == 0) { /* FCLNX-GPL-FX-564 */
		/* select queue */
		org_pp = pp;
#if !defined(BLK_MQ_ONLY)
		/* FCLNX-GPL-FX-547 start */
		use_blk_mq = shost_use_blk_mq(pp->hosts);
#endif
		if (use_blk_mq)
		{
#if defined(HFC_RHEL7_SCSI_MQ)  /* FCLNX-GPL-FX-609 */
			tag = hfc_fx_blk_mq_unique_tag(cmnd->request);
#else
			tag = blk_mq_unique_tag(cmnd->request);
#endif /* FCLNX-GPL-FX-609 */			
			hwq = blk_mq_unique_tag_to_hwq(tag);

			queue_no = hwq % org_pp->mq_num;
	...
	{
        	char  core_no;
		if(( mig_mode==1 )&&(HFC_FX_MMODE_CHECK_SHADOW(pp) )){
			core_no = (char)pp->mig_master_core_no; /* FCLNX-GPL-FX-488 */
			core_no_int = pp->mig_master_core_no;	/* FCLNX-GPL-FX-593 */
		}else{
			core_no = -1;
			core_no_int = -1;						/* FCLNX-GPL-FX-593 */
		}
		if (core_no < 0) { /* core_no auto select */
			hfcp->core = hfc_fx_choose_core(pp, hfcp);
		} else {	/* core_no is selected. */
			if (pp->core_control != HFC_FX_CORECTL_AT_ASSGN_LU) { /* ? */
				atomic_set(&pp->curr_core, core_no);
			}
			hfcp->dev->curr_core = core_no;
			hfcp->core = hfcp->rp->core_arg[(uchar)core_no];
	...
	if (hfcp->core) { /* ? */
		core = hfcp->core;
		if (!mig_mode) {	/* FCLNX-GPL-FX-488 */
			HFC_CORELOCK_IRQSAVE(core, flags); /* FCLNX-GPL-FX-547 */
		}

		/* FCLNX-GPL-FX-547 start */
		if (hfc_fx_strategy_port(hfcp, &func_rc)) { /* FCLNX-GPL-FX-593 *//* FCLNX-GPL-FX-645 */
		...
		if (!mig_mode) {	/* FCLNX-GPL-FX-488 */
			...
			hfc_fx_strategy_port(hfcp, &func_rc); /* FCLNX-GPL-FX-586 */ /* FCLNX-GPL-FX-593 *//* FCLNX-GPL-FX-645 */
	}
...
uchar hfc_fx_choose_core_minque(struct port_info * pp, struct hfc_pkt_fx *hfcp, uchar *use_core_list)
{
	...
	uchar socket_number=0;
	 ...
	 /* FCLNX-GPL-FX-547 start -------- */
	switch(pp->core_num) {
	case 1: /* 1 Core HBA Port */
		core_no = pp->master_core_no;
		break;
	case 2: /* 2 Core HBA Port */
		core_no  = atomic_add_return(2, &pp->curr_core); /* 0, 2, 0, 2, 0, 2 .... */
		core_no &= (MAX_CORE_PROBE_FX -1); /* FCLNX-GPL-FX-547 */
		break;
	case 4: /* 4 Core HBA Port */
		core_no  = atomic_add_return(1, &pp->curr_core); /* 0, 1, 2, 3, 0, 1 .... */
		core_no &= (MAX_CORE_PROBE_FX -1); /* FCLNX-GPL-FX-547 */
		break;
	default: /* Other Case (Illegal Case) */
		core_no = pp->master_core_no;
		break;
	}
	/* -------- FCLNX-GPL-FX-547 end */
...
struct core_info *hfc_fx_choose_core(struct port_info *pp,struct hfc_pkt_fx *hfcp)
{
	...
	uchar socket_number=0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 */
	uint	ioctl_mode = 0; /* FCLNX-GPL-FX-562 */
	uint	tag; /* FCLNX-GPL-FX-562 */
	ushort	hwq; /* FCLNX-GPL-FX-562 */
#endif /* FCLNX-GPL-FX-609 */
	/* 4-port adapter */
	if( pp->core_num == 1 ){
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) || defined(HFC_RHEL7_SCSI_MQ) /* FCLNX-GPL-FX-609 start */
	/* FCLNX-GPL-FX-562 start ------ */
	/* SCSI-MQ ENABLE and RSS disable */
	if(( hfcp->cmd_pkt->scsi_done == (void *) hfc_fx_ioctl_iodone )
		||( hfcp->cmd_pkt->scsi_done == (void *) hfc_fx_mlpf_mig_iodone ))
		ioctl_mode=1;

	if ( ioctl_mode == 0 && 
			(!hfc_manage_info.hfcldd_mp_mod) && /* FCLNX-GPL-FX-715 */
#if !defined(BLK_MQ_ONLY)
			shost_use_blk_mq(pp->hosts) && 
#endif
			((pp->msi_flag == HFC_INT_TYPE_MSI) || (pp->msi_flag == HFC_INT_TYPE_MSIX))) {
/* FCLNX-GPL-FX-609 start ------ */
#if defined(HFC_RHEL7_SCSI_MQ) 
		tag = hfc_fx_blk_mq_unique_tag(hfcp->cmd_pkt->request);
#else
		tag = blk_mq_unique_tag(hfcp->cmd_pkt->request);
#endif
/* ------ FCLNX-GPL-FX-609 end */
		hwq = blk_mq_unique_tag_to_hwq(tag);
		core_no = hwq % pp->core_num;
		/* 2port-HBA */
		if (pp->core_num == 2){
			core_no = core_no*2; /* 2portHBA available_core : 0 , 2 */
		}
		if( hfc_fx_check_cs_disable(pp, hfcp->rp->core_arg[core_no]) ){
			core_no = pp->master_core_no;
		}
...
int hfc_fx_strategy_port(struct hfc_pkt_fx *hfcp, int *func_rc) /* FCLNX-GPL-FX-547 */ /* FCLNX-GPL-FX-593 *//* FCLNX-GPL-FX-645 */
{
	struct	port_info		*pp;
	struct	core_info		*core;
...
//	int			func_rc = 0 ; /* FCLNX-GPL-FX-547 */
	uint		lun,hit,entry;
	...
	pp = hfcp->pp;
	core = hfcp->core; /* FCLNX-GPL-FX-547 */	
	if(( cmnd->scsi_done == (void *) hfc_fx_ioctl_iodone ) 
	|| ( cmnd->scsi_done == (void *) hfc_fx_mlpf_mig_iodone ))
		ioctl_mode=1;	/* FCLNX-GPL-FX-488 */
    ...
    if( test_bit( HFC_PS_DIAG, (ulong *)&pp->status ) )		/* FCLNX-GPL-FX-126 */
	{
		...
		if(ioctl_mode == 1)  *func_rc=EIO; /* FCLNX-GPL-FX-547 */
		...
		return (0); /* FCLNX-GPL-FX-547 */
    }														/* FCLNX-GPL-FX-126 */

	/* FCLNX-GPL-FX-529 start */
	if (HFC_FX_MMODE_CHECK_SHADOW(pp)) {
		if (!ioctl_mode) { /* Non ioctl, Non mig */
			/* Reject SCSI command */
			set_bit(CFLAG_INH_ALTPATH, (ulong *)&hfcp->cmd_flags);
			hfcp->adap_status = SCS_OFFLINE;

//			hfc_fx_stra_trace(
//				HFC_FX_TRC_STRATEGY ,0x40 ,pp ,hfcp->rp, NULL, NULL, hfcp,
//				(ulong)cmnd, 0,0);
	...
	/* FCLNX-GPL-FX-547 start */
	if ((core) && (hfc_fx_check_cs_disable(pp, core))) {
		if(ioctl_mode == 1)
			*func_rc=EIO;
		/* FCLNX-GPL-FX-697 start */
		if( !test_bit(HFC_PS_ISOL, (ulong *)&pp->status) ) {
			set_bit(CFLAG_INH_ALTPATH, (ulong *)&hfcp->cmd_flags);
		}
		/* FCLNX-GPL-FX-697 end */
		hfcp->adap_status = SCS_MCK;
...
//				}
	...
	if(target == NULL)
	{
		...
		hfc_fx_stra_trace(
			HFC_FX_TRC_STRATEGY, 0x31 ,pp ,hfcp->rp, core, NULL, hfcp,
			(ulong)cmnd, 0,0);	/* FCLNX-GPL-FX-619 */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	if ( !( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ) ){ /* FCLNX-GPL-FX-472 */
		if (ioctl_mode == 0) { /* FCLNX-GPL-FX-523 start */
			...
			if (err) {
				...
				hfc_fx_stra_trace(HFC_FX_TRC_STRATEGY, 0x33 ,pp ,hfcp->rp, core, NULL, hfcp, (ulong)cmnd, 0,0);	/* FCLNX-GPL-FX-619 */
				hfc_fx_iodone(pp, NULL, cmnd, hfcp);
				return 0; /* FCLNX-GPL-FX-547 */
			}
...
// 		cmnd->result |= (result << 16);
	...
	if (!HFC_TG_FX_FLAGS_TEST(HFC_TF_WWN_VALID, target))
	{
		...
		hfc_fx_stra_trace(
			HFC_FX_TRC_STRATEGY ,0x32 ,pp ,hfcp->rp, core, target, hfcp,
			(ulong)cmnd, 0,0);	/* FCLNX-GPL-FX-619 */
	...
	if (pp->narrowmap == 1) {
		...
		if (!hit && !ioctl_mode) {
			...
			hfc_fx_stra_trace(
				HFC_FX_TRC_STRATEGY ,0x33 ,pp ,hfcp->rp ,core,target, hfcp,
				(ulong)cmnd, 0,0);	/* FCLNX-GPL-FX-619 */
	...
	if(hfc_manage_info.hfcldd_mp_mod) {
		...
		{	/* HFC-PCM will select core_no. */
			if (pp->core_control != HFC_FX_CORECTL_AT_ASSGN_LU) {
				...
				atomic_set(&pp->curr_core, core_no);	/* FCLNX-GPL-FX-593 */
			}
			hfcp->dev->curr_core = core_no;
			hfcp->core = hfcp->rp->core_arg[core_no];
			hfcp->core_no = core_no;
	  ...
	  && !(dev->lustat & HFC_DS_BLOCKED_SCSI) )
	{
		...
		return 1; /* FXLNX-GPL-FX-547 */ /* Need WX_QUE queuing */
	}
	else {
		/* busy status */
...
void hfc_fx_start(struct port_info *pp, struct region_info *rp, struct core_info *core, struct target_info_fx *target)
{
	...
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX=488 */
		xob_p = &pp->mig_xob_p[core->frame_start_xob[lp].start];
	}else{
		xob_p = &core->xob[core->frame_start_xob[lp].start];
	}
	if (HFC_FX_FRAME_BUSY(core, xob_p, lp)) {/* FCLNX-GPL-FX-014 */
		...
		if (target->core_queue[core->core_no].wx_que_tskmgm_cnt == 0) {
			...
			if (hfcp_wk == NULL) { // not found reset cmd
				...
				if (HFC_FX_FRAME_BUSY(core, xob_p, lp)) {
...
//				printk(KERN_ERR "hfcldd%d hfc_queue_check()=1.\n", ap->dev_minor);
		...
		else if (pp->pport->cscsi_mb_num) { /* FCLNX-GPL-FX-556 */
			skip_deq_wx = 1;
		}
		...
		/* FCLNX-GPL-FX-488 >>> */
		if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){
			HFC_BZERO(&pp->mig_xob_p[core->drv_next_xob],sizeof(struct xob_fx));
		}else{
...
//		}
		...
		/* FCLNX-GPL-FX-488 >>> */
		if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){
			xob_p = &pp->mig_xob_p[core->drv_next_xob];
		}else{
		...
		if( hfcp->cmd_flags & CFLAG_RESET_ANY){
			...
			if( hfcp->cmd_flags & CFLAG_CSCSI_ANY){
				...
				if (HFC_HFCP_FX_CFLAG_TEST(CFLAG_CSCSI_LU_WITHOUT_DMA, hfcp))
				{
					...
					clear_bit(HFC_DC_NEED_CSCSI_LU_WITHOUT_DMA, (ulong *)&hfcp->dev->dev_core_stat.core[pp->vport_id][core->core_no]); 
					set_bit(HFC_DC_WAIT_CSCSI_LU_WITHOUT_DMA, (ulong *)&hfcp->dev->dev_core_stat.core[pp->vport_id][core->core_no]);
				...
				else if (HFC_HFCP_FX_CFLAG_TEST(CFLAG_CSCSI_LU_WAIT_DMA, hfcp))
				{
					...
					cancel_xob_p->cancel_nexus = HFC_CANCEL_ITLNEXUS;
					/* FCLNX-GPL-FX-556 start */
					clear_bit(HFC_DC_NEED_CSCSI_LU_WAIT_DMA, (ulong *)&hfcp->dev->dev_core_stat.core[pp->vport_id][core->core_no]);
					set_bit(HFC_DC_WAIT_CSCSI_LU_WAIT_DMA, (ulong *)&hfcp->dev->dev_core_stat.core[pp->vport_id][core->core_no]);
					/* FCLNX-GPL-FX-556 end */
				} 
				...
				|| HFC_HFCP_FX_CFLAG_TEST(CFLAG_BUS_RESET, hfcp) ){	/* FCLNX-GPL-FX-176 */
					...
					/* FCLNX-GPL-FX-556 start */
					clear_bit( HFC_TS_NEED_TARGET_RESET, (ulong *)&target_wk->status );
					set_bit( HFC_TS_WAIT_TARGET_RESET, (ulong *)&target_wk->status );
					/* FCLNX-GPL-FX-556 end  */
				}
					...
					/* FCLNX-GPL-FX-556 start */
					clear_bit(HFC_DC_NEED_LUN_RESET_OR_ABORT, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);	/* FCLNX-GPL-FX-014 */
					set_bit(HFC_DC_WAIT_LUN_RESET_OR_ABORT, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);	/* FCLNX-GPL-FX-014 */
					/* FCLNX-GPL-FX-556 end */	
		...
		if (pp->pm_control == HFC_FX_PM_ON) {
			if (hfcp->pm_pkt_no != 0xffff) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,17,0) /* FCLNX-GPL-FX-699 --> 717 */
				pp->pm_pkt_pool[hfcp->pm_pkt_no].tsc_enq_xob = rdtsc_ordered();
#else
	...
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
		core->frame_start_xob[lp].pkt_no = ((struct hfc_pkt_fx *)(ulong)pp->mig_xob_p[initial_xob_no].drv_work)->pkt_no;
		((struct hfc_pkt_fx *)(ulong)pp->mig_xob_p[initial_xob_no].drv_work)->frame_no = lp;	/* FCLNX-GPL-FX-014 */
	}else{
	...
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
		xob_exec |= (uint)( (pp->mig_rid << 16) & 0x00ff0000) ;
	}else{
...
/*
 * Function:    hfc_set_new_hfcp
 * 
 * Purpose:     Set a hfc_pkt @ pport info
 *
 * Arguments:
 *  pp          Pointer to port_info
 *  
 ...
 * Notes:
 */
int hfc_set_new_hfcp(struct port_info *pp, struct hfc_pkt_fx *new_hfcp)
{
	struct port_info *ppp 		= NULL;
	struct hfc_pkt_fx *end_hfcp 	= NULL;

	if(pp == NULL) return INT_MAX;
	ppp = pp->pport;
	if(ppp == NULL) return INT_MAX;

	end_hfcp = ppp->pkt_end;

	if (ppp->pkt_top == NULL) {	/* Set -> The TOP point */
		/* Set hfcp_info chain */
		new_hfcp->pkt_prev = NULL;
		new_hfcp->pkt_next = NULL;
		/* Set func_info chain */
		ppp->pkt_top = new_hfcp;
		ppp->pkt_end = new_hfcp;
		atomic64_set(&ppp->pkt_next, (uint64_t)new_hfcp); /* CPFLNX-GPL-068 */
	}
	else {							/* Set -> The END point */
		/* Set hfcp_info chain */
		end_hfcp->pkt_next = new_hfcp;
		new_hfcp->pkt_prev = end_hfcp;
		new_hfcp->pkt_next = NULL;
		/* Set func_info chain */
		ppp->pkt_end = new_hfcp;
	}
	new_hfcp->pkt_no = ppp->pkt_num;
	ppp->pkt_num++;

	return ppp->pkt_num;
}

static inline void hfc_clear_hfcp(struct hfc_pkt_fx *hfcp)
{
	/* Clear flags */
	hfcp->cmd_flags &= (1UL << CFLAG_VALID);
	...
	hfcp->iov_cnt			= 0;
	hfcp->timeout			= 0;
//	hfcp->pkt_no			= 0; /* Don't Clear */
	hfcp->pm_pkt_no			= 0;
	hfcp->fail			= 0;
	hfcp->lun_id			= 0;
	hfcp->tout_slog_sbc 		= 0;
	hfcp->tout_slog_ssn 		= 0;
	hfcp->core_no			= 0;
	hfcp->cmd_xob			= 0;
	hfcp->target_id			= 0;
	hfcp->cpu_no			= 0;
	hfcp->group_id			= 0;
	hfcp->err_count_enable		= 0;
	hfcp->rid			= 0;
	hfcp->frame_no			= 0;
	hfcp->cmd_pkt			= NULL;
	hfcp->cmd_forw			= NULL;
	hfcp->cmd_prev			= NULL;
//	hfcp->pkt_prev			= NULL;	/* Don't Clear */
//	hfcp->pkt_next			= NULL;	/* Don't Clear */
	hfcp->target			= NULL;
	hfcp->dev				= NULL;
	hfcp->pp				= NULL;
	hfcp->rp				= NULL;
	hfcp->core				= NULL;
	memset(&hfcp->cmd_timeout, 0, sizeof(struct wtimer));
	hfcp->dma_handle		= 0;
	hfcp->upathmap			= 0;
	hfcp->cpathmap			= 0;
...
static inline void hfc_clear_pm_hfcp(struct hfc_pm_pkt_fx *pm_hfcp)
{
	uchar *clear_top = NULL;

	/* Clear flags */
	pm_hfcp->cmd_flags &= (1UL << CFLAG_VALID);

	/* Clear other field */
	clear_top = (uchar *)pm_hfcp;
	clear_top = &clear_top[sizeof(pm_hfcp->cmd_flags)];
	memset(clear_top, 0, (sizeof(struct hfc_pm_pkt_fx) - sizeof(pm_hfcp->cmd_flags)));
...
hfc_fx_get_new_hfcp(struct port_info *pp)
{
	...
	/* FCLNX-GPL-FX-547 :blk-mq & scsi-mq support start---------------------*/
	pkt_next = (struct hfc_pkt_fx *)atomic64_read(&pp->pport->pkt_next); /* CPFLNX-GPL-068 */	
	for (i=0;i<pp->pport->pkt_num;i++)
	{
		...
		if (pkt_next == NULL) {
			pkt_next = pp->pport->pkt_top; /* goto the top */
		}

		/* Challenge -> Getting the CFLAG_VALID flag */
		if (!test_and_set_bit_lock(CFLAG_VALID, (ulong *)&hfcp->cmd_flags))
		{
			/* old_bit is 0 (Other CPU didn't get flag) -> This CPU just got flag. */
			//atomic_inc(&pp->pkt_cnt);
			atomic64_set(&pp->pport->pkt_next, (uint64_t)pkt_next); /* Set Next */
			hfc_clear_hfcp(hfcp);
			/* CPFLNX-GPL-068 end */
			if (pp->pm_control == HFC_FX_PM_ON) {
				pm_hfcp = hfc_fx_get_new_pm_hfcp(pp, hfcp->pkt_no); /* CPFLNX-GPL-113 */	
				if (pm_hfcp != NULL) {
	...
	/*------------------------- FCLNX-GPL-FX-547 :blk-mq & scsi-mq suppor end */	
...
hfc_fx_get_new_rsv_hfcp(struct port_info *pp)
{
	...
	for (i=0;i<pp->rsv_pkt_num;i++)
	{
		...
		if (next_no >= pp->rsv_pkt_num) { next_no = 0; } /* goto the top */

		hfcp = &pp->rsv_pkt_pool[current_no];
		if (!test_and_set_bit_lock(CFLAG_VALID, (ulong *)&hfcp->cmd_flags))
		{
			/* old_bit is 0 (Other CPU didn't get flag) -> This CPU just got flag. */
			pp->rsv_pkt_no = next_no; /* Set Next */
			hfc_clear_hfcp(hfcp);
			hfcp->pm_pkt_no = 0xffff;
...
hfc_fx_get_new_pm_hfcp(struct port_info *pp, uint pm_pkt_no)
{
	struct hfc_pm_pkt_fx	*pm_hfcp;
	
	pm_hfcp = &pp->pm_pkt_pool[pm_pkt_no];
	if (pm_hfcp == NULL)
		return(NULL);
	if (!test_and_set_bit_lock(CFLAG_VALID, (ulong *)&pm_hfcp->cmd_flags)) {
		hfc_clear_pm_hfcp(pm_hfcp);
		pm_hfcp->pm_pkt_no = pm_pkt_no;
		return (pm_hfcp);
	}
...
int hfc_fx_eh_abort_pg(struct scsi_cmnd *cmnd)
{
	...
	uint                lun,find;
	unsigned long		flags = 0, io_flag = 0;	/* FCLNX-GPL-FX-669 */
	...
	spin_lock_irqsave(cmnd->device->host->host_lock, io_flag); /* FCLNX-GPL-FX-669 */
	...
	if( pp == NULL )
	{
		...
		spin_unlock_irqrestore(cmnd->device->host->host_lock, io_flag); /* FCLNX-GPL-FX-669 */
		return(FAILED);
    }
...
//	set_bit(CFLAG_VALID, (ulong *)&abort_pkt->cmd_flags); /*FCLNX-GPL-FX-547 Already set_bit:abort_pkt = hfc_fx_get_new_hfcp comment out*/
...
int hfc_fx_eh_device_reset_pg(struct scsi_cmnd *cmnd)
{
	...
	int                 rtn;
	unsigned long		flags = 0;
#if !( defined(HFC_X8664_SLES15SP2) || defined(HFC_RHEL8) ) /* FCLNX-GPL-FX-720, 728 */
	unsigned long io_flag = 0;	/* FCLNX-GPL-FX-669 */
#endif
	...
	uint						flags)
{
	...
	if (test_bit(CFLAG_TARGET_RESET, (ulong *)&flags)||test_bit(CFLAG_BUS_RESET, (ulong *)&flags)) {
		...
		/* FCLNX-GPL-FX-556 start */
		HFC_DBGPRT("%s - 1 rid=%d, core_no=%d, target=%d\n",
							__FUNCTION__,
							pp->rid,
							pp->region_arg[pp->rid]->core_arg [dev->abtcmd_core_no]->core_no,
							target->target_id);
		/* FCLNX-GPL-FX-556 end */
	...
	{
		/* FCLNX-GPL-FX-556 start */ 
		if(pp->pport->lpar_support){
			for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
				if ((core_wk = pp->region_arg[pp->rid]->core_arg[i]) == NULL)
					continue;
				if (hfc_fx_check_cs_disable(pp, core_wk))
					continue; 
				if (((test_bit( HFC_TS_WAIT_TGTRSP, (ulong *)&target->status ))&&(i == target->tgtrst_core_no))
				|| ((test_bit( HFC_PD_WAIT_BUSRSP, (ulong *)&pp->status_detail2 ))&&(i == target->tgtrst_core_no)))
					continue;
			...
			if (HFC_FX_MQ_VALID(pp)) {
				for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
					if ((core_wk = pp->region_arg[pp->rid]->core_arg[i]) == NULL)
						continue;
					if (test_bit(HFC_CS_CHK_STOP,(ulong *)&core_wk->status))
						continue; 
					for (j=1; j<=pp->pport->max_vport_count; j++) {
						...
						if (test_bit(CFLAG_CSCSI_TGT_WITHOUT_DMA, (ulong *)&flags)) {
							HFC_DBGPRT("%s - 5 rid=%d, core_no=%d, target=%d\n",
								 __FUNCTION__, vpp->rid, core_wk->core_no, vpp->target_arg[target->pseq]->target_id);
							hfc_fx_issue_task_mgm(
									vpp,
									vpp->region_arg[vpp->rid]->core_arg[core_wk->core_no],
									vpp->target_arg[target->pseq],
									NULL,
									dev,
									HFC_ISSUE_CSCSI_TGT_WITHOUT_DMA);
							issue_cscsi=1;
						} else {
							HFC_DBGPRT("%s - 6 rid=%d, core_no=%d, target=%d\n",
								 __FUNCTION__, vpp->rid, core_wk->core_no, vpp->target_arg[target->pseq]->target_id);
							hfc_fx_issue_task_mgm(
									vpp,
									vpp->region_arg[vpp->rid]->core_arg[core_wk->core_no],
	...
	uint						flags)
{
	...
	if (test_bit(CFLAG_ABORT, (ulong *)&flags)) {
		...
		/* FCLNX-GPL-FX-556 start ------ */
		HFC_DBGPRT("%s - 1 rid=%d, core_no=%d, target=%d, lun=%d\n",
							__FUNCTION__,
							pp->rid,
							pp->region_arg[pp->rid]->core_arg [dev->abtcmd_core_no]->core_no,
							target->target_id,
	...
	if (test_bit(CFLAG_LUN_RESET, (ulong *)&flags)) {
		// Abort Task Set (wait stop DMA)
		/* FCLNX-GPL-FX-556 start ------ */
		HFC_DBGPRT("%s - 2 rid=%d, core_no=%d, target=%d, lun=%d\n",
							__FUNCTION__,
							pp->rid,
							pp->region_arg[pp->rid]->core_arg [dev->abtcmd_core_no]->core_no,
							target->target_id,
		...
		if(pp->pport->lpar_support) { 
			for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
				...
				if (test_bit(CFLAG_CSCSI_LU_WITHOUT_DMA, (ulong *)&flags)) {
					HFC_DBGPRT("%s - 3 rid=%d, core_no=%d, target=%d, lun=%d\n",
								 __FUNCTION__, pp->rid, core_wk->core_no, target->target_id, dev->lun);
					hfc_fx_issue_task_mgm(pp, core_wk, target, NULL, dev, HFC_ISSUE_CSCSI_LU_WITHOUT_DMA);
					issue_cscsi=1;
				} else {
					HFC_DBGPRT("%s - 4 rid=%d, core_no=%d, target=%d, lun=%d\n",
								 __FUNCTION__, pp->rid, core_wk->core_no, target->target_id, dev->lun);
			...
			if (HFC_FX_MQ_VALID(pp)) {	/* FCLNX-GPL-FX-279, 286 */
				for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
					...
					for (j=1; j<=pp->pport->max_vport_count; j++) {
						...
						if (test_bit(CFLAG_CSCSI_LU_WITHOUT_DMA, (ulong *)&flags)) {
							HFC_DBGPRT("%s - 5 rid=%d, core_no=%d, target=%d, lun=%d\n",
								 __FUNCTION__, vpp->rid, core_wk->core_no, vpp->target_arg[target->pseq]->target_id, dev->lun);
							hfc_fx_issue_task_mgm(
									vpp,
									vpp->region_arg[vpp->rid]->core_arg[core_wk->core_no],
									vpp->target_arg[target->pseq],
									NULL,
									dev,
									HFC_ISSUE_CSCSI_LU_WITHOUT_DMA);
							issue_cscsi=1;
						} else {
							HFC_DBGPRT("%s - 6 rid=%d, core_no=%d, target=%d, lun=%d\n",
								 __FUNCTION__, vpp->rid, core_wk->core_no, vpp->target_arg[target->pseq]->target_id, dev->lun);
							hfc_fx_issue_task_mgm(
									vpp,
									vpp->region_arg[vpp->rid]->core_arg[core_wk->core_no],
	...
	uchar						mode)
{
	...
	struct region_info			*rp=NULL;
	struct target_info_fx		*target_wk	= target;
	...
	{	/* Issue Target Reset */
		...
		if(( reset_pkt == NULL)||( dummy_cmnd == NULL)){
			/* FCLNX-GPL-FX-556 */
			clear_bit( HFC_TS_NEED_TARGET_RESET, (ulong *)&target_wk->status );	/* FCLNX-GPL-328 */
		...
		/* Initiate Target Reset *//* FCLNX-GPL-FX-178 Start *//* FCLNX-GPL-FX-556 >>> */
		if ( !(pp->status & HFC_PS_BLOCKED_SCSI)
		  && !( test_bit(HFC_PD_WAIT_CLOSE, (ulong *)&pp->status_detail2))
		  && !( test_bit(HFC_TS_NEED_CANCEL_SCSI_WITHOUT_DMA, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_WAIT_CANCEL_SCSI_WITHOUT_DMA, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_NEED_CANCEL_SCSI_WAIT_DMA, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_WAIT_CANCEL_SCSI_WAIT_DMA, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_NEED_PLOGI, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_WAIT_PLOGI, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_NEED_PRLI, (ulong *)&target_wk->status))
		  && !( test_bit(HFC_TS_WAIT_PRLI, (ulong *)&target_wk->status)))
		{
		...
		}	/* FCLNX-GPL-FX-178 End *//* FCLNX-GPL-FX-556 <<< */
...
//		set_bit(CFLAG_VALID, (ulong *)&reset_pkt->cmd_flags); /* FCLNX-GPL-FX-547:Aready set bit: reset_pkt = hfc_fx_get_new_hfcp  */
		...
		if ( !(pp->status & HFC_PS_BLOCKED_SCSI)
		  && !(pp->status_detail2 & HFC_PD2_BLOCKED_SCSI)
		  && !(target_wk->status & HFC_TS_BLOCKED_SCSI) )	/* FCLNX-GPL-FX-556 */
		{
			hfc_fx_start( pp, rp, core, target);
		}else{
			if (target->core_queue[core->core_no].next_dstart_flag == 0) {
...
//		set_bit(CFLAG_VALID, (ulong *)&reset_pkt->cmd_flags); /* FCLNX-GPL-FX-547:Areadey set_bit : reset_pkt=hfc_fx_get_new_hfcp  */
	...
	else if( (mode == HFC_ISSUE_CSCSI_TGT_WAIT_DMA)
	 || (mode == HFC_ISSUE_CSCSI_TGT_WITHOUT_DMA)
	 || (mode == HFC_ISSUE_CSCSI_LU_WAIT_DMA)
	 || (mode == HFC_ISSUE_CSCSI_LU_WITHOUT_DMA) ){	/* FCLNX-GPL-FX-014 Start */ /* FCLNX-GPL-FX-547 if->else if */
		/* Issue C_XOB  */
		...
		if ((mode == HFC_ISSUE_CSCSI_TGT_WAIT_DMA) || (mode == HFC_ISSUE_CSCSI_TGT_WITHOUT_DMA)){
			...
			memset(reset_pkt, 0, sizeof(struct hfc_pkt_fx));
			set_bit(CFLAG_VALID, (ulong *)&reset_pkt->cmd_flags); /* FCLNX-GPL-FX-547 Not Aready set_bit*/
		...
		if ((mode == HFC_ISSUE_CSCSI_LU_WAIT_DMA) || (mode == HFC_ISSUE_CSCSI_LU_WITHOUT_DMA)){
			...
			if(mode == HFC_ISSUE_CSCSI_LU_WAIT_DMA){
				...
				set_bit(HFC_DC_NEED_CSCSI_LU_WAIT_DMA, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);	/* FCLNX-GPL-FX-255,272 */ /* FCLNX-GPL-FX-556 */
			}else{
				/* Issue CANCEL SCSI for Target */
				HFC_DBGPRT("%s - 2\n", __FUNCTION__);
				set_bit(CFLAG_CSCSI_LU_WITHOUT_DMA, (ulong *)&reset_pkt->cmd_flags);
				set_bit(HFC_DC_NEED_CSCSI_LU_WITHOUT_DMA, (ulong *)&dev->dev_core_stat.core[pp->vport_id][core->core_no]);	/* FCLNX-GPL-FX-255,272 */ /* FCLNX-GPL-FX-556 */
			}
...
//		set_bit(CFLAG_VALID, (ulong *)&reset_pkt->cmd_flags); /* FCLNX-GPL-FX-547  */
		core->pkt_cnt++;
		...
		||(mode == HFC_ISSUE_CSCSI_TGT_WITHOUT_DMA)){	/* FCLNX-GPL-FX-178 Start */
			...
			if ( !(pp->status & HFC_PS_BLOCKED_SCSI)	/* FCLNX-GPL-FX-556 >>> */
			  && !( test_bit(HFC_PD_WAIT_CLOSE, (ulong *)&pp->status_detail2))
			  && !( test_bit(HFC_TS_NEED_CANCEL_SCSI_WITHOUT_DMA, (ulong *)&target_wk->status))
			  && !( test_bit(HFC_TS_WAIT_CANCEL_SCSI_WITHOUT_DMA, (ulong *)&target_wk->status))
			  && !( test_bit(HFC_TS_NEED_CANCEL_SCSI_WAIT_DMA, (ulong *)&target_wk->status))
...
/* FCLNX-GPL-FX-556 start */
void hfc_fx_clear_tgt_core_stat(
	struct port_info			*pp,
	struct target_info_fx		*target)
{
	struct port_info			*vpp;
	...
	if (HFC_FX_MQ_VALID(pp)) {
		for (j=0; j<=pp->pport->max_vport_count; j++) {
			...
			vpp->target_arg[target->pseq]->tgt_core_stat.all = 0;
		}
	}
	else {
		target->tgt_core_stat.all = 0;
	}
}

int hfc_fx_test_tgt_core_stat(
	struct port_info			*pp,
	struct target_info_fx		*target)
{
	struct port_info			*vpp;
	...
	if (HFC_FX_MQ_VALID(pp)) {
		for (j=0; j<=pp->pport->max_vport_count; j++) {
			...
			if (vpp->target_arg[target->pseq]->tgt_core_stat.all != 0)
				cnt++;
		}
	}
	else {
		if (target->tgt_core_stat.all)
			cnt++;
	}
					...
					uchar				type)
{
	...
	{
		if (HFC_FX_MMODE_CHECK_SHADOW(pp) && i == HFC_MIG_XOB_MAX )
			break;	/* FCLNX-GPL-FX-488 */
	...
	do {
		...
		if (HFC_FX_MMODE_CHECK_SHADOW(pp)  && (pp->mig_xob_p != NULL) ){	/* FCLNX-GPL-FX-488 */
			xob_ptr = &pp->mig_xob_p[outp_num];
		}else{
	...
	uchar					type)
{
	...
	for (i=0 ; i< MAX_CORE_PROBE_FX ; i += (MAX_CORE_PROBE_FX/pp->core_num)) {
		...
		if (HFC_FX_MMODE_CHECK_SHADOW(pp) && (pp->mig_xrb_p != NULL)){	/* FCLNX-GPL-FX-488 */
			xrb	   = (struct xrb_fx *)pp->mig_xrb_p;
		}else{
	...
	uint						type)
{
	...
	if (type == HFC_FLASH_TARGET)
	{
		...
		hfc_fx_clear_tgt_core_stat(pp, target);	/* FCLNX-GPL-FX-014 */ /* FCLNX-GPL-FX-556 */
	}
	...
	if (type == HFC_FLASH_TARGET)				/* FCLNX-GPL-0343 */
	{
		...
		while( dev != NULL ){
			...
			memset(dev->dev_core_stat.all, 0, sizeof(uint)*HFC_FX_MAX_MQ_NUM);	/* FCLNX-GPL-FX-014 */ /* FCLNX-GPL-FX-556 */
			dev = dev->next;
		}
	}
						...
						uchar					type)
{
	...
	if (!HFC_FX_MQ_VALID(pp))
		return;
	for (i=0; i<=pp->max_vport_count; i++) { /* FCLNX-GPL-FX-556 */
		vpp = pp->pport->vport_ptr[i].vport_arg; /* FCLNX-GPL-FX-572 */
		if (vpp == NULL)
			continue;
		...
		/* FCLNX-GPL-FX-556 start */
//		if (pp->rid != vpp->rid) {
//			HFC_ALLCORELOCK(pp->region_arg[vpp->rid]); /* FCLNX-GPL-FX-569 */
//		}
		/* FCLNX-GPL-FX-556 end */
		...
		/* FCLNX-GPL-FX-556 start */
//		if (pp->rid != vpp->rid) {
//			HFC_ALLCOREUNLOCK(pp->region_arg[vpp->rid]); /* FCLNX-GPL-FX-569 */
//		}
		/* FCLNX-GPL-FX-556 end */
...
//	}
	...
	/* xob full check */
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
		if((core->drv_next_xob+chk_next) >= HFC_MIG_XOB_MAX) {	/* FCLNX-GPL-FX-014 Start */
			next_xob = (core->drv_next_xob + chk_next) - HFC_MIG_XOB_MAX;
		}
		else {
			next_xob = core->drv_next_xob+chk_next;
		}	/* FCLNX-GPL-FX-014 End */
		if ((pp->mig_xob_p[core->drv_next_xob].flag & HFC_XOB_VALID) || // current
		    (pp->mig_xob_p[next_xob].flag           & HFC_XOB_VALID))   // next
		{
			hfc_fx_stra_trace(
			HFC_FX_TRC_RES_CHK ,0x22 , pp, hfcp->rp, core ,hfcp->target ,hfcp,
				(ulong)cmnd, 0, 0);
			return( HFC_XOB_FULL );
		}
	}else{
...
void hfc_fx_dma_map(struct port_info *pp, struct core_info *core, struct hfc_pkt_fx *hfcp)
{
	...
	ushort				seg_cnt;
	ulong				MIG_SEG_LIST_OFFSET	= HFC_PAGE_SIZE/2; /* FCLNX-GPL-FX-533 */
	const int			MIG_SEG_4 = 4; /* FCLNX-GPL-FX-533 */
			...
			if(hfcp->seg_cnt > 0)
			{
				scsi_for_each_sg(cmnd, cur_seg, hfcp->seg_cnt, cn) {
					...
					{	/* If hfcp->iov_no is smaller than 4, use SEG-INFO in XOB area */
						/* FCLNX-GPL-FX-533 start */
						if (HFC_FX_MMODE_CHECK_SHADOW(pp)) {	/* FCLNX-GPL-FX-488 */
							seg_info_ptr = (struct seg_info_fx *)&(pp->mig_xob_p[xob_no].seg_info_xob[cn]);
						} else {
							seg_info_ptr = (struct seg_info_fx *)&(core->xob[xob_no].seg_info_xob[cn]);
						}
						b_dma_a = sg_dma_address(cur_seg);
						b_length = sg_dma_len(cur_seg);
						dma_add = (uint64_t)(ulong)b_dma_a;
						/* FCLNX-GPL-FX-533 end */
						if(cn == 0)
						{
...
//						dma_add = (uint64_t)b_dma_a;
					...
					{	/* Otherwise, use SEG-INFO-LIST */
						/* FCLNX-GPL-FX-533 start */
						if (HFC_FX_MMODE_CHECK_SHADOW(pp)) {	/* FCLNX-GPL-FX-488 */
							char *mig_seg_4 = pp->mig_seg_svaddr[MIG_SEG_4];
							seg_info_ptr = (struct seg_info_fx *)&(mig_seg_4[MIG_SEG_LIST_OFFSET]);
						} else {
...
//						}
	...
	if(cn < 4) {
		...
		/* FCLNX-GPL-FX-533 start */
		if (HFC_FX_MMODE_CHECK_SHADOW(pp)) {	/* FCLNX-GPL-FX-488 */
			seg_info_baddr = pp->mig_seg_gpaddr[MIG_SEG_4] + MIG_SEG_LIST_OFFSET;
			HFC_8L_TO_8B(pp->mig_xob_p[xob_no].seg_5th, seg_info_baddr);
		} else {
	...
	seg_cnt = (ushort)cn;
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
		HFC_2L_TO_2B(pp->mig_xob_p[xob_no].seg_cnt, seg_cnt);	/* No count for F=1 case */	/* Bug */
	} else {
...
//	HFC_DBGPRT(" hfcldd%d : hfc_fx_make_cmdiu - start\n", pp->dev_minor);
	...
	if( hfcp->cmd_flags & CFLAG_CSCSI_ANY){	/* FCLNX-GPL-FX-014 */
		...
		if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
			HFC_MEMCPY((fcp_cmd_iu_fx_t *)(&pp->mig_xob_p[hfcp->cmd_xob].fcp_cmd),
	...
	/* FCP-CNTL Byte1 */
	fcp_cmd.fcp_cntl.qtype = 0; /* Simple-Q : FIVE-FX */ /* FCLNX-GPL-FX-714 */
							 ...
							 struct hfc_pkt_fx *hfcp)
{
	...
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
		for(i=0;i<3;i++) xob_p->trans_s_id[i] =
						 (uchar)((pp->mig_scsi_id     & 0x0000000000ffffff) >> ((2-i)*8)); /* Trans S_ID */
	}else{
		for(i=0;i<3;i++) xob_p->trans_s_id[i] =
						 (uchar)((pp->scsi_id     & 0x0000000000ffffff) >> ((2-i)*8)); /* Trans S_ID */
	}
	...
	core->drv_next_xob++ ;
	if (HFC_FX_MMODE_CHECK_SHADOW(pp) ){	/* FCLNX-GPL-FX-488 */
		if ( core->drv_next_xob >= HFC_MIG_XOB_MAX )
			core->drv_next_xob = 0 ;
	}else{
...
void hfc_fx_set_cmnd_res(
	...
	uint					result )
{
	/* FCLNX-GPL-FX-600 start ------ */
	if (pp != NULL && core != NULL && cmnd != NULL && hfcp != NULL){
		if ( hfcp->cmd_pkt->scsi_done == (void *) hfc_fx_mlpf_mig_iodone ) {
			ushort mig_result = HFCFX_MIG_TL_RESULT_SCSIERR;
			uchar xob_no = 0;
			if (result != 0){
				xob_no = hfcp->cmd_xob;
				HFC_2L_TO_2B(pp->mig_exp_log.result, mig_result);
				if (hfcp->target != NULL)
					pp->mig_exp_log.uni.scsi_error.scsi_retry = (uchar)hfcp->target->mig_scsi_retry;

				pp->mig_exp_log.uni.scsi_error.xob_cdb0 = core->xob[xob_no].fcp_cmd.fcp_cdb[0];
				HFC_4L_TO_4B(pp->mig_exp_log.uni.scsi_error.xob_fcp_dl, core->xob[xob_no].fcp_cmd.fcp_dl);
				pp->mig_exp_log.uni.scsi_error.xrb_scsi_status = core->xrb[core->drv_next_xrb].resp_iu1.scsi_status;
				pp->mig_exp_log.uni.scsi_error.sense_key = hfcp->cmd_pkt->sense_buffer[2];
				pp->mig_exp_log.uni.scsi_error.sense_asc = hfcp->cmd_pkt->sense_buffer[12];
				pp->mig_exp_log.uni.scsi_error.sense_ascq = hfcp->cmd_pkt->sense_buffer[13];
				pp->mig_exp_log.uni.scsi_error.xrb_fsb = core->xrb[core->drv_next_xrb].xcrb.fsb;
				pp->mig_exp_log.uni.scsi_error.xrb_errcode[0] = core->xrb[core->drv_next_xrb].xcrb.err_code[0];
				pp->mig_exp_log.uni.scsi_error.xrb_errcode[1] = core->xrb[core->drv_next_xrb].xcrb.err_code[1];
				pp->mig_exp_log.uni.scsi_error.xrb_errcode[2] = core->xrb[core->drv_next_xrb].xcrb.err_code[2];
			}
		}
	}
	/* ------ FCLNX-GPL-FX-600 end */
	...
	struct hfc_pkt_fx		*hfcp )
{
	...
	if (hfcp != NULL) {
		uint hsdldd_cmnd = HFC_HFCP_FX_CFLAG_TEST(CFLAG_HSDLDD_VALID, hfcp); /* This scsi_cmnd is came from hsdldd */ /* FCLNX-GPL-FX-712 */
		uint pm_pkt_no   = hfcp->pm_pkt_no; /* FCLNX-GPL-FX-712 */
		...
		if (hfcp->seg_cnt)
		{
			if( ioctl_mode !=1 ){
				/* FCLNX-GPL-FX-685 */
				dma_unmap_sg(&pp->pci_cfginf->dev, scsi_sglist(cmnd), scsi_sg_count(cmnd),
						cmnd->sc_data_direction);
			}
		}
		...
		if ( hfcp->cmd_flags & CFLAG_RESET_ANY )	/* FCLNX-GPL-FX-014 */
		{
			...
			if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ) {	/* FCLNX-GPL-FX-261 */
				if ( !hfc_manage_info.npubp->hfc_fx_check_io_reset_complete(hfcp) ) {
					clear_bit_unlock(CFLAG_VALID, (ulong *)&hfcp->cmd_flags); /* FCLNX-FX-069 */
					return;			/* continue reset process */
				}
			}	/* FCLNX-GPL-FX-261 */
			
			clear_bit_unlock(CFLAG_VALID, (ulong *)&hfcp->cmd_flags); /* FCLNX-GPL-FX-547 */
			if (core)
				core->pkt_cnt--;

			/* FCLNX-GPL-FX-503 >>> */
			if (pp->pm_control == HFC_FX_PM_ON) {
				if (pm_pkt_no != 0xffff) { /* FCLNX-GPL-FX-712 start */
					clear_bit_unlock(CFLAG_VALID, (ulong *)&pp->pm_pkt_pool[pm_pkt_no].cmd_flags); /* FCLNX-GPL-FX-547 */
				} /* FCLNX-GPL-FX-712 end */
			}
			/* FCLNX-GPL-FX-503 <<< */
		}
		else 
		{
			...
			if (hsdldd_cmnd) /* FCLNX-GPL-FX-712 */
				clear_bit(CFLAG_VALID, (ulong *)&hfcp->cmd_flags); /* FCLNX-GPL-FX-712 */

			if ( cmnd->scsi_done != NULL ) {
				cmnd->scsi_done(cmnd);
			}
			
			if (pp->pm_control == HFC_FX_PM_ON) { /* FCLNX-GPL-FX-712 start */
				if (pm_pkt_no != 0xffff) {
					clear_bit_unlock(CFLAG_VALID, (ulong *)&pp->pm_pkt_pool[pm_pkt_no].cmd_flags);
				}
			} /* FCLNX-GPL-FX-712 end */
	...
	unsigned long long		etc3)
{
	...
	switch (id)
	{
		...
		case HFC_FX_TRC_LUN_RESET :
			...
			if(id != HFC_FX_TRC_STRATEGY)
				port_trace = 1; /* FCLNX-GPL-FX-619 */
			break;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0) /* FCLNX-GPL-717 */
		resid =  scsi_get_resid(cmnd);
		HFC_4L_TO_4B(trc1->resid, resid);
		HFC_4L_TO_4B(trc1->serial_number, serial_number);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0) /* FCLNX-GPL-717 */
		resid =  scsi_get_resid(cmnd);
		HFC_4L_TO_4B(trc2->resid, resid);
		HFC_4L_TO_4B(trc2->serial_number, serial_number);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0) /* FCLNX-GPL-717 */
		resid =  scsi_get_resid(cmnd);
		HFC_4L_TO_4B(trc3->resid, resid);
		HFC_4L_TO_4B(trc3->serial_number, serial_number);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0) /* FCLNX-GPL-717 */
		resid =  scsi_get_resid(cmnd);
		HFC_4L_TO_4B(trc4->resid, resid);
		HFC_4L_TO_4B(trc4->serial_number, serial_number);
#else
