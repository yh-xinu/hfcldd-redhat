...
const uint fx_hg_addr_max[5] = {         /* FCLNX-GPL-120 */
						...
						0x4ff   /* 4 HFC_PKTYPE_FIVE_FX *//* FCLNX-GPL-579,595 */
						};
	...
	struct	diag_ioctl	*diag)	/* struct diag_ioctl */
{
	...
	uchar	errlog_no ,mq_num; /* FCLNX-GPL-FX-558 */
	ulong	flags=0;
	int		i, j,q_no; /* FCLNX-GPL-FX-558 */
	int		rtn = 0;
	uint	port_exec = 0;
	size_t pm_size=0; /* FCLNX-GPL-FX-558 */	
	...
	switch( diag->uni.performance.opcode )
	{
		...
		case HFC_PFM_OPR_IOCNT:
			...
			/* FCLNX-GPL-FX-558 start ------ */
			if(HFC_FX_MQ_VALID(pp)){
				mq_num = pp->pport->max_vport_count + 1; /* vport_count + Pysical Port */
				pm_size = sizeof(struct performance_info)*MAX_CORE_PROBE_FX*HFC_FX_MAX_MQ_NUM;
			}
			else{
				mq_num = 1; /* Pysical Port Only */
				pm_size = sizeof(struct performance_info)*MAX_CORE_PROBE_FX;
			}
			/* ------ FCLNX-GPL-FX-558 end */
			
			if (diag->length < pm_size) {
				HFC_DBGPRT("ioctl error(trcid=0x%04x, subid=0x%04x) \n",
						HFC_TRC_IOCTL_PERFORMANCE, 0x0A );
				rtn = EINVAL;
				break;
			}
			
			pm = (struct performance_info *)hfc_fx_kmalloc(pp, pm_size, GFP_ATOMIC);
			if (pm == NULL) {
			...
			/* FCLNX-GPL-FX-558 start ------ */			
			for(q_no=0;q_no < mq_num; q_no++){
				/* FCLNX-GPL-FX-558 start ------ */
				if(HFC_FX_MQ_VALID(pp)){
					wkpp = pp->vport_ptr[q_no].vport_arg;
					if (wkpp == NULL)
						continue;
					rp = wkpp->region_arg[wkpp->rid];
					if (rp == NULL)
				...
				for(i=0;i<MAX_CORE_PROBE_FX;i++){
					...
					pmc = &pm[MAX_CORE_PROBE_FX*q_no+i];
					...
					if ((!HFC_FX_MMODE_CHECK_SHARED(pp)) && HFC_FX_PHYSICAL_PORT(pp)) {
						...
						pmc->tx_frames                  +=(uint64_t)pp->tx_frames[i];
						pmc->tx_words                   +=(uint64_t)pp->tx_words[i];
						pmc->rx_frames                  +=(uint64_t)pp->rx_frames[i];
						pmc->rx_words                   +=(uint64_t)pp->rx_words[i];
					}
			...
			/* FCLNX-GPL-FX-558 start ------ */
			if ( COPYOUT( (char *)pm, (char *)(ulong)diag->addr, pm_size) != 0 ) {
				HFC_DBGPRT("ioctl error(trcid=0x%04x, subid=0x%04x) \n",
					HFC_TRC_IOCTL_PERFORMANCE, 0x0B );
				errlog_no = 0x35 ;
					hfc_fx_errlog(wkpp, core, NULL, NULL, HFC_ERRLOG_TYPE_NONE, ERRID_HFCP_ERRC, 0x5F, &errlog_no, 1) ;
...
//	if ((HFC_FX_NPIV_ENABLE(pp) | HFC_FX_MQ_ENABLE(pp)) && (diag.rid != 0)) {	/* FCLNX-GPL-FX-196 */
	if (diag.vport_no != 0) {
		if  ((  diag.sub_cmd == HFC_DGRD_INITTBL)
			...
			|| (diag.sub_cmd == HFC_DG_RDHG)		/* FCLNX-GPL-FX-579,595 */
