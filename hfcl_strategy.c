/*
 * hfcl_strategy.c
 * Copyright (C) 2007, 2021, Hitachi, Ltd.
...
int hfc_strategy(struct Scsi_Host *shost, struct scsi_cmnd *cmd) {
	if (!shost) {
		HFC_DBGPRT(" hfcldd : hfc_strategy - shost is NULL\n");
		cmd->result |= (DID_ERROR << 16);
		cmd->scsi_done(cmd);
		return 0;
	}
	return hfc_strategy_ext(cmd, cmd->scsi_done);
}
int hfc_strategy_ext(struct scsi_cmnd *cmnd, void (*iodone)(struct scsi_cmnd *))
...
int hfc_strategy_pg(struct scsi_cmnd *cmnd, void (*iodone)(struct scsi_cmnd *))
{
	...
	unsigned long	flags = 0, io_flag = 0;	/* FCLNX-0274 *//* FCLNX-GPL-FX-669 */
	struct dev_info		*dev = NULL;		/* FCLNX-GPL-0343 */
	struct Scsi_Host  *s_host = cmnd->device->host; /* FCLNX-GPL-FX-584 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	...
	spin_lock_irqsave(s_host->host_lock, io_flag); /* FCLNX-GPL-FX-584 *//* FCLNX-GPL-FX-669 */
	...
	if(hfcp != NULL)
	{
		if( test_bit(CFLAG_VALID, (ulong *)&hfcp->cmd_flags ) )
		{
			...
			spin_unlock_irqrestore(s_host->host_lock, io_flag); /* FCLNX-GPL-FX-584 *//* FCLNX-GPL-FX-669 */
			return(0);
		}
	}
...
//				ap->scsi_exec_cnt++;
				...
				if ((((err >> 16) & 0xff) == DID_NO_CONNECT)||(((err >> 16) & 0xff) == DID_TRANSPORT_FAILFAST)) {
					ap->scsi_err_cnt++; /* scsi_err count up */
				}
...
WAIT_LUN_RST :
	...
	while (1) {
		HFC_ADAPUNLOCK_IRQRESTORE(flags);
		spin_unlock_irqrestore(host->host_lock, io_flag); /* FCLNX-GPL-FX-669 */

		set_current_state(TASK_UNINTERRUPTIBLE);
		schedule_timeout(HZ/10+1);					/* Wait 100ms */

		spin_lock_irqsave(host->host_lock, io_flag);	/* FCLNX-GPL-FX-669 */
		HFC_ADAPLOCK_IRQSAVE(flags);
...
void hfc_make_cmdiu( struct adap_info *ap, struct hfc_pkt *hfcp )
{
	 ...
	 && !test_bit(CFLAG_BUS_RESET, (ulong *)&hfcp->cmd_flags) ) 
	{
  		if (cmnd->device->tagged_supported) {
			fcp_cmd.fcp_cntl.qtype = 0;	/* Simple-Q   */ /* FCLNX-GPL-FX-714 */
  		} else {
	...
	else if ( cmnd->sc_data_direction == SCSI_DATA_READ)	/* FCLNX-GPL-FX-721 */
	...
	if (hfcp != NULL)
	{
		...
		if (hfcp->seg_cnt)
		{
			if( ioctl_mode !=1 ){		/* FCLNX-GPL-464 */
				/* FCLNX-GPL-685 */
				dma_unmap_sg(&ap->pci_cfginf->dev, scsi_sglist(cmnd), scsi_sg_count(cmnd),
						cmnd->sc_data_direction);
			}							/* FCLNX-GPL-464 */
		}
#else
...
//		trc1->cmd_timeout	= hfcp->cmd_timeout ;				/* hfc_pkt timeout time */
		...
		if( cmnd != NULL )
		{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0)                 /* FCLNX-GPL-0717 */
			unsigned int resid = 0; /* FCLNX-GPL-717 */
#endif
			trc1->pkt_flag = 0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)			/* FCLNX-GPL-0343 */
			sdev = cmnd->device;
			...
			HFC_4L_TO_4B(trc1->pkt_resid, resid) ;
#else
