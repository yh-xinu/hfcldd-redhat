...
int hfc_fx_rtn_scsi_cmnd( struct scsi_cmnd *cmnd, void *arg, uint64_t *buffer, int type ) {
	...
	if( cmnd->result ) {
...
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)		/* FCLNX-GPL-0343 */
		...
		if( cmnd->resid == cmnd->request_bufflen ) {
			...
			/* Copy inquiry data */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0) /* FCLNX-GPL-717 */
			if( scsi_get_resid(cmnd) > 0 ) {
				wrk -> resid_flags = HFC_RESID_UNDERFLOW;
				wrk -> resid	   = (ushort)scsi_get_resid(cmnd);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)		/* FCLNX-GPL-0343 */
			if( cmnd->sdb.resid > 0 ) {
				wrk -> resid_flags = HFC_RESID_UNDERFLOW;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,1,0) /* FCLNX-GPL-717 */
			data_length = cmnd->sdb.length - scsi_get_resid(cmnd);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)		/* FCLNX-GPL-0343 */
...
#define MAX_CNT            0xffffffff
	...
	int                cmnd_device_alloc_flag =0;	//FCLNX-0147
#endif
	ulong				page_link_vaddr[HFC_SCATTERLIST_NUM]={0};	/* FCLNX-GPL-FX-526 */
...
//	STRUCTDUMP( DMP_IOCMD, (uchar *)&cdb, sizeof(struct hfc_ioctl_cdb) );
	...
	/* FCLNX-GPL-526 start */
	if (internal != TRUE) { /* from ioctl */
		if (cdb.flags & SCSI_WRITE_CDB) { /* unsupport CDB */
			HFC_DBGPRT( "ioctl error(trcid=0x%04x, subid=0x%04x) \n", HFC_TRC_IOCTL_SC_SCICMD, 0x0a );
			return EINVAL;
		}
	}
	if (cdb.data_length > (HFC_PAGE_SIZE*HFC_SCATTERLIST_NUM)) { /* sg_list size over */
	...
	{	/* FCLNX-GPL-FX-526 start */
		...
		for (i = 0; i < HFC_SCATTERLIST_NUM; i++) {
			...
			memset((char *)sgl->page_link, 0, HFC_PAGE_SIZE); /* page_link buffer size = PAGE_SIZE */
			page_link_vaddr[i] = sgl->page_link; /* Backup Virtual addr */
		}
		/* Set sg_list data buff */
		for (i = 0; i < (cdb.data_length/HFC_PAGE_SIZE); i++) {
			sgl = &cmnd->sdb.table.sgl[data_buf_num];
			sgl->dma_length = HFC_PAGE_SIZE ;
			if (cdb.flags & SCSI_WRITE_CDB) { /* Write Case Only */
				buff_addr = (ulong)cdb.buffer + (HFC_PAGE_SIZE * data_buf_num);
				memcpy((char *)sgl->page_link, (char *)buff_addr, sgl->dma_length);
				sg_set_buf(sgl, (char *)(char *)buff_addr, sgl->dma_length); /* Set Page Addr */
			}
			data_buf_num++;
		}
...
//	STRUCTDUMP( DMP_SCSICMD, (uchar *)cmnd    , sizeof(struct scsi_cmnd) );
	...
	if (cdb.flags & SCSI_WRITE_CDB) {
		struct scatterlist *sgl = NULL;
		uint i = 0;
		/* Restore Virtual addr */
		for (i = 0; i < HFC_SCATTERLIST_NUM; i++) {
			sgl = &cmnd->sdb.table.sgl[i];
			sgl->page_link = page_link_vaddr[i];
		}
	}
	/* FCLNX-GPL-FX-526 end */
...
int hfc_fx_get_all_port_ids( struct port_info *pp, void *arg ) {
	...
	HFC_ALLLOCK_MQ_IRQSAVE(pp,flags);	/* FCLNX-GPL-FX-466 */ /* FCLNX-GPL-FX-569 */
...
int hfc_fx_get_sid_from_wwpn( struct port_info *pp, void *arg ) {
	...
	/* Mailbox processing */
	HFC_ALLUNLOCK_MQ_IRQRESTORE(pp,flags);	/* FCLNX-GPL-FX-466 */ /* FCLNX-GPL-FX-569 */
	rtn = hfc_fx_mailbox_proc(pp, core, HFC_FX_MB_RSP_TMR,
...
CMDSKIP:
	...
	if ((!HFC_FX_MMODE_CHECK_SHARED(pp)) && HFC_FX_PHYSICAL_PORT(pp)) {
		...
		for (i=0; i<MAX_CORE_PROBE_FX; i+=MAX_CORE_PROBE_FX/pp->core_num) {
			adapstat.tx_frames		+=(uint64_t)pp->tx_frames[i];
			adapstat.tx_words		+=(uint64_t)pp->tx_words[i];
			adapstat.rx_frames		+=(uint64_t)pp->rx_frames[i];
			adapstat.rx_words		+=(uint64_t)pp->rx_words[i];
		}
	}
	/* >>> FCLNX-GPL-FX-552 */
	HFC_ALLUNLOCK_MQ_IRQRESTORE(pp,flags); /* FCLNX-GPL-FX-569 */
...
int hfc_fx_procinfo( struct port_info *pp, void *arg ) {
	...
	/* FCLNX-GPL-FX-682 start */
	if (&pport->vport_ptr[procinfo->vport_no] != NULL) {
...
//	procinfo->max_vport_count = pp->max_vport_count;
	procinfo->max_vport_count = pp->max_vport_num;	/* FCLNX-GPL-FX-558 */
	...
	procinfo->mq_mode = pp->mq_mode; /* FCLNX-GPL-FX-558 */
	procinfo->mq_num = pp->mq_num; /* FCLNX-GPL-FX-558 */
...
int hfc_fx_isolate_operation( struct port_info *pp, void *arg ) {	/* FCLNX-GPL-147 */
	...
	case HFC_FORCE_ISOLATE: 											/* FCLNX-0488 */
		...
		if (hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable()) {	/* FCLNX-0625 */
			...
			if(issue_hfcp_top != NULL){
				HFC_ALLCOREUNLOCK_MQ(pp);	/* FCLNX-GPL-FX-331 */ /* FCLNX-GPL-FX-569 */
				...
				HFC_ALLCORELOCK_MQ(pp);	/* FCLNX-GPL-FX-331 */ /* FCLNX-GPL-FX-569 */
			}
	...
	case HFC_RECOV_ISOLATE:												/* FCLNX-0488 */
		...
		if ( !(HFC_FX_MMODE_CHECK_SHARED(pp)) ){	/* FCLNX-GPL-393 */
			...
			pp->after_isolrec = 1;									/* FCLNX-GPL-FX-462,605 */
			...
			pp->after_isolrec = 0;									/* FCLNX-GPL-FX-462,605 */
			HFC_ALLUNLOCK_MQ_IRQRESTORE(pp, flags); /* FCLNX-GPL-FX-569 */
			if ( hfc_manage_info.hfcldd_mp_mod && hfc_manage_info.npubp->hfc_fx_check_mp_enable() ){
